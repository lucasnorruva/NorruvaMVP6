...
export const generateMockCodeSnippet = (
  ...
  switch (endpointKey) {
    ...
    case "getDppStatus":
      ...
      break;
    case "onchainStatus":
      urlPath = `/dpp/${params.productId || "{productId}"}/onchain-status`;
      break;
    case "onchainLifecycleStage":
      urlPath = `/dpp/${params.productId || "{productId}"}/onchain-lifecycle-stage`;
      break;
    case "logCriticalEvent":
      urlPath = `/dpp/${params.productId || "{productId}"}/log-critical-event`;
      break;
    case "registerVcHash":
      urlPath = `/dpp/${params.productId || "{productId}"}/register-vc-hash`;
      break;
    case "mintToken":
      urlPath = `/token/mint/${params.productId || "{productId}"}`;
      break;
    case "updateTokenMetadata":
      urlPath = `/token/metadata/${params.tokenId || "{tokenId}"}`;
      break;
    case "getTokenStatus":
      urlPath = `/token/status/${params.tokenId || "{tokenId}"}`;
      break;
    // Private Layer Endpoints
    case "getSupplierAttestations":
      urlPath = `/private/dpp/${params.productId || "{productId}"}/supplier/${params.supplierId || "{supplierId}"}/attestations`;
      break;
    case "postComponentTransfer":
      urlPath = `/private/dpp/${params.productId || "{productId}"}/component-transfer`;
      break;
    // ZKP Layer Endpoints
    case "zkpSubmitProof":
      urlPath = `/zkp/submit-proof/${params.dppId || "{dppId}"}`;
      break;
    case "zkpVerifyClaim":
      urlPath = `/zkp/verify-claim/${params.dppId || "{dppId}"}?claimType=${encodeURIComponent(params.claimType || "{claimType}")}`;
      break;
    default:
