...
const MetricCardWidget: React.FC<{
  title: string;
  value: string | number;
...
}> = ({ title, value, icon: Icon, description, trend, trendDirection }) => {
...
const generateMockInspectionTimelineForProduct = (
  ...
  const addEvent = (
    ...
    badgeVariant?: InspectionEvent["badgeVariant"],
  ) => {
  ...
  addEvent(
    etaDate,
    -5,
    `Departure from ${product.origin.split(",")[1] || product.origin}`,
    `Shipment left origin. Transport: ${product.transport}`,
    product.transport === "Ship"
      ? Ship
  ...
  if (product.stage.toLowerCase().includes("approaching")) {
    addEvent(
      etaDate,
      -1,
      `Approaching EU Border (${product.stage.split("(")[1]?.split(")")[0] || "Entry Port"})`,
      `Shipment nearing EU.`,
      product.transport === "Ship"
        ? Ship
        : product.transport === "Truck"
  ...
  addEvent(
    etaDate,
    0,
    `Arrival at EU Border (${product.stage.split("(")[1]?.split(")")[0] || "Entry Port"})`,
    `Product arrived. Current Stage: ${product.stage}`,
    Anchor,
    product.stage.toLowerCase().includes("cleared") ||
      product.stage.toLowerCase().includes("arrived")
      ? "Completed"
      : "In Progress",
  );
  addEvent(
    etaDate,
    0,
    "Initial Customs Scan & DPP Check",
    `Automated scan. DPP Status: ${product.dppStatus}.`,
    ScanLine,
    "Completed",
  );
  addEvent(
    ...
    `Import declarations and compliance documents checked.`,
    FileText,
    "Completed",
  );

  const isCbamRelevant =
    dpp?.compliance?.euCustomsData?.cbamGoodsIdentifier ||
    ["Automotive Parts", "Construction Materials"].includes(
      product.category || dpp?.category || "",
    );
  if (isCbamRelevant) {
    const cbamStatus = dpp?.compliance?.euCustomsData?.status
      ?.toLowerCase()
      .includes("cbam")
      ? dpp.compliance.euCustomsData.status
      : "Pending CBAM Check";
    addEvent(
      etaDate,
      1,
      "CBAM Declaration Verification",
      `Status: ${cbamStatus}. Identifier: ${dpp?.compliance?.euCustomsData?.cbamGoodsIdentifier || "N/A"}`,
      Globe,
      cbamStatus.includes("Pending") ? "Action Required" : "Completed",
      cbamStatus.includes("Pending") ? "outline" : "default",
    );
  }

  if (
    product.dppStatus === "Pending Review" ||
    product.dppStatus === "Non-Compliant" ||
    product.id === "PROD789"
  ) {
    addEvent(
      etaDate,
      ...
      `Reason: ${product.dppStatus === "Non-Compliant" ? "DPP Discrepancy" : product.dppStatus === "Pending Review" ? "Incomplete Data" : "Random Check"}.`,
      AlertTriangle,
      "Action Required",
      "outline",
    );
    addEvent(
      ...
      `Results: ${product.dppStatus === "Non-Compliant" ? "Minor issues, report filed." : "Passed."}`,
      CheckCircle,
      "Completed",
      product.dppStatus === "Non-Compliant" ? "destructive" : "default",
    );
  }

  const clearanceOffset =
    product.dppStatus === "Pending Review" ||
    product.dppStatus === "Non-Compliant" ||
    product.id === "PROD789"
      ? isCbamRelevant
  ...
  addEvent(
    ...
    `Product cleared for entry into EU market.`,
    ShieldCheck,
    "Completed",
    "default",
  );
  addEvent(
    etaDate,
    clearanceOffset,
    `Released for Inland Transit to ${product.destination.split(",")[0]}`,
    `Released to logistics partner.`,
    Truck,
    "In Progress",
  );
  addEvent(
    etaDate,
    clearanceOffset + 2,
    `Arrival at Destination Warehouse (${product.destination.split(",")[0]})`,
    `Expected delivery.`,
    Warehouse,
    "Upcoming",
  );
...
export default function CustomsDashboardPage() {
  ...
  const [selectedProductForTimeline, setSelectedProductForTimeline] =
    useState<TransitProduct | null>(null);
  const [dynamicInspectionTimeline, setDynamicInspectionTimeline] = useState<
    InspectionEvent[]
  >([]);

  const filteredProducts = country
    ? MOCK_TRANSIT_PRODUCTS.filter(
        (p) =>
  ...
  const filteredAlerts = country
    ? MOCK_CUSTOMS_ALERTS.filter((alert) => {
        const prod = MOCK_TRANSIT_PRODUCTS.find(
  ...
  const handleViewTimeline = useCallback((product: TransitProduct) => {
    ...
    setDynamicInspectionTimeline(
      generateMockInspectionTimelineForProduct(product, dpp),
    );
  }, []);
  ...
  useEffect(() => {
    if (!selectedProductForTimeline && filteredProducts.length > 0) {
      ...
      !filteredProducts.find((p) => p.id === selectedProductForTimeline.id)
    ) {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-5">
        {" "}
        {/* Adjusted for 5 metrics */}
        <MetricCardWidget
          title="Shipments in Transit (EU)"
          value="132"
          icon={Truck}
          description="Active customs entries"
          trend="+5 from last hour"
          trendDirection="up"
        />
        <MetricCardWidget
          title="Products at Customs Checkpoints"
          value="27"
          icon={Anchor}
          description="Ports, Airports, Land Borders"
          trend="+3 new arrivals"
          trendDirection="up"
        />
        <MetricCardWidget
          title="Overall DPP Compliance Rate"
          value="92%"
          icon={ShieldCheck}
          description="For incoming goods"
          trend="-1% vs last week"
          trendDirection="down"
        />
        <MetricCardWidget
          title="Products Under CBAM Review"
          value="18"
          icon={Globe}
          description="Steel, Aluminium, Cement etc."
          trend="+2 new"
          trendDirection="up"
        />
        <MetricCardWidget
          title="Flagged for Inspection"
          value="5"
          icon={AlertTriangle}
          description="2 critical issues pending"
          trend="No change"
      ...
      <Card className="shadow-lg">
        <CardHeader>
          <CardTitle className="font-headline flex items-center">
            ...
            Products in Transit / At Customs
          </CardTitle>
          <CardDescription>
            Overview of products. Click "View Timeline" for details. CBAM
            relevant products are indicated.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
            ...
            <TableBody>
              {filteredProducts.map((product) => {
                ...
                const isEtaToday =
                  etaDate.toDateString() === today.toDateString();
                ...
                const cbamIdentifier =
                  dppEntry?.compliance?.euCustomsData?.cbamGoodsIdentifier;
                const isCbamCatRelevant = [
                  "Automotive Parts",
                  "Construction Materials",
                ].includes(product.category || dppEntry?.category || "");
                const cbamStatusText =
                  dppEntry?.compliance?.euCustomsData?.status;
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell className="text-xs">
                      ...
                      {cbamStatusText &&
                        cbamStatusText.toLowerCase().includes("cbam") && (
                    ...
                    <TableCell className="capitalize flex items-center">
                      ...
                      {product.transport === "Truck" && (
                        <Truck className="h-4 w-4 mr-1.5 text-orange-500" />
                      )}
                      {product.transport === "Plane" && (
                        <Plane className="h-4 w-4 mr-1.5 text-purple-500" />
                      )}
                      {product.transport}
                    </TableCell>
                    <TableCell className="text-xs text-muted-foreground">
                      {product.origin} &rarr; {product.destination}
                    </TableCell>
                    <TableCell>
                      {isEtaPast ? (
        ...
        <Card className="shadow-lg md:col-span-2">
          <CardHeader>
            <CardTitle className="font-headline flex items-center">
              ...
              {selectedProductForTimeline
                ? `for ${selectedProductForTimeline.name} (${selectedProductForTimeline.id})`
                : "(Select a Product)"}
            </CardTitle>
            <CardDescription>
              Chronological overview of a product's conceptual journey through
              customs.
            </CardDescription>
          </CardHeader>
          <CardContent className="pr-2">
            {selectedProductForTimeline ? (
              <ul className="space-y-3 max-h-[450px] overflow-y-auto">
                {dynamicInspectionTimeline.map((event) => {
                  ...
                  return (
                    <li
                      key={event.id}
                      className="flex items-start space-x-3 p-3.5 border rounded-md bg-background hover:bg-muted/30 transition-colors shadow-sm"
                    >
                      <div
                        className={cn(
                          "flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center",
                          badgeColorClass.split(" ")[0],
                        )}
                      >
                        <event.icon
                          className={cn(
              ...
              <p className="text-sm text-muted-foreground text-center py-10">
                Select a product from the table above to view its detailed
                inspection timeline.
              </p>
            )}
          </CardContent>
        ...
        <div className="space-y-6 md:col-span-1">
          <Card className="shadow-lg">
            <CardHeader>
              ...
              <CardDescription>
                Breakdown of incoming products by DPP compliance status.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div>
                ...
                <Progress
                  value={9}
                  className="h-2.5 [&>div]:bg-yellow-500"
                  aria-label="9% Pending Review/Data"
                />
              </div>
              <div>
                <div className="flex justify-between mb-1 text-sm">
                ...
                <Progress
                  ...
                  aria-label="6% Issues/Non-Compliant"
                />
              </div>
              <p className="text-xs text-muted-foreground pt-2">
                Note: This is a static mock representation.
              </p>
            </CardContent>
          </Card>
          <Card className="shadow-lg">
            <CardHeader>
              ...
              <CardDescription>
                Products currently flagged or requiring attention at customs.
              </CardDescription>
            </CardHeader>
            <CardContent>
              {filteredAlerts.length > 0 ? (
                <ul className="space-y-3 max-h-[200px] overflow-y-auto">
                  {filteredAlerts.map((alert) => (
                    <li
                      key={alert.id}
                      className="p-3 border rounded-md bg-background hover:bg-muted/30"
                    >
