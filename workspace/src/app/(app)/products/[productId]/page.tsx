...
import { fetchProductDetails } from "@/utils/productDetailUtils";

export default function ProductDetailPage() {
  const params = useParams();
  const productId = params.productId as string;
  const [product, setProduct] = useState<
    SimpleProductDetail | null | undefined
  >(undefined);
  ...
  const handleSupplyChainUpdate = (updatedLinks: ProductSupplyChainLink[]) => {
    ...
    if (product.id.startsWith("USER_PROD")) {
      try {
        ...
        if (productIndex > -1) {
          ...
          toast({
            ...
            description:
              "Could not find product in local storage to update supply chain.",
            variant: "destructive",
          });
        }
      } catch (error) {
        toast({
          title: "Storage Error",
          description: "Failed to save supply chain updates to local storage.",
          variant: "destructive",
        });
      ...
      if (mockDppIndex > -1) {
        ...
        MOCK_DPPS[mockDppIndex].metadata.last_updated =
          new Date().toISOString();
      }
      toast({
        title: "Supply Chain Updated (Session Only)",
        description:
          "Supply chain links updated for this session (mock product).",
        variant: "default",
      });
    }
  };
  ...
  const handleSyncEprel = async () => {
    if (!product || !product.modelNumber) {
      toast({
        ...
        description: "Product model number is required to sync with EPREL.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      const currentComplianceSummary = product.complianceSummary || {
        overallStatus:
      ...
      if (
        result.syncStatus.toLowerCase().includes("successfully") ||
        result.syncStatus.toLowerCase().includes("mismatch")
      ) {
        eprelIdToSet = result.eprelId;
      } else if (
        result.syncStatus.toLowerCase().includes("not found") ||
        result.syncStatus.toLowerCase().includes("error")
      ) {
      ...
      if (product.id.startsWith("USER_PROD")) {
        ...
        if (productIndex > -1) {
          if (!userProducts[productIndex].complianceSummary) {
            userProducts[productIndex].complianceSummary = {
              overallStatus: "N/A",
            };
          }
        ...
        const mockDppIndex = MOCK_DPPS.findIndex(
          (dpp) => dpp.id === product.id,
        );
      ...
      toast({
        title: "EPREL Sync Error",
        description: `An unexpected error occurred during EPREL sync. ${error instanceof Error ? error.message : ""}`,
        variant: "destructive",
      });
