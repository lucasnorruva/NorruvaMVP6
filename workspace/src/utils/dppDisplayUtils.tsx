...
const BASE_ESSENTIAL_FIELDS_CONFIG: Array<{
  ...
  {
    key: "productDetails.description",
    label: "Description",
    check: (p) => !!p.productDetails?.description,
  },
  {
    key: "productDetails.imageUrl",
    label: "Image URL",
    check: (p) =>
      !!p.productDetails?.imageUrl &&
      !p.productDetails.imageUrl.includes("placehold.co") &&
      !p.productDetails.imageUrl.includes("?text="),
  },
  {
    key: "productDetails.materials",
    label: "Materials Info",
    check: (p) =>
      !!p.productDetails?.materials && p.productDetails.materials.trim() !== "",
  },
  {
    key: "productDetails.sustainabilityClaims",
    label: "Sustainability Claims",
    check: (p) =>
      !!p.productDetails?.sustainabilityClaims &&
      p.productDetails.sustainabilityClaims.trim() !== "",
  },
  {
    key: "productDetails.energyLabel",
    label: "Energy Label",
    categoryScope: ["Appliances", "Electronics"],
  },
  {
    key: "productDetails.specifications",
    label: "Specifications",
    check: (p) => {
      if (typeof p.productDetails?.specifications === "string")
        return (
          !!p.productDetails.specifications &&
          p.productDetails.specifications.trim() !== "" &&
          p.productDetails.specifications.trim() !== "{}"
        );
      if (
        typeof p.productDetails?.specifications === "object" &&
        p.productDetails.specifications !== null
      )
        return Object.keys(p.productDetails.specifications).length > 0;
      return false;
    },
  },
  {
  ...
  {
    key: "productDetails.carbonFootprint.value",
    label: "General Carbon Footprint Value",
    check: (p) =>
      p.productDetails?.carbonFootprint?.value !== null &&
      p.productDetails?.carbonFootprint?.value !== undefined,
  },
  {
    key: "productDetails.esprSpecifics.durabilityInformation",
    label: "ESPR Durability Info",
    categoryScope: ["Appliances", "Electronics", "Furniture", "Textiles"],
  }, // Example category scope
];
...
const BATTERY_FIELDS_CONFIG: Array<{
  ...
  {
    key: "batteryRegulation.batteryChemistry",
    label: "Battery Chemistry",
    check: (p) => !!p.batteryRegulation?.batteryChemistry,
  },
  {
    key: "batteryRegulation.stateOfHealth.value",
    label: "Battery State of Health (SoH)",
    check: (p) =>
      p.batteryRegulation?.stateOfHealth?.value !== null &&
      p.batteryRegulation?.stateOfHealth?.value !== undefined,
  },
  {
    key: "batteryRegulation.carbonFootprint.value",
    label: "Battery Mfg. Carbon Footprint",
    check: (p) =>
      p.batteryRegulation?.carbonFootprint?.value !== null &&
      p.batteryRegulation?.carbonFootprint?.value !== undefined,
  },
  {
    key: "batteryRegulation.recycledContent",
    label: "Battery Recycled Content",
    check: (p) => (p.batteryRegulation?.recycledContent || []).length > 0,
  },
];
...
export const calculateDppCompletenessForList = (
  ...
  const isBatteryRelevantCategory =
    ...
    (product.batteryRegulation &&
      product.batteryRegulation.status !== "not_applicable");
