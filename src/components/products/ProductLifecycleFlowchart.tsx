...
const getStatusClasses = (
  ...
  if (isCurrent) {
    ...
    bgColor =
      status === "in_progress" ? "bg-primary/20 dark:bg-primary/30" : bgColor;
  }
...
const MetricStatusIcon = ({ status }: { status: Metric["status"] }) => {
  ...
  if (status === "pending_review")
    return <InfoIcon className="h-4 w-4 text-warning" />;
...
const SustainabilityMetricChart = ({ metric }: { metric: Metric }) => {
  ...
  const data = [
    { name: metric.name, value: metric.value, target: metric.targetValue },
  ];
        ...
        >
          ...
          <RechartsTooltip
            ...
            formatter={(value: number, name: string, props) => {
              const currentMetric =
                props.payload && (props.payload as any).metric
                  ? ((props.payload as any).metric as Metric)
                  : metric;
...
const ProductLifecycleFlowchart: React.FC<ProductLifecycleFlowchartProps> = ({
  phases,
  currentPhaseIndex,
}) => {
  if (!phases || phases.length === 0) {
    return (
      <p className="text-muted-foreground">
        No lifecycle phases defined for this product.
      </p>
    );
  }
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Track the product&apos;s progress through its lifecycle stages.
        </CardDescription>
      </CardHeader>
      <CardContent>
        <div className="overflow-x-auto pb-4">
          <div className="flex items-start space-x-2 min-w-max">
            {phases.map((phase, index) => {
              ...
              const { bgColor, textColor, borderColor } = getStatusClasses(
                phase.status,
                isCurrent,
              );
                ...
                <React.Fragment key={phase.id}>
                  <Popover>
                    <PopoverTrigger asChild>
                      ...
                      >
                        <IconComponent
                          className={cn("h-8 w-8 mb-2", textColor)}
                        />
                    ...
                    <PopoverContent
                      className="w-80 shadow-xl z-50 bg-card"
                      side="bottom"
                      align="center"
                    >
                      <div className="space-y-1">
                              ...
                              <ul className="space-y-1 text-xs">
                                {phase.keyDocuments.map((doc) => {
                                  ...
                                  return (
                                    <li
                                      key={doc.name}
                                      className="flex items-center justify-between p-1.5 bg-muted/30 rounded-sm"
                                    >
                                ...
                                >
                                  ...
                                  {event.status && (
                                    <Badge
                                      ...
                                      className={`text-[0.6rem] px-1.5 py-0.5 ${event.status === "completed" ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}
                                    >
                                      {event.status}
                                    </Badge>
                                  )}
                        ...
                        {phase.complianceMetrics &&
                          phase.complianceMetrics.length > 0 && (
                            <div className="pt-2 mt-2 border-t border-border/50">
                              <h5 className="text-xs font-semibold mb-1 text-muted-foreground uppercase tracking-wider">
                                Compliance Checks:
                              </h5>
                              <ul className="space-y-1 text-xs">
                                {phase.complianceMetrics.map((metric) => (
                                  <li
                                    key={metric.name}
                                    className="flex items-center justify-between p-1.5 bg-muted/50 rounded-sm"
                                  >
                        ...
                        {phase.sustainabilityMetrics &&
                          phase.sustainabilityMetrics.length > 0 && (
                            <div className="pt-2 mt-2 border-t border-border/50">
                              <h5 className="text-xs font-semibold mb-1 text-muted-foreground uppercase tracking-wider">
                              ...
                              <div className="space-y-1 text-xs">
                                {phase.sustainabilityMetrics.map((metric) => (
                                  <div
                                    key={metric.name}
                                    className="py-1 p-1.5 bg-muted/50 rounded-sm"
                                  >
