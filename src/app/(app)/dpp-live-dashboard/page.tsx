...
export default function DPPLiveDashboardPage() {
  ...
  const [selectedProductForSummary, setSelectedProductForSummary] =
    useState<DigitalProductPassport | null>(null);

  const handleViewAISummary = useCallback(
    async (productId: string) => {
      const product = dpps.find((p) => p.id === productId);
      if (!product) {
        toast({
          title: "Error",
          description: "Product not found for AI summary.",
          variant: "destructive",
        });
        return;
      }
      ...
      try {
        ...
        if (
          product.productDetails?.sustainabilityClaims &&
          product.productDetails.sustainabilityClaims.length > 0
        ) {
          sustainabilityInfoParts.push(
            product.productDetails.sustainabilityClaims
              .map((c) => c.claim)
              .join(", "),
          );
        ...
        if (
          ...
          product.productDetails.materials.length > 0
        ) {
          sustainabilityInfoParts.push(
            "Materials: " +
              product.productDetails.materials
                .map((m) => `${m.name}${m.isRecycled ? " (recycled)" : ""}`)
                .join(", "),
          );
        }
        if (product.productDetails?.energyLabel) {
          sustainabilityInfoParts.push(
            `Energy Label: ${product.productDetails.energyLabel}`,
          );
        }
        if (product.productDetails?.repairabilityScore) {
          sustainabilityInfoParts.push(
            `Repairability Score: ${product.productDetails.repairabilityScore.value}/${product.productDetails.repairabilityScore.scale}`,
          );
        }
        const sustainabilityInformation =
          sustainabilityInfoParts.filter(Boolean).join(". ") ||
          "General sustainability information not detailed.";

        const complianceInfoParts = [];
        if (product.compliance.eprel) {
          let eprelStr = "EPREL: ";
          if (product.compliance.eprel.id)
            eprelStr += `ID ${product.compliance.eprel.id}, `;
          eprelStr += `Status ${product.compliance.eprel.status}`;
          complianceInfoParts.push(eprelStr);
        }
        if (product.compliance.esprConformity) {
          complianceInfoParts.push(
            `ESPR Conformity: ${product.compliance.esprConformity.status}`,
          );
        }
        if (product.compliance.battery_regulation) {
          complianceInfoParts.push(
            `Battery Regulation: ${product.compliance.battery_regulation.status}`,
          );
        }
        if (product.ebsiVerification?.status) {
          const ebsiStatusText = product.ebsiVerification.status.replace(
            /_/g,
            " ",
          );
          complianceInfoParts.push(
            `EBSI Status: ${ebsiStatusText.charAt(0).toUpperCase() + ebsiStatusText.slice(1)}`,
          );
        }
        const complianceInformation =
          complianceInfoParts.filter(Boolean).join(". ") ||
          "General compliance information not detailed.";

        const input = {
          productName: product.productName,
          productDescription:
            product.productDetails?.description ||
            `A product in the ${product.category} category. No detailed description available.`,
    ...
    <div className="space-y-6 p-4 md:p-6 lg:p-8">
      ...
      <Card className="shadow-lg">
        <CardHeader>
          ...
          <CardDescription>
            Explore and manage all active Digital Product Passports. Click any
            ID for public view, or use Actions for more options.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <DPPTable
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This action will delete product "
              {dpps.find((p) => p.id === productToDeleteId)?.productName ||
                productToDeleteId}
              ".
              {productToDeleteId &&
                !productToDeleteId.startsWith("USER_PROD") &&
                " (This is a system mock product; deletion is temporary for this session.)"}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
