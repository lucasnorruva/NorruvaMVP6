...
import { PlusCircle, MoreHorizontal, Edit, Trash2, Eye, PackageSearch, Factory, CheckCircle2 } from "lucide-react";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
import type { MockProductType } from "@/app/(app)/products/[productId]/page"; // Import for richer structure

// Use a type that can accommodate both rich mock products and stored user products for completeness calculation
export interface DisplayableProduct extends Partial<MockProductType>, Partial<ProductFormData> {
  id: string; // MockProductType uses productId, StoredUserProduct uses id
  productName: string; // Ensure productName is always present
  productCategory?: string; // from ProductFormData
  category?: string; // from MockProductType
  status: string;
  compliance: string;
  lastUpdated: string;
  // Ensure all fields needed for calculateDppCompleteness are optional here
  // If a field is named differently (e.g. id vs productId), handle in mapping
}
...
const calculateDppCompletenessForList = (product: DisplayableProduct): { score: number; filledFields: number; totalFields: number; missingFields: string[] } => {
  const essentialFieldsConfig: Array<{ key: keyof DisplayableProduct | string; label: string; check?: (p: DisplayableProduct) => boolean; categoryScope?: string[] }> = [
    { key: 'productName', label: 'Product Name' }, { key: 'gtin', label: 'GTIN' }, { key: 'category', label: 'Category', check: p => !!(p.category || p.productCategory) }, { key: 'manufacturer', label: 'Manufacturer' }, { key: 'modelNumber', label: 'Model Number' }, { key: 'description', label: 'Description', check: p => !!(p.description || p.productDescription)}, { key: 'imageUrl', label: 'Image URL', check: (p) => !!p.imageUrl && !p.imageUrl.includes('placehold.co') && !p.imageUrl.includes('?text=') }, { key: 'materials', label: 'Materials' }, { key: 'sustainabilityClaims', label: 'Sustainability Claims' }, { key: 'energyLabel', label: 'Energy Label', categoryScope: ['Appliances', 'Electronics'] }, { key: 'specifications', label: 'Specifications', check: (p) => {
      if (typeof p.specifications === 'string') return !!p.specifications && p.specifications !== '{}'; // For StoredUserProduct
      if (typeof p.specifications === 'object') return p.specifications && Object.keys(p.specifications).length > 0; // For MockProductType
      return false;
    }},
  ...
  essentialFieldsConfig.forEach(fieldConfig => {
    ...
    // Adapt for DisplayableProduct which might have fields from either source
...
const initialMockProducts: DisplayableProduct[] = [
  { 
    id: "PROD001", productId: "PROD001", productName: "EcoFriendly Refrigerator X2000", productCategory: "Appliances", category: "Appliances", status: "Active", compliance: "Compliant", lastUpdated: "2024-07-20",
    gtin: "01234567890123", manufacturer: "GreenTech Appliances", modelNumber: "X2000-ECO", description: "State-of-the-art energy efficient refrigerator.", imageUrl: "https://placehold.co/600x400.png", materials: "Recycled Steel, Bio-polymers", sustainabilityClaims: "Energy Star Certified", energyLabel: "A+++", specifications: { "Capacity": "400L" }, lifecycleEvents: [{id:"lc1", name:"Manufacturing", status:"completed", icon:Factory}], complianceData: {REACH: {status:"Compliant", lastChecked:"2023-01-01", reportId:"test"}},
  },
  { 
    id: "PROD002", productId: "PROD002", productName: "Smart LED Bulb Pack (4-pack)", productCategory: "Electronics", category: "Electronics", status: "Active", compliance: "Pending", lastUpdated: "2024-07-18",
    gtin: "98765432109876", manufacturer: "BrightSpark Electronics", modelNumber: "BS-LED-S04B", description: "Tunable white and color smart LED bulbs.", imageUrl: "https://placehold.co/600x400.png", materials: "Polycarbonate, Aluminum", sustainabilityClaims: "Uses 85% less energy", energyLabel: "A+", specifications: { "Lumens": "800lm" }, batteryChemistry: "Li-ion", stateOfHealth: 99, carbonFootprintManufacturing: 5, recycledContentPercentage: 10,
    lifecycleEvents: [{id:"lc1", name:"Manufacturing", status:"completed", icon:Factory}], complianceData: {RoHS: {status:"Compliant", lastChecked:"2023-01-01", reportId:"test"}}
  },
  { 
    id: "PROD003", productId: "PROD003", productName: "Organic Cotton T-Shirt", productCategory: "Apparel", category: "Apparel", status: "Archived", compliance: "Compliant", lastUpdated: "2024-06-10",
    description: "100% organic cotton t-shirt.", materials: "Organic Cotton", sustainabilityClaims: "GOTS Certified",
  },
  { 
    id: "PROD004", productId: "PROD004", productName: "Recycled Plastic Water Bottle", productCategory: "Homeware", category: "Homeware", status: "Active", compliance: "Non-Compliant", lastUpdated: "2024-07-21",
    description: "Made from 100% recycled ocean-bound plastic.", materials: "Recycled PET", sustainabilityClaims: "Reduces ocean plastic",
  },
  { 
    id: "PROD005", productId: "PROD005", productName: "Solar Powered Garden Light", productCategory: "Outdoor", category: "Outdoor", status: "Draft", compliance: "N/A", lastUpdated: "2024-07-22",
    description: "Solar-powered LED light for gardens.", materials: "Aluminum, Solar Panel", energyLabel: "N/A"
  },
];
...
interface StoredUserProduct extends ProductFormData {
  ...
  compliance: string;
  lastUpdated: string;
  // Add origin fields if they exist in your StoredUserProduct structure from new/page.tsx
...
export default function ProductsPage() {
  ...
  useEffect(() => {
    ...
    const userAddedDisplayableProducts: DisplayableProduct[] = userAddedStoredProducts.map(p => ({
      ...
      id: p.id, // ensure id is mapped from StoredUserProduct
      productId: p.id, // also map to productId for compatibility with MockProductType parts
      productName: p.productName || "Unnamed Product",
      category: p.productCategory, // Keep productCategory for user products
                                   // It will be used by calculateDppCompletenessForList
      // other fields from ProductFormData are already part of StoredUserProduct
    }));

    const combinedProducts = [
      ...initialMockProducts.filter(mock => !userAddedDisplayableProducts.find(userProd => userProd.id === mock.id)),
                  ...
                  <TableCell>
                    <TooltipProvider>
                      <Tooltip delayDuration={100}>
                        ...
                        <TooltipContent align="start" className="bg-background shadow-lg p-3 rounded-md border">
