    ...
    event.respondWith(
      (async () => {
        try {
          ...
          return offlineResponse || new Response("Offline fallback not found", {status: 404, headers: {"Content-Type": "text/plain"}});
        }
      })()
    );
  } else {
    ...
    event.respondWith(
      caches.match(request).then((cachedResponse) => {
        ...
        return fetch(request).then((networkResponse) => {
          ...
          if (request.url.includes('/_next/static/') && networkResponse && networkResponse.ok && networkResponse.type !== 'opaque') {
            const cache = caches.open(CACHE_NAME);
            cache.then(c => c.put(request, networkResponse.clone()));
          }
          return networkResponse;
        }).catch(error => {
            console.warn('[Service Worker] Fetch failed for non-navigation, no cache hit:', request.url, error);
