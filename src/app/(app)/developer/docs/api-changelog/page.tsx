...
const mockChangelogEntries = [
  {
    ...
    type: "Feature",
    summary:
      "Added new endpoints for managing supplier-specific data sections within a DPP. Enhanced filtering options for the `/dpp` list endpoint.",
    details: [
  ...
  },
  {
    ...
    type: "Improvement",
    summary:
      "Improved performance for QR code validation endpoint. Updated EBSI integration mock responses for better testing.",
    details: [
  ...
  },
  {
    ...
    summary:
      "Initial public release of the Norruva DPP API. Core endpoints for DPP management and retrieval available.",
    details: ["GET `/dpp/{dppId}`", "POST `/dpp`", "GET `/qr/validate`"],
  },
  {
    version: "v0.9.0 (Beta)",
    date: "2024-06-01",
    type: "Beta",
    summary:
      "Internal beta release with focus on EU Battery Regulation data points and basic DPP structure.",
    details: [
...
export default function ApiChangelogPage() {
  const getBadgeVariant = (
    ...
    if (
      type.toLowerCase() === "feature" ||
      type.toLowerCase() === "initial release"
    )
      return "default";
    if (type.toLowerCase() === "improvement") return "outline";
    if (type.toLowerCase() === "fix" || type.toLowerCase() === "security")
      return "destructive";
    return "secondary";
  };
  const getBadgeClass = (type: string) => {
    ...
    if (type.toLowerCase() === "improvement")
        ...
        <CardContent className="space-y-6">
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-2">
                ...
                <div className="flex items-center gap-3 mt-1 sm:mt-0">
                  <Badge
                    ...
                    className={cn(getBadgeClass(entry.type))}
                  >
                    {entry.type}
                  </Badge>
          ...
          {mockChangelogEntries.length === 0 && (
            <p className="text-muted-foreground text-center py-4">
              No changelog entries available yet. Check back soon for updates!
            </p>
          )}
        </CardContent>
      </Card>
      ...
      <Card className="shadow-md">
        ...
        <CardContent>
          <p className="text-sm text-muted-foreground">
            Our API follows semantic versioning principles (Major.Minor.Patch).
            Breaking changes will result in a new Major version. Minor versions
            introduce new features in a backward-compatible manner. Patch
            versions are for backward-compatible bug fixes.
          </p>
        </CardContent>
      </Card>
    </DocsPageLayout>
