...
export async function GET(
  ...
  if (productWithToken) {
    return NextResponse.json({
      ...
      mintedAt: productWithToken.metadata.created_at || productWithToken.metadata.last_updated || new Date(Date.now() - Math.random() * 1000000000).toISOString(),
    ...
    if (isNaN(parseInt(tokenId))) { // Original check for non-numeric token IDs
       return NextResponse.json({ error: { code: 404, message: `Token with ID ${tokenId} not found or invalid format.` } }, { status: 404 });
    }
    return NextResponse.json({
      tokenId: tokenId,
      contractAddress: "DEFAULT_DPP_TOKEN_CONTRACT", 
      ownerAddress: `0xmock_owner_unlinked_${tokenId.slice(-4)}`,
      mintedAt: new Date(Date.now() - Math.random() * 1000000000).toISOString(),
