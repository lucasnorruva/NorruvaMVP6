...
self.addEventListener('install', (event) => {
  event.waitUntil(
    caches.open(CACHE_VERSION)
      .then((cache) => {
        console.log('[ServiceWorker] Caching core assets:', CORE_ASSETS_TO_CACHE);
      ...
      .catch(error => {
        console.error('[ServiceWorker] Failed to cache core assets during install:', error);
      })
  );
});
            ...
            return caches.match(request).then((cachedResponse) => {
              ...
              const offlineErrorBody = {
                error: {
                  ...
                  message: 'Network error. The requested API data is not available offline and could not be fetched from the cache.',
                },
              };
