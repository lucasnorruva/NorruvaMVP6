...
export default function ComplianceHotspotsTable() {
  const hotspotsData = useMemo(() => {
    const categoryMap: Record<string, { nonCompliant: number; pending: number; total: number }> = {};
    MOCK_DPPS.forEach(dpp => {
      ...
      } else if (overallStatus === 'pending review' || overallStatus === 'pending') {
    ...
    return Object.entries(categoryMap)
      ...
      .filter(item => item.nonCompliantCount > 0 || item.pendingReviewCount > 0) // Only show categories with issues
      .sort((a, b) => (b.nonCompliantCount + b.pendingReviewCount) - (a.nonCompliantCount + a.pendingReviewCount)); // Sort by total issues
  }, []);
  ...
  return (
    <Card className="shadow-lg lg:col-span-2"> {/* Adjusted span */}
      <CardHeader>
        ...
        <CardDescription>Categories with products requiring compliance attention.</CardDescription>
      </CardHeader>
      <CardContent>
            ...
            <TableBody>
              {hotspotsData.map(hotspot => (
                <TableRow key={hotspot.category}>
                  <TableCell className="font-medium">{hotspot.category}</TableCell>
                  <TableCell className="text-center">
                    {hotspot.nonCompliantCount > 0 ? (
                      <Badge variant="destructive" className="bg-red-100 text-red-700 border-red-300">
                        <ShieldAlert className="mr-1 h-3 w-3" /> {hotspot.nonCompliantCount}
                      </Badge>
                    ) : (
                  ...
                  <TableCell className="text-center">
                     {hotspot.pendingReviewCount > 0 ? (
                      <Badge variant="outline" className="bg-yellow-100 text-yellow-700 border-yellow-300">
                         <ShieldQuestion className="mr-1 h-3 w-3" /> {hotspot.pendingReviewCount}
                      </Badge>
                    ) : (
                       <span className="text-muted-foreground">-</span>
                    )}
                  </TableCell>
                  <TableCell className="text-center">{hotspot.totalInCategory}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
        ) : (
          <p className="text-center text-muted-foreground py-6">No significant compliance hotspots identified across categories.</p>
        )}
      </CardContent>
    </Card>
  );
