...
const calculateOverallStatus = (dpp: DigitalProductPassport): string => {
  ...
  if (compliantCount === regulationsChecked.length && regulationsChecked.length > 0) return "Fully Compliant";
...
export async function GET(
  ...
  if (product) {
    const complianceSummary = {
      ...
      batteryRegulationStatus: product.compliance.battery_regulation?.status || "N/A",
      esprStatus: product.compliance.eu_espr?.status || (product.compliance.esprConformity?.status ? `${product.compliance.esprConformity.status} (Conformity)` : "N/A"),
    ...
    return NextResponse.json({ error: { code: 404, message: `Compliance summary for product ID ${productId} not found.` } }, { status: 404 });
  }
}
