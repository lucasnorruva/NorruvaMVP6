...
import { FormField, FormItem, FormLabel, FormControl, FormDescription, FormMessage } from "@/components/ui/form"; // Keep for fields not in sub-components
import { Textarea } from "@/components/ui/textarea"; // Keep for fields not in sub-components
import { Input } from "@/components/ui/input"; // Keep for fields not in sub-components
...
const formSchema = z.object({
  ...
  imageHint: z.string().max(30, "Hint should be concise, max 2-3 words or 30 chars.").optional(), // Added max length
...
// AiIndicator can be defined here if not used by other sub-components, or moved to a shared util
// For this refactor, assuming BasicInfoFormSection has its own, and ProductImageFormSection will too.
...
export default function ProductForm({ id, initialData, onSubmit, isSubmitting, isStandalonePage = true }: ProductFormProps) {
  ...
  const [isGeneratingImage, setIsGeneratingImage] = useState(false); // State managed here
      ...
      <AccordionItem value="item-1">
        ...
        <AccordionContent>
          <BasicInfoFormSection
            ...
            setIsSuggestingName={setIsSuggestingName} // Pass setter if needed internally by sub-component, or keep state here
            callSuggestNameAI={callSuggestNameAI}
            isSuggestingDescription={isSuggestingDescription}
            setIsSuggestingDescription={setIsSuggestingDescription} // Pass setter
      ...
      <AccordionItem value="item-5">
        ...
        <AccordionContent className="pt-4">
          <ProductImageFormSection
            ...
            toast={toast}
            isGeneratingImageState={isGeneratingImage} // Pass state
            setIsGeneratingImageState={setIsGeneratingImage} // Pass setter
