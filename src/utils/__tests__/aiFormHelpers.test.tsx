...
describe("aiFormHelpers", () => {
  ...
  describe("handleSuggestNameAI", () => {
    it("should call generateProductName and update form on success", async () => {
      ...
      (generateProductName as jest.Mock).mockResolvedValue({
    ...
    it("should show error toast if AI flow fails", async () => {
      ...
      const { generateProductName } = await import(
        "@/ai/flows/generate-product-name-flow"
      ); // Import here
      (generateProductName as jest.Mock).mockRejectedValue(
  ...
  describe("handleGenerateImageAI", () => {
    ...
    it("should show error toast if product name is missing", async () => {
      ...
      const { generateProductImage } = await import(
        "@/ai/flows/generate-product-image-flow"
      ); // Import here
      ...
      expect(mockToast).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          description:
            "Please enter a product name before generating an image.",
          variant: "destructive",
        }),
      );
    ...
    it("should show error toast if AI image generation fails", async () => {
      ...
      (generateProductImage as jest.Mock).mockRejectedValue(
        new Error(errorMessage),
      );
