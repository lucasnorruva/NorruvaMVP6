...
const BASE_ESSENTIAL_FIELDS_CONFIG: Array<{
  ...
  { key: 'productDetails.specifications', label: 'Specifications', check: (p) => {
      if (typeof p.productDetails?.specifications === 'string') return !!p.productDetails.specifications && p.productDetails.specifications.trim() !== '' && p.productDetails.specifications.trim() !== '{}';
      if (typeof p.productDetails?.specifications === 'object' && p.productDetails.specifications !== null) return Object.keys(p.productDetails.specifications).length > 0;
      return false;
    }
  },
