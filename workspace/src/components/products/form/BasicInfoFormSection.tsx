...
import { handleSuggestNameAI, handleSuggestDescriptionAI } from "@/utils/aiFormHelpers";
...
interface BasicInfoFormSectionProps {
  ...
  onFetchCategoryComplianceSummary?: (category: string, focusedRegulations?: string) => void; // Handler from parent
}
...
export default function BasicInfoFormSection({
  ...
  const [categoryComplianceError, setCategoryComplianceError] = useState<string | null>(null);
    ...
    const fetchSummary = async () => {
      if (productCategoryValue && productCategoryValue.trim() !== "") {
        ...
        try {
          ...
          const result = await generateComplianceSummaryForCategory({ productCategory: productCategoryValue });
  ...
  const callSuggestDescriptionAIInternal = async () => {
    ...
    if (result) {
      ...
      form.setValue("productDetails.description", result, { shouldValidate: true });
      // Update the origin for the description within productDetailsOrigin
      form.setValue("productDetailsOrigin.descriptionOrigin" as any, 'AI_EXTRACTED');
    }
  };
    ...
    <div className="space-y-6 pt-4">
      ...
      {categoryComplianceSummary && !isLoadingCategoryCompliance && (
        <Alert className="mt-2 bg-blue-500/10 border-blue-500/30 text-blue-700 dark:text-blue-300">
          ...
          <AlertTitle className="font-semibold !text-blue-700 dark:!text-blue-300">AI Compliance Insights for "{productCategoryValue}"</AlertTitle>
      ...
      <FormField
        ...
        render={({ field }) => (
          <FormItem>
            <div className="flex items-center justify-between">
              <FormLabel className="flex items-center">Product Description <AiIndicator fieldOrigin={form.getValues("productDetailsOrigin.descriptionOrigin") || initialData?.productDetailsOrigin?.descriptionOrigin} fieldName="Product Description" /></FormLabel>
            ...
            <FormControl>
              <Textarea
                ...
                {...field}
                rows={4}
                onChange={(e) => { field.onChange(e); form.setValue("productDetailsOrigin.descriptionOrigin" as any, "manual"); }}
              />
            </FormControl>
            <FormMessage />
          </FormItem>
