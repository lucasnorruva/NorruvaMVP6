...
} from "../dppDisplayUtils";
import type {
  DigitalProductPassport,
  EbsiVerificationDetails,
} from "@/types/dpp"; // Ensure types are imported
import {
  ShieldCheck,
  ShieldAlert,
  ShieldQuestion,
...
describe("dppDisplayUtils", () => {
  ...
  describe("getStatusBadgeClasses", () => {
    it("returns green classes for compliant statuses", () => {
      expect(getStatusBadgeClasses("compliant")).toBe(
        "bg-green-100 text-green-700 border-green-300",
      );
    });

    it("returns red classes for non-compliant statuses", () => {
      expect(getStatusBadgeClasses("non-compliant")).toBe(
        "bg-red-100 text-red-700 border-red-300",
      );
    });

    it("returns yellow classes for pending statuses", () => {
      expect(getStatusBadgeClasses("pending_review")).toBe(
        "bg-yellow-100 text-yellow-700 border-yellow-300",
      );
    });

    it("returns muted classes for N/A or default statuses", () => {
      expect(getStatusBadgeClasses("n/a")).toBe(
  ...
  describe("getOverallComplianceDetails", () => {
    ...
    it('returns "Fully Compliant" correctly', () => {
      const dpp = {
        ...mockBaseDpp,
        compliance: {
          eprel: { status: "compliant", lastChecked: "" },
