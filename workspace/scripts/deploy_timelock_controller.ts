...
async function main() {
  ...
  console.log("  Initial Proposers:", proposers.length > 0 ? proposers.join(', ') : "None (to be set by Governor)");
  console.log("  Initial Executors:", executors.length > 0 ? executors.join(', ') : "None (to be set by Governor)");
  ...
  console.log("The deployer (current admin) should renounce the TIMELOCK_ADMIN_ROLE after the Governor is fully set up if the DAO is to be autonomous.");
}
