...
import {
  ...
  PieChart,
  Edit3,
} from "lucide-react"; // Added PieChart, Edit3
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
} from "@/types/dpp"; // Added DigitalProductPassport
import { USER_PRODUCTS_LOCAL_STORAGE_KEY } from "@/types/dpp";
import ProductManagementFiltersComponent, {
  type ProductManagementFilterState,
} from "@/components/products/ProductManagementFiltersComponent";
...
const initialMockProductsData: RichMockProduct[] = InitialMockDppsData.map(
  (dpp) => ({
    ...dpp,
    productId: dpp.id,
    ...
    sustainabilityClaims: dpp.productDetails?.sustainabilityClaims
      ?.map((c) => c.claim)
      .join(", "),
    energyLabel: dpp.productDetails?.energyLabel,
    specifications: dpp.productDetails?.specifications,
    lifecycleEvents: dpp.lifecycleEvents?.map((e) => ({
      id: e.id,
      eventName: e.type,
      date: e.timestamp,
      status: "Completed",
...
export default function ProductsPage() {
  ...
  const [productToDelete, setProductToDelete] =
    useState<ProductWithCompleteness | null>(null);
  ...
  useEffect(() => {
    ...
    const userAddedStoredProducts: StoredUserProduct[] = storedProductsString
      ? JSON.parse(storedProductsString)
      : [];

    const userAddedDisplayable: ProductWithCompleteness[] =
      userAddedStoredProducts.map((p) => ({
        ...p,
        productId: p.id,
        complianceSummary: p.complianceSummary || {
          overallStatus: "N/A",
          eprel: { status: "N/A", lastChecked: p.lastUpdated },
          ebsi: { status: "N/A", lastChecked: p.lastUpdated },
        },
    ...
    const initialDisplayable: ProductWithCompleteness[] =
      initialMockProductsData.map((mock) => ({
        ...mock,
        productId: mock.productId || mock.id,
        complianceSummary: mock.complianceSummary || {
          overallStatus: "N/A",
          eprel: { status: "N/A", lastChecked: mock.lastUpdated },
          ebsi: { status: "N/A", lastChecked: mock.lastUpdated },
        },
        lifecycleEvents: mock.lifecycleEvents || [],
        supplyChainLinks: mock.supplyChainLinks || [],
        completeness: calculateDppCompletenessForList(
          mock as DisplayableProduct,
        ), // Calculate completeness
        blockchainIdentifiers: mock.blockchainIdentifiers, // Include from mock
      }));

    const combined = [
      ...initialDisplayable.filter(
        (mock) =>
          !userAddedDisplayable.find((userProd) => userProd.id === mock.id),
      ),
  ...
  const filteredAndSortedProducts = useMemo(() => {
    ...
    if (filters.compliance !== "All") {
      tempProducts = tempProducts.filter(
        (p) => p.compliance === filters.compliance,
      );
    }
    if (filters.category !== "All") {
      tempProducts = tempProducts.filter(
        (p) => (p.category || p.productCategory) === filters.category,
      );
    }
    ...
    if (filters.blockchainAnchored && filters.blockchainAnchored !== "all") {
      if (filters.blockchainAnchored === "anchored") {
        tempProducts = tempProducts.filter(
          (p) => !!p.blockchainIdentifiers?.anchorTransactionHash,
        );
    ...
    const getValue = (
      product: ProductWithCompleteness,
      key: SortableProductKeys,
    ) => {
      if (key === "category") {
    ...
    if (sortConfig.key && sortConfig.direction) {
      tempProducts.sort((a, b) => {
        ...
        if (valA === undefined || valA === null)
          valA = typeof valA === "number" ? -Infinity : "";
        if (valB === undefined || valB === null)
          valB = typeof valB === "number" ? -Infinity : "";
  ...
  const handleDeleteProduct = () => {
    ...
    if (productToDelete.id.startsWith("USER_PROD")) {
      ...
      let userAddedProducts: StoredUserProduct[] = storedProductsString
        ? JSON.parse(storedProductsString)
        : [];
  ...
  const categoryOptions = useMemo(
    () => [
      ...
      ...new Set(
        allProducts
          .map((p) => p.category || p.productCategory)
          .filter(Boolean)
          .sort(),
      ),
    ],
  ...
  const summaryMetrics = useMemo(() => {
    ...
    const issues = allProducts.filter(
      (p) => p.compliance === "Non-Compliant" || p.compliance === "Pending",
    ).length;
    const totalCompletenessScore = allProducts.reduce(
      (sum, p) => sum + p.completeness.score,
      0,
    );
    const averageCompleteness =
      total > 0 ? (totalCompletenessScore / total).toFixed(1) + "%" : "0%";
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
        ...
        <MetricCard
          ...
          value={summaryMetrics.issues}
          trendDirection={summaryMetrics.issues > 0 ? "up" : "neutral"}
        />
        <MetricCard
          title="Avg. DPP Completeness"
          value={summaryMetrics.averageCompleteness}
          icon={PieChart}
        />
      </div>
      ...
      <Card className="shadow-lg">
        <CardHeader>
          ...
          <CardDescription>
            Manage and track all products in your system and their Digital
            Product Passports. Click headers to sort.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              product "{productToDelete?.productName || productToDelete?.id}".
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
