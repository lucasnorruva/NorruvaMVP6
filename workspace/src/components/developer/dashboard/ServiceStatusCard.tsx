...
export default function ServiceStatusCard({
  ...
  const getStatusBadgeVariant = (statusText: string): "default" | "destructive" | "outline" | "secondary" => {
    const lowerStatus = statusText.toLowerCase();
  ...
  const getStatusBadgeCustomClasses = (statusText: string): string => {
    const lowerStatus = statusText.toLowerCase();
    if (lowerStatus === "operational") return "bg-success/10 border-success/30 text-success";
    if (lowerStatus.includes("degraded")) return "bg-warning/10 border-warning/30 text-warning";
    if (lowerStatus.includes("maintenance")) return "bg-info/10 border-info/30 text-info";
    ...
    <Card className="shadow-lg">
      ...
      <CardContent className="space-y-3">
        <div className={cn("p-3 rounded-md flex items-center text-sm font-medium",
          overallSystemStatus.color === "text-success" && "bg-success/10 border border-success/30",
          overallSystemStatus.color === "text-warning" && "bg-warning/10 border border-warning/30",
          overallSystemStatus.color === "text-danger" && "bg-destructive/10 border border-destructive/30"
        )}>
        ...
        {systemStatusData.map((service) => {
          ...
          const badgeCustomClasses = getStatusBadgeCustomClasses(service.status);
          ...
          return (
            <div key={service.name} className="flex items-center justify-between p-2.5 border-b last:border-b-0 rounded-md hover:bg-muted/30">
              <div className="flex items-center">
                <StatusIcon className={cn("h-5 w-5 mr-3", serviceStatusColor)} />
