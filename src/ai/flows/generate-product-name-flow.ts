...
const GenerateProductNameInputSchema = z.object({
  ...
  productCategory: z.string().describe('The category of the product (e.g., Electronics, Apparel).').optional(),
});
...
const GenerateProductNameOutputSchema = z.object({
  productName: z.string().describe('A concise and marketable product name suggested by the AI.'),
});
...
const generateProductNameFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    if (!output?.productName) {
      ...
      throw new Error("AI failed to generate a product name in the expected format.");
    }
    return output;
  }
);
