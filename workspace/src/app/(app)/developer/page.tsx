...
export default function DeveloperPortalPage() {
  ...
  const endpointConfigs = [
    { id: 'get-product', section: 'core', title: 'GET /dpp/{productId}', description: 'Retrieve details for a specific product by its ID.', onSendRequest: handleMockGetProductDetails, isLoading: isGetProductLoading, response: getProductResponse, codeSnippet: getProductCodeSnippet, snippetLanguage: getProductSnippetLang, onSnippetLanguageChange: (lang: string) => { setGetProductSnippetLang(lang); updateSnippet('getProduct','GET',lang,{ productId: getProductId },null,setGetProductCodeSnippet); }, children: ( <div> <Label htmlFor="productIdInput-get">Product ID (Path Parameter)</Label> <Input id="productIdInput-get" value={getProductId} onChange={(e) => setGetProductId(e.target.value)} placeholder="e.g., DPP001" /> </div> ) },
    { id: 'list-dpps', section: 'core', title: 'GET /dpp', description: 'Retrieve a list of available Digital Product Passports with filtering.', onSendRequest: handleMockListProducts, isLoading: isListProductsLoading, response: listProductsResponse, codeSnippet: listDppsCodeSnippet, snippetLanguage: listDppsSnippetLang, onSnippetLanguageChange: (lang: string) => { setListDppsSnippetLang(lang); updateSnippet('listDpps','GET',lang,listDppFilters,null,setListDppsCodeSnippet); }, children: ( <div className="grid grid-cols-1 md:grid-cols-2 gap-4"> <div> <Label htmlFor="list-dpp-search">Search Query (ID, Name, GTIN, Manufacturer)</Label> <Input id="list-dpp-search" value={listDppFilters.searchQuery} onChange={(e) => setListDppFilters(prev => ({...prev, searchQuery: e.target.value}))} placeholder="e.g., EcoSmart, DPP001" /> </div> <div> <Label htmlFor="list-dpp-category">Category</Label> <Input id="list-dpp-category" value={listDppFilters.category === 'all' ? '' : listDppFilters.category} onChange={(e) => setListDppFilters(prev => ({...prev, category: e.target.value || 'all'}))} placeholder="e.g., Appliances (or 'all')" /> </div> <div> <Label htmlFor="list-dpp-status">Status</Label> <Select value={listDppFilters.status} onValueChange={(value) => setListDppFilters(prev => ({...prev, status: value}))}> <SelectTrigger id="list-dpp-status"><SelectValue /></SelectTrigger> <SelectContent> {['all','draft','published','archived','pending_review','revoked','flagged'].map(s => <SelectItem key={`listStatus-${s}`} value={s}>{s.charAt(0).toUpperCase() + s.slice(1).replace('_',' ')}</SelectItem>)} </SelectContent> </Select> </div> <div> <Label htmlFor="list-dpp-anchored">Blockchain Anchored</Label> <Select value={listDppFilters.blockchainAnchored} onValueChange={(value) => setListDppFilters(prev => ({...prev, blockchainAnchored: value}))}> <SelectTrigger id="list-dpp-anchored"><SelectValue /></SelectTrigger> <SelectContent> <SelectItem value="all">All</SelectItem> <SelectItem value="anchored">Anchored</SelectItem> <SelectItem value="not_anchored">Not Anchored</SelectItem> </SelectContent> </Select> </div> </div> ) },
    { id: 'create-dpp', section: 'core', title: 'POST /dpp', description: 'Create a new Digital Product Passport.', onSendRequest: handleMockPostDpp, isLoading: isPostDppLoading, response: postDppResponse, codeSnippet: createDppCodeSnippet, snippetLanguage: createDppSnippetLang, onSnippetLanguageChange: (lang: string) => { setCreateDppSnippetLang(lang); updateSnippet('createDpp','POST',lang,{},postDppBody,setCreateDppCodeSnippet); }, children: ( <div> <Label htmlFor="postDppBody">Request Body (JSON)</Label> <Textarea id="postDppBody" value={postDppBody} onChange={(e) => setPostDppBody(e.target.value)} rows={5} className="font-mono text-xs" /> <p className="text-xs text-muted-foreground mt-1">Example required fields: productName, category. See API Reference for full schema.</p> </div> ) },
    { id: 'update-dpp', section: 'core', title: 'PUT /dpp/{productId}', description: 'Update an existing Digital Product Passport.', onSendRequest: handleMockPutProduct, isLoading: isPutProductLoading, response: putProductResponse, codeSnippet: updateDppCodeSnippet, snippetLanguage: updateDppSnippetLang, onSnippetLanguageChange: (lang: string) => { setUpdateDppSnippetLang(lang); updateSnippet('updateDpp','PUT',lang,{productId: putProductId},putProductBody,setUpdateDppCodeSnippet); }, children: ( <> <div> <Label htmlFor="productIdInput-put">Product ID (Path Parameter)</Label> <Input id="productIdInput-put" value={putProductId} onChange={(e) => setPutProductId(e.target.value)} placeholder="e.g., DPP001" /> </div> <div className="mt-2"> <Label htmlFor="putProductBody">Request Body (JSON)</Label> <Textarea id="putProductBody" value={putProductBody} onChange={(e) => setPutProductBody(e.target.value)} rows={4} className="font-mono text-xs" /> <p className="text-xs text-muted-foreground mt-1">Send partial or full updates. See API Reference for updatable fields.</p> </div> </> ) },
    { id: 'patch-dpp-extend', section: 'core', title: 'PATCH /dpp/extend/{productId}', description: 'Extend a DPP by adding document references or other modular data.', onSendRequest: handleMockPatchDppExtend, isLoading: isPatchDppExtendLoading, response: patchDppExtendResponse, codeSnippet: patchDppExtendCodeSnippet, snippetLanguage: patchDppExtendSnippetLang, onSnippetLanguageChange: (lang: string) => { setPatchDppExtendSnippetLang(lang); updateSnippet('patchDppExtend','PATCH',lang,{productId: patchDppExtendProductId},patchDppExtendBody,setPatchDppExtendCodeSnippet); }, children: ( <> <div> <Label htmlFor="productIdInput-patch">Product ID (Path Parameter)</Label> <Input id="productIdInput-patch" value={patchDppExtendProductId} onChange={(e) => setPatchDppExtendProductId(e.target.value)} placeholder="e.g., DPP001" /> </div> <div className="mt-2"> <Label htmlFor="patchDppExtendBody">Request Body (JSON)</Label> <Textarea id="patchDppExtendBody" value={patchDppExtendBody} onChange={(e) => setPatchDppExtendBody(e.target.value)} rows={4} className="font-mono text-xs" /> <p className="text-xs text-muted-foreground mt-1">Example: Add a document reference. See API Reference.</p> </div> </> ) },
    { id: 'delete-dpp', section: 'core', title: 'DELETE /dpp/{productId}', description: 'Archive a Digital Product Passport.', onSendRequest: handleMockDeleteProduct, isLoading: isDeleteProductLoading, response: deleteProductResponse, codeSnippet: deleteDppCodeSnippet, snippetLanguage: deleteDppSnippetLang, onSnippetLanguageChange: (lang: string) => { setDeleteDppSnippetLang(lang); updateSnippet('deleteDpp','DELETE',lang,{productId: deleteProductId},null,setDeleteDppCodeSnippet); }, children: ( <div> <Label htmlFor="productIdInput-delete">Product ID (Path Parameter)</Label> <Input id="productIdInput-delete" value={deleteProductId} onChange={(e) => setDeleteProductId(e.target.value)} placeholder="e.g., PROD002" /> </div> ) },
    { id: 'get-status', section: 'utility', title: 'GET /dpp/status/{productId}', description: 'Retrieve the current status of a specific DPP.', onSendRequest: handleMockGetStatus, isLoading: isGetStatusLoading, response: getStatusResponse, codeSnippet: getStatusCodeSnippet, snippetLanguage: getStatusSnippetLang, onSnippetLanguageChange: (lang: string) => { setGetStatusSnippetLang(lang); updateSnippet('getDppStatus','GET',lang,{ productId: getStatusProductId },null,setGetStatusCodeSnippet); }, children: ( <div> <Label htmlFor="productIdInput-get-status">Product ID (Path Parameter)</Label> <Input id="productIdInput-get-status" value={getStatusProductId} onChange={(e) => setGetStatusProductId(e.target.value)} placeholder="e.g., DPP001" /> </div> ) },
    { id: 'qr-validate', section: 'utility', title: 'POST /qr/validate', description: 'Validate a QR identifier and retrieve a DPP summary.', onSendRequest: handleMockPostQrValidate, isLoading: isPostQrValidateLoading, response: postQrValidateResponse, codeSnippet: qrValidateCodeSnippet, snippetLanguage: qrValidateSnippetLang, onSnippetLanguageChange: (lang: string) => { setQrValidateSnippetLang(lang); updateSnippet('qrValidate','POST',lang,{},postQrValidateBody,setQrValidateCodeSnippet); }, children: ( <div> <Label htmlFor="postQrValidateBody">Request Body (JSON)</Label> <Textarea id="postQrValidateBody" value={postQrValidateBody} onChange={(e) => setPostQrValidateBody(e.target.value)} rows={3} className="font-mono text-xs" /> </div> ) },
    { id: 'post-lifecycle-event', section: 'utility', title: 'POST /dpp/{productId}/lifecycle-events', description: 'Add a new lifecycle event to a specific DPP.', onSendRequest: handleMockPostLifecycleEvent, isLoading: isPostLifecycleEventLoading, response: postLifecycleEventResponse, codeSnippet: addLifecycleEventCodeSnippet, snippetLanguage: addLifecycleEventSnippetLang, onSnippetLanguageChange: (lang: string) => { setAddLifecycleEventSnippetLang(lang); updateSnippet('addLifecycleEvent','POST',lang,{productId: postLifecycleEventProductId},postLifecycleEventBody,setAddLifecycleEventCodeSnippet); }, children: ( <> <div> <Label htmlFor="productIdInput-post-event">Product ID (Path Parameter)</Label> <Input id="productIdInput-post-event" value={postLifecycleEventProductId} onChange={(e) => setPostLifecycleEventProductId(e.target.value)} placeholder="e.g., DPP001" /> </div> <div className="mt-2"> <Label htmlFor="postLifecycleEventBody">Request Body (JSON)</Label> <Textarea id="postLifecycleEventBody" value={postLifecycleEventBody} onChange={(e) => setPostLifecycleEventBody(e.target.value)} rows={5} className="font-mono text-xs" /> </div> </> ) },
    { id: 'get-compliance-summary', section: 'utility', title: 'GET /dpp/{productId}/compliance-summary', description: 'Retrieve a compliance summary for a specific product.', onSendRequest: handleMockGetComplianceSummary, isLoading: isGetComplianceLoading, response: getComplianceResponse, codeSnippet: getComplianceSummaryCodeSnippet, snippetLanguage: getComplianceSummarySnippetLang, onSnippetLanguageChange: (lang: string) => { setGetComplianceSummarySnippetLang(lang); updateSnippet('getComplianceSummary','GET',lang,{productId: getComplianceProductId},null,setGetComplianceSummaryCodeSnippet); }, children: ( <div> <Label htmlFor="productIdInput-get-compliance">Product ID (Path Parameter)</Label> <Input id="productIdInput-get-compliance" value={getComplianceProductId} onChange={(e) => setGetComplianceProductId(e.target.value)} placeholder="e.g., DPP001" /> </div> ) },
    { id: 'verify-dpp', section: 'utility', title: 'POST /dpp/verify/{productId}', description: 'Perform compliance and authenticity checks on a DPP.', onSendRequest: handleMockPostVerify, isLoading: isPostVerifyLoading, response: postVerifyResponse, codeSnippet: verifyDppCodeSnippet, snippetLanguage: verifyDppSnippetLang, onSnippetLanguageChange: (lang: string) => { setVerifyDppSnippetLang(lang); updateSnippet('verifyDpp','POST',lang,{productIdPath: postVerifyProductIdPath},null,setVerifyDppCodeSnippet); }, children: ( <div> <Label htmlFor="productIdInput-verify-path">Product ID (Path Parameter)</Label> <Input id="productIdInput-verify-path" value={postVerifyProductIdPath} onChange={(e) => setPostVerifyProductIdPath(e.target.value)} placeholder="e.g., DPP001" /> <p className="text-xs text-muted-foreground mt-1">No request body needed for this mock endpoint.</p> </div> ) },
    { id: 'get-history', section: 'utility', title: 'GET /dpp/history/{productId}', description: 'Retrieve the audit trail / history for a specific DPP.', onSendRequest: handleMockGetHistory, isLoading: isGetHistoryLoading, response: getHistoryResponse, codeSnippet: getDppHistoryCodeSnippet, snippetLanguage: getDppHistorySnippetLang, onSnippetLanguageChange: (lang: string) => { setGetDppHistorySnippetLang(lang); updateSnippet('getDppHistory','GET',lang,{productId: getHistoryProductId},null,setGetDppHistoryCodeSnippet); }, children: ( <div> <Label htmlFor="historyProductIdInput">Product ID (Path Parameter)</Label> <Input id="historyProductIdInput" value={getHistoryProductId} onChange={(e) => setGetHistoryProductId(e.target.value)} placeholder="e.g., DPP001" /> </div> ) },
    { id: 'import-dpps', section: 'utility', title: 'POST /dpp/import', description: 'Batch import Digital Product Passports (CSV, JSON, etc.).', onSendRequest: handleMockPostImport, isLoading: isPostImportLoading, response: postImportResponse, codeSnippet: importDppsCodeSnippet, snippetLanguage: importDppsSnippetLang, onSnippetLanguageChange: (lang: string) => { setImportDppsSnippetLang(lang); updateSnippet('importDpps','POST',lang,{fileType: postImportFileType, sourceDescription: postImportSourceDescription}, JSON.stringify({fileType: postImportFileType, data: 'mock_base64_data', sourceDescription: postImportSourceDescription}), setImportDppsCodeSnippet); }, children: ( <> <div className="grid grid-cols-1 md:grid-cols-2 gap-4"> <div> <Label htmlFor="importFileType">File Type</Label> <Select value={postImportFileType} onValueChange={setPostImportFileType}> <SelectTrigger id="importFileType"> <SelectValue placeholder="Select file type" /> </SelectTrigger> <SelectContent> <SelectItem value="csv">CSV</SelectItem> <SelectItem value="json">JSON</SelectItem> <SelectItem value="api">API (Simulate direct data feed)</SelectItem> </SelectContent> </Select> </div> <div> <Label htmlFor="importSourceDescription">Source Description (Optional)</Label> <Input id="importSourceDescription" value={postImportSourceDescription} onChange={(e) => setPostImportSourceDescription(e.target.value)} placeholder="e.g., Q3 Supplier Data Upload" /> </div> </div> <p className="text-xs text-muted-foreground mt-1">For this mock, 'data' field is simulated. See API Reference.</p> </>) },
    { id: 'get-graph', section: 'utility', title: 'GET /dpp/graph/{productId}', description: 'Retrieve data for supply chain visualization.', onSendRequest: handleMockGetGraph, isLoading: isGetGraphLoading, response: getGraphResponse, codeSnippet: getDppGraphCodeSnippet, snippetLanguage: getDppGraphSnippetLang, onSnippetLanguageChange: (lang: string) => { setGetDppGraphSnippetLang(lang); updateSnippet('getDppGraph','GET',lang,{productId: getGraphProductId},null,setGetDppGraphCodeSnippet); }, children: ( <div> <Label htmlFor="graphProductIdInput">Product ID (Path Parameter)</Label> <Input id="graphProductIdInput" value={getGraphProductId} onChange={(e) => setGetGraphProductId(e.target.value)} placeholder="e.g., DPP001" /> </div> ) },
    { id: 'update-onchain-status', section: 'onchain', title: 'POST /dpp/{productId}/onchain-status', description: 'Conceptually update the on-chain status of a DPP.', onSendRequest: handleMockUpdateOnChainStatus, isLoading: isPostOnchainStatusLoading, response: postOnchainStatusResponse, codeSnippet: postOnchainStatusCodeSnippet, snippetLanguage: postOnchainStatusSnippetLang, onSnippetLanguageChange: (lang: string) => { setPostOnchainStatusSnippetLang(lang); updateSnippet('onchainStatus','POST',lang,{productId: postOnchainStatusProductId},postOnchainStatusBody,setPostOnchainStatusCodeSnippet); }, children: ( <> <div> <Label htmlFor="onchainStatusProductIdPlayground">Product ID (Path Parameter)</Label> <Input id="onchainStatusProductIdPlayground" value={postOnchainStatusProductId} onChange={(e) => setPostOnchainStatusProductId(e.target.value)} placeholder="e.g., DPP001" /> </div> <div className="mt-2"> <Label htmlFor="onchainStatusBodyPlayground">Request Body (JSON)</Label> <Textarea id="onchainStatusBodyPlayground" value={postOnchainStatusBody} onChange={(e) => setPostOnchainStatusBody(e.target.value)} rows={3} className="font-mono text-xs" /> </div> </> ) },
    { id: 'update-onchain-lifecycle-stage', section: 'onchain', title: 'POST /dpp/{productId}/onchain-lifecycle-stage', description: 'Conceptually update the on-chain lifecycle stage of a DPP.', onSendRequest: handleMockUpdateOnChainLifecycleStage, isLoading: isPostOnchainLifecycleLoading, response: postOnchainLifecycleResponse, codeSnippet: postOnchainLifecycleCodeSnippet, snippetLanguage: postOnchainLifecycleSnippetLang, onSnippetLanguageChange: (lang: string) => { setPostOnchainLifecycleSnippetLang(lang); updateSnippet('onchainLifecycleStage','POST',lang,{productId: postOnchainLifecycleProductId},postOnchainLifecycleBody,setPostOnchainLifecycleCodeSnippet); }, children: ( <> <div> <Label htmlFor="onchainLifecycleProductIdPlayground">Product ID (Path Parameter)</Label> <Input id="onchainLifecycleProductIdPlayground" value={postOnchainLifecycleProductId} onChange={(e) => setPostOnchainLifecycleProductId(e.target.value)} placeholder="e.g., DPP001" /> </div> <div className="mt-2"> <Label htmlFor="onchainLifecycleBodyPlayground">Request Body (JSON)</Label> <Textarea id="onchainLifecycleBodyPlayground" value={postOnchainLifecycleBody} onChange={(e) => setPostOnchainLifecycleBody(e.target.value)} rows={3} className="font-mono text-xs" /> </div> </> ) },
    { id: 'log-critical-event', section: 'onchain', title: 'POST /dpp/{productId}/log-critical-event', description: 'Conceptually log a critical event on-chain for a DPP.', onSendRequest: handleMockLogCriticalEvent, isLoading: isPostLogCriticalEventLoading, response: postLogCriticalEventResponse, codeSnippet: postLogCriticalEventCodeSnippet, snippetLanguage: postLogCriticalEventSnippetLang, onSnippetLanguageChange: (lang: string) => { setPostLogCriticalEventSnippetLang(lang); updateSnippet('logCriticalEvent','POST',lang,{productId: postLogCriticalEventProductId},postLogCriticalEventBody,setPostLogCriticalEventCodeSnippet); }, children: ( <> <div> <Label htmlFor="logCriticalEventProductIdPlayground">Product ID (Path Parameter)</Label> <Input id="logCriticalEventProductIdPlayground" value={postLogCriticalEventProductId} onChange={(e) => setPostLogCriticalEventProductId(e.target.value)} placeholder="e.g., DPP001" /> </div> <div className="mt-2"> <Label htmlFor="logCriticalEventBodyPlayground">Request Body (JSON)</Label> <Textarea id="logCriticalEventBodyPlayground" value={postLogCriticalEventBody} onChange={(e) => setPostLogCriticalEventBody(e.target.value)} rows={3} className="font-mono text-xs" /> </div> </> ) },
    { id: 'register-vc-hash', section: 'onchain', title: 'POST /dpp/{productId}/register-vc-hash', description: "Conceptually register a VC's hash on-chain for a DPP.", onSendRequest: handleMockRegisterVcHash, isLoading: isPostRegisterVcHashLoading, response: postRegisterVcHashResponse, codeSnippet: postRegisterVcHashCodeSnippet, snippetLanguage: postRegisterVcHashSnippetLang, onSnippetLanguageChange: (lang: string) => { setPostRegisterVcHashSnippetLang(lang); updateSnippet('registerVcHash','POST',lang,{productId: postRegisterVcHashProductId},postRegisterVcHashBody,setPostRegisterVcHashCodeSnippet); }, children: ( <> <div> <Label htmlFor="registerVcHashProductIdPlayground">Product ID (Path Parameter)</Label> <Input id="registerVcHashProductIdPlayground" value={postRegisterVcHashProductId} onChange={(e) => setPostRegisterVcHashProductId(e.target.value)} placeholder="e.g., DPP001" /> </div> <div className="mt-2"> <Label htmlFor="registerVcHashBodyPlayground">Request Body (JSON)</Label> <Textarea id="registerVcHashBodyPlayground" value={postRegisterVcHashBody} onChange={(e) => setPostRegisterVcHashBody(e.target.value)} rows={3} className="font-mono text-xs" /> </div> </> ) },
    { id: 'post-component-transfer', section: 'private', title: 'POST /private/dpp/{productId}/component-transfer', description: '[Private Layer] Record a private B2B component transfer.', onSendRequest: handleMockPostComponentTransfer, isLoading: isPostComponentTransferLoading, response: postComponentTransferResponse, codeSnippet: postComponentTransferCodeSnippet, snippetLanguage: postComponentTransferSnippetLang, onSnippetLanguageChange: (lang: string) => { setPostComponentTransferSnippetLang(lang); updateSnippet('postComponentTransfer','POST',lang,{productId: postComponentTransferProductId},postComponentTransferBody,setPostComponentTransferCodeSnippet); }, children: ( <> <div> <Label htmlFor="componentTransferProductIdPlayground">Product ID (Path Parameter)</Label> <Input id="componentTransferProductIdPlayground" value={postComponentTransferProductId} onChange={(e) => setPostComponentTransferProductId(e.target.value)} placeholder="e.g., DPP001" /> </div> <div className="mt-2"> <Label htmlFor="componentTransferBodyPlayground">Request Body (JSON)</Label> <Textarea id="componentTransferBodyPlayground" value={postComponentTransferBody} onChange={(e) => setPostComponentTransferBody(e.target.value)} rows={8} className="font-mono text-xs" /> </div> </> ) },
    { id: 'zkp-submit-proof', section: 'zkp', title: 'POST /zkp/submit-proof/{dppId}', description: '[ZKP Layer] Submit a Zero-Knowledge Proof for a DPP (Mock).', onSendRequest: handleMockZkpSubmitProof, isLoading: isZkpSubmitLoading, response: zkpSubmitResponse, codeSnippet: zkpSubmitCodeSnippet, snippetLanguage: zkpSubmitSnippetLang, onSnippetLanguageChange: (lang: string) => { setZkpSubmitSnippetLang(lang); updateSnippet('zkpSubmitProof','POST',lang,{dppId: zkpSubmitDppId},zkpSubmitBody,setZkpSubmitCodeSnippet); }, children: ( <> <div> <Label htmlFor="zkpSubmitDppIdPlayground">DPP ID (Path Parameter)</Label> <Input id="zkpSubmitDppIdPlayground" value={zkpSubmitDppId} onChange={(e) => setZkpSubmitDppId(e.target.value)} placeholder="e.g., DPP001" /> </div> <div className="mt-2"> <Label htmlFor="zkpSubmitBodyPlayground">Request Body (JSON)</Label> <Textarea id="zkpSubmitBodyPlayground" value={zkpSubmitBody} onChange={(e) => setZkpSubmitBody(e.target.value)} rows={5} className="font-mono text-xs" /> <p className="text-xs text-muted-foreground mt-1">Body should include claimType, proofData, publicInputs. See API Reference.</p> </div> </> ) },
    { id: 'zkp-verify-claim', section: 'zkp', title: 'GET /zkp/verify-claim/{dppId}', description: '[ZKP Layer] Verify a ZKP claim for a DPP (Mock).', onSendRequest: handleMockZkpVerifyClaim, isLoading: isZkpVerifyLoading, response: zkpVerifyResponse, codeSnippet: zkpVerifyCodeSnippet, snippetLanguage: zkpVerifySnippetLang, onSnippetLanguageChange: (lang: string) => { setZkpVerifySnippetLang(lang); updateSnippet('zkpVerifyClaim','GET',lang,{dppId: zkpVerifyDppId, claimType: zkpVerifyClaimType},null,setZkpVerifyCodeSnippet); }, children: ( <> <div> <Label htmlFor="zkpVerifyDppIdPlayground">DPP ID (Path Parameter)</Label> <Input id="zkpVerifyDppIdPlayground" value={zkpVerifyDppId} onChange={(e) => setZkpVerifyDppId(e.target.value)} placeholder="e.g., DPP001" /> </div> <div className="mt-2"> <Label htmlFor="zkpVerifyClaimTypePlayground">Claim Type (Query Parameter)</Label> <Input id="zkpVerifyClaimTypePlayground" value={zkpVerifyClaimType} onChange={(e) => setZkpVerifyClaimType(e.target.value)} placeholder="e.g., material_compliance_svhc_lead_less_0.1" /> </div> </> ) },
    { id: 'mint-token', section: 'token', title: 'POST /token/mint/{productId}', description: 'Mints a blockchain token representing the specified DPP.', onSendRequest: handleMockMintToken, isLoading: isMintTokenLoading, response: mintTokenResponsePlayground, codeSnippet: mintTokenCodeSnippet, snippetLanguage: mintTokenSnippetLang, onSnippetLanguageChange: (lang: string) => { setMintTokenSnippetLang(lang); updateSnippet('mintToken', 'POST', lang, { productId: mintTokenProductId }, mintTokenBody, setMintTokenCodeSnippet); }, children: ( <> <div><Label htmlFor="mintTokenProductIdPlayground">Product ID (Path Parameter)</Label><Input id="mintTokenProductIdPlayground" value={mintTokenProductId} onChange={(e) => setMintTokenProductId(e.target.value)} placeholder="e.g., DPP001" /></div> <div className="mt-2"><Label htmlFor="mintTokenBodyPlayground">Request Body (JSON)</Label><Textarea id="mintTokenBodyPlayground" value={mintTokenBody} onChange={(e) => setMintTokenBody(e.target.value)} rows={4} className="font-mono text-xs" /></div> </> ) },
    { id: 'update-token-metadata', section: 'token', title: 'PATCH /token/metadata/{tokenId}', description: 'Updates the on-chain metadata URI for a minted DPP token.', onSendRequest: handleMockUpdateTokenMetadata, isLoading: isUpdateTokenMetaLoading, response: updateTokenMetaResponse, codeSnippet: updateTokenMetaCodeSnippet, snippetLanguage: updateTokenMetaSnippetLang, onSnippetLanguageChange: (lang: string) => { setUpdateTokenMetaSnippetLang(lang); updateSnippet('updateTokenMetadata', 'PATCH', lang, { tokenId: updateTokenMetaTokenId }, updateTokenMetaBody, setUpdateTokenMetaCodeSnippet); }, children: ( <> <div><Label htmlFor="updateTokenMetaTokenIdPlayground">Token ID (Path Parameter)</Label><Input id="updateTokenMetaTokenIdPlayground" value={updateTokenMetaTokenId} onChange={(e) => setUpdateTokenMetaTokenId(e.target.value)} placeholder="e.g., 101" /></div> <div className="mt-2"><Label htmlFor="updateTokenMetaBodyPlayground">Request Body (JSON)</Label><Textarea id="updateTokenMetaBodyPlayground" value={updateTokenMetaBody} onChange={(e) => setUpdateTokenMetaBody(e.target.value)} rows={3} className="font-mono text-xs" /></div> </> ) },
    { id: 'get-token-status', section: 'token', title: 'GET /token/status/{tokenId}', description: 'Retrieves on-chain information for a DPP token.', onSendRequest: handleMockGetTokenStatus, isLoading: isGetTokenStatusLoading, response: getTokenStatusResponse, codeSnippet: getTokenStatusCodeSnippet, snippetLanguage: getTokenStatusSnippetLang, onSnippetLanguageChange: (lang: string) => { setGetTokenStatusSnippetLang(lang); updateSnippet('getTokenStatus', 'GET', lang, { tokenId: getTokenStatusTokenId }, null, setGetTokenStatusCodeSnippet); }, children: ( <div><Label htmlFor="getTokenStatusTokenIdPlayground">Token ID (Path Parameter)</Label><Input id="getTokenStatusTokenIdPlayground" value={getTokenStatusTokenId} onChange={(e) => setGetTokenStatusTokenId(e.target.value)} placeholder="e.g., 101" /></div> ) }
  ] as const;
