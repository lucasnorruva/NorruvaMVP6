...
const MOCK_PRODUCTS_FOR_SELECTION: ProductForSelection[] = [
  {
    id: "PROD001",
    name: "EcoFriendly Refrigerator X2000",
    category: "Appliances",
    carbonFootprint: 180,
...
export default function CompareSustainabilityPage() {
  ...
  const handleGenerateComparison = () => {
    ...
    const productsToCompare = MOCK_PRODUCTS_FOR_SELECTION.filter((p) =>
    ...
    const results: ComparisonData[] = [
      {
        ...
        unit: "kg CO₂e",
        icon: BarChart3,
        values: productsToCompare.map((p) => ({
          productId: p.id,
          productName: p.name,
          value: p.carbonFootprint,
        })),
      },
      {
    ...
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <h1 className="text-3xl font-headline font-semibold">
          Compare Product Sustainability
        </h1>
      </div>
      ...
      <Card className="shadow-lg">
        <CardHeader>
          ...
          <CardDescription>
            Choose two or more products from the list below to compare their key
      ...
      {comparisonResults && (
        <Card className="shadow-lg">
          <CardHeader>
            ...
            <CardDescription>
              Side-by-side comparison of selected products' sustainability
              metrics.
            </CardDescription>
          </CardHeader>
          <CardContent>
                  ...
                  {comparisonResults.map((metricData) => (
                    <TableRow key={metricData.metric}>
                      ...
                      {metricData.values.map((productValue) => (
                        ...
                        >
                          {productValue.value.toLocaleString()}
                        </TableCell>
                      ))}
                    </TableRow>
                  ))}
      ...
      {selectedProductIds.length > 0 && !comparisonResults && (
        <Card className="shadow-lg border-dashed">
          <CardContent className="p-6 text-center text-muted-foreground">
            ...
            <p>
              Click "Generate Comparison" to see the results for the selected
              products.
            </p>
          </CardContent>
        </Card>
