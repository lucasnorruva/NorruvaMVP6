...
export default function ComplianceCopilot() {
  ...
  useEffect(() => {
    ...
    if (contextQuery) {
      ...
      toast({
        ...
        description:
          "Your question from the product page has been pre-filled. Press send to ask the Co-Pilot.",
      });
    }
  }, [searchParams, toast]);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const response = await queryComplianceCopilot({
      ...
      toast({
        ...
        title: "Error",
        description:
          "Failed to get a response from the AI Co-Pilot. Please try again.",
      });
  ...
  return (
    <Card className="w-full max-w-2xl mx-auto shadow-lg">
      <CardHeader>
        ...
        <CardDescription>
          Ask questions about EU Digital Product Passport regulations and
          related compliance topics.
        </CardDescription>
      </CardHeader>
      <CardContent className="flex flex-col h-[60vh]">
        <ScrollArea
          className="flex-grow p-4 border rounded-md mb-4 bg-muted/50"
          ref={scrollAreaRef}
        >
            ...
            {messages.length === 0 && !isLoading && (
              <div className="text-center text-muted-foreground py-8">
                ...
                <p className="text-xs mt-1">
                  E.g., "What is ESPR?" or "Tell me about battery passports."
                </p>
              </div>
            )}
          </div>
        ...
        <p className="text-xs text-muted-foreground mt-2 text-center">
          AI Co-Pilot is for informational purposes and does not constitute
          legal advice. Responses are based on general knowledge.
        </p>
      </CardContent>
    </Card>
  );
