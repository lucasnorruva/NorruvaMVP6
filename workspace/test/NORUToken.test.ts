...
describe("NORUToken", function () {
  ...
  describe("Deployment & Initialization", function () {
    ...
    it("Should grant DEFAULT_ADMIN_ROLE and MINTER_ROLE to the initialAdmin (owner)", async function () {
      expect(await noruToken.hasRole(DEFAULT_ADMIN_ROLE, owner.address)).to.be
        .true;
  ...
  describe("Minting (Controlled by MINTER_ROLE)", function () {
    ...
    it("Should allow an account with MINTER_ROLE to mint new tokens within cap", async function () {
      ...
      await expect(
        noruToken.connect(minterUser).mint(user1.address, mintAmount),
      )
        .to.emit(noruToken, "Transfer")
        .withArgs(ethers.ZeroAddress, user1.address, mintAmount);

      expect(await noruToken.balanceOf(user1.address)).to.equal(mintAmount);
      expect(await noruToken.totalSupply()).to.equal(
        initialTotalSupply + mintAmount,
      );
    });

    it("Should not allow minting beyond the cap", async function () {
      const almostCapAmount =
        TOKEN_CAP - (await noruToken.totalSupply()) - BigInt(1); // Mint just under cap
      if (almostCapAmount > 0) {
        await noruToken
          .connect(minterUser)
          .mint(user1.address, almostCapAmount);
      }
      ...
      await expect(
        noruToken
          .connect(minterUser)
          .mint(user1.address, ethers.parseUnits("1", 18)),
  ...
  describe("Burning (ERC20Burnable)", function () {
    ...
    it("Should allow a token holder to burn their own tokens", async function () {
      ...
      expect(await noruToken.balanceOf(owner.address)).to.equal(
        ownerBalance - burnAmount,
      );
      expect(await noruToken.totalSupply()).to.equal(
    ...
    it("Should allow burning tokens from another account with allowance (burnFrom)", async function () {
      ...
      expect(await noruToken.allowance(owner.address, user1.address)).to.equal(
        burnAmount,
      );
    ...
    it("Should not allow burning more tokens than an account has", async function () {
      ...
      await expect(noruToken.connect(owner).burn(tooLargeBurnAmount))
        .to.be.revertedWithCustomError(noruToken, "ERC20InsufficientBalance")
        .withArgs(owner.address, ownerBalance, tooLargeBurnAmount);
    });
    ...
    it("Should not allow burnFrom more than approved amount", async function () {
      ...
      await expect(
        ...
        .to.be.revertedWithCustomError(noruToken, "ERC20InsufficientAllowance")
        .withArgs(user1.address, approveAmount, burnFromAmount);
    });
  });
  ...
  describe("ERC-20 Functionality", function () {
    it("Should allow transfer of tokens between accounts", async function () {
      ...
      await expect(
        noruToken.connect(owner).transfer(user1.address, transferAmount),
      )
        .to.emit(noruToken, "Transfer")
    ...
    it("Should not allow transfer more than balance", async function () {
      ...
      await expect(
        ...
        .to.be.revertedWithCustomError(noruToken, "ERC20InsufficientBalance")
        .withArgs(owner.address, INITIAL_SUPPLY, tooLargeAmount);
    });
    ...
    it("Should allow approval and transferFrom", async function () {
      ...
      await expect(
        noruToken.connect(owner).approve(user1.address, approveAmount),
      )
    ...
    it("Should not allow transferFrom more than approved amount", async function () {
      ...
      await expect(
        ...
        .to.be.revertedWithCustomError(noruToken, "ERC20InsufficientAllowance")
        .withArgs(user1.address, approveAmount, transferAmount);
    });
  });
  ...
  describe("Role Management", function () {
    it("Only DEFAULT_ADMIN_ROLE should be able to grant roles", async function () {
      ...
      await expect(
        noruToken.connect(user1).grantRole(SOME_OTHER_ROLE, user2.address),
      )
        .to.be.revertedWithCustomError(
          noruToken,
          "AccessControlUnauthorizedAccount",
      ...
      await expect(
        noruToken.connect(owner).grantRole(SOME_OTHER_ROLE, user2.address),
      )
        .to.emit(noruToken, "RoleGranted")
        .withArgs(SOME_OTHER_ROLE, user2.address, owner.address);
      expect(await noruToken.hasRole(SOME_OTHER_ROLE, user2.address)).to.be
        .true;
    });
  });
  ...
  describe("Upgradeability (UUPS)", function () {
    it("Should be upgradeable by an admin to NORUTokenV2", async function () {
      ...
      expect(await upgradedToken.balanceOf(owner.address)).to.equal(
        INITIAL_SUPPLY,
      );
    ...
    it("Should not be upgradeable by a non-admin", async function () {
      ...
      await expect(
        noruToken
          .connect(user1)
          .upgradeToAndCall(await noruTokenV2Impl.getAddress(), "0x"),
      )
        .to.be.revertedWithCustomError(
          noruToken,
          "AccessControlUnauthorizedAccount",
