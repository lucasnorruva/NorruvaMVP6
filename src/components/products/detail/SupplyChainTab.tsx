...
export default function SupplyChainTab({
  product,
  onSupplyChainLinksChange,
}: SupplyChainTabProps) {
  const { toast } = useToast();
  const { currentRole } = useRole();
  const [isClient, setIsClient] = useState(false);

  const currentLinkedSuppliers = useMemo(
  ...
  const [editingLink, setEditingLink] = useState<ProductSupplyChainLink | null>(
    null,
  );
  ...
  const [attestations, setAttestations] = useState<
    Record<string, MockAttestation[]>
  >({});
  const [loadingAttestations, setLoadingAttestations] = useState<
    Record<string, boolean>
  >({});
  const [errorAttestations, setErrorAttestations] = useState<
    Record<string, string | null>
  >({});

  useEffect(() => {
    setIsClient(true);
    const storedSuppliersString = localStorage.getItem(
      USER_SUPPLIERS_LOCAL_STORAGE_KEY,
    );
    const userAddedSuppliers: Supplier[] = storedSuppliersString
      ? JSON.parse(storedSuppliersString)
      : [];
    const combinedSuppliers = [
      ...MOCK_SUPPLIERS.filter(
        (mockSup) =>
          !userAddedSuppliers.find((userSup) => userSup.id === mockSup.id),
      ),
      ...userAddedSuppliers,
  ...
  const canManageLinks = useMemo(() => {
    return (
      product.id.startsWith("USER_PROD") &&
      (currentRole === "admin" || currentRole === "manufacturer")
    );
  ...
  const handleLinkSupplier = () => {
    if (!canManageLinks) {
      toast({
        ...
        description: getDisabledTooltipText(),
        variant: "destructive",
      });
      return;
    }
    if (!selectedSupplierId || !suppliedItem.trim()) {
      toast({
        ...
        description: "Please select a supplier and enter the supplied item.",
        variant: "destructive",
      });
      return;
    }
    ...
    toast({
      ...
      description: "The new link has been added.",
    });
  ...
  const handleUnlinkSupplier = (
    ...
    const updatedLinks = currentLinkedSuppliers.filter(
      (link) =>
        !(
          link.supplierId === supplierIdToUnlink &&
          link.suppliedItem === itemSupplied
        ),
    );
    onSupplyChainLinksChange(updatedLinks);
    toast({
  ...
  const handleUpdateSupplierLink = () => {
    ...
    if (!editSuppliedItem.trim()) {
      toast({
        title: "Missing Information",
        description: "Supplied item cannot be empty.",
        variant: "destructive",
      });
      return;
    }
    const updatedLinks = currentLinkedSuppliers.map((link) =>
      link.supplierId === editingLink.supplierId &&
      link.suppliedItem === editingLink.suppliedItem
        ? {
            ...link,
            suppliedItem: editSuppliedItem.trim(),
            notes: editLinkNotes.trim(),
          }
        : link,
    );
    onSupplyChainLinksChange(updatedLinks);
    toast({
      title: "Supplier Link Updated",
      description: "The link details have been updated.",
    });
  ...
  const handleFetchAttestations = async (supplierId: string) => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/v1/private/dpp/${product.id}/supplier/${supplierId}/attestations`,
        {
          headers: {
      ...
      toast({
        title: "Attestations Loaded (Mock)",
        description: `Simulated private attestations for supplier ${supplierId} loaded.`,
      });
    } catch (err) {
      const message =
        err instanceof Error
          ? err.message
          : "Unknown error fetching attestations.";
      setErrorAttestations((prev) => ({ ...prev, [supplierId]: message }));
      toast({
        title: "Error Loading Attestations",
        description: message,
        variant: "destructive",
      });
    } finally {
  ...
  if (!isClient) {
    return (
      <p className="text-muted-foreground p-4">
        Loading supply chain information...
      </p>
    );
  }
      ...
      <CardHeader className="flex flex-row items-center justify-between">
        <div>
          <CardTitle className="text-lg font-semibold flex items-center">
            <Layers className="mr-2 h-5 w-5 text-primary" /> Product Supply
            Chain
          </CardTitle>
          <CardDescription>
            Overview of suppliers and components linked to this product.
            Attestations are mock private data.
          </CardDescription>
        </div>
        ...
        <Dialog open={isLinkDialogOpen} onOpenChange={setIsLinkDialogOpen}>
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <div
                  ...
                  tabIndex={!canManageLinks ? 0 : undefined}
                >
          ...
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Select a supplier and specify the item they provide for this
                product.
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-4 py-4">
              {availableSuppliers.length === 0 ? (
                <p className="text-sm text-muted-foreground">
                  No suppliers have been added to the system yet. You can add
                  suppliers from the main{" "}
                  <Link
                    href="/suppliers"
                    ...
                    <SelectContent>
                      {availableSuppliers.map((s) => (
                        <SelectItem key={s.id} value={s.id}>
                          {s.name} ({s.materialsSupplied.substring(0, 30)}...)
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
              ...
              <div>
                ...
                <Input
                  id="supplied-item"
                  value={suppliedItem}
                  onChange={(e) => setSuppliedItem(e.target.value)}
                  placeholder="e.g., Battery Cells, Organic Cotton"
                />
              </div>
              <div>
                ...
                <Textarea
                  ...
                  placeholder="e.g., Primary source for EU market"
                />
              </div>
            </div>
            <DialogFooter>
      ...
      <CardContent>
        {currentLinkedSuppliers.length > 0 ? (
          <div className="space-y-3">
            {currentLinkedSuppliers.map((link, index) => {
              const supplier = availableSuppliers.find(
                (s) => s.id === link.supplierId,
              );
              const supplierAttestations = attestations[link.supplierId] || [];
              const isLoadingSupplierAttestations =
                loadingAttestations[link.supplierId];
              const errorSupplierAttestations =
                errorAttestations[link.supplierId];

              return (
                <Card
                  key={`${link.supplierId}-${link.suppliedItem}-${index}`}
                ...
                >
                  ...
                  <CardContent className="p-3 pt-0 text-xs">
                    ...
                    >
                      {isLoadingSupplierAttestations ? (
                        <Loader2 className="h-3 w-3 mr-1.5 animate-spin" />
                      ) : (
                        <VenetianMask className="h-3 w-3 mr-1.5" />
                      )}
                      {isLoadingSupplierAttestations
                        ? "Loading..."
                        : "View Private Attestations (Mock)"}
                    </Button>
                    ...
                    {supplierAttestations.length > 0 &&
                      !errorSupplierAttestations && (
                        ...
                        >
                          <AccordionItem
                            value={`item-${link.supplierId}`}
                            className="border-t"
                          >
                            <AccordionTrigger className="text-xs py-2 hover:no-underline text-primary">
                              Show/Hide {supplierAttestations.length} Private
                              Attestation(s)
                            </AccordionTrigger>
                                ...
                                {supplierAttestations.map((att) => (
                                  ...
                                  >
                                    <p className="font-medium text-foreground/90">
                                      {att.attestationType} ({att.componentId})
                                    </p>
                                    ...
                                    <p className="text-muted-foreground text-[0.7rem]">
                                      ...
                                      {att.expiryDate
                                        ? `- Expires: ${new Date(att.expiryDate).toLocaleDateString()}`
                                        : ""}
                                    </p>
                                    {att.evidence &&
                                      att.evidence.map((ev, idx) => (
                                        <p
                                          key={idx}
                                          className="text-muted-foreground text-[0.7rem] pl-2"
                                        >
                                          &bull; Evidence: {ev.type}{" "}
                                          {ev.documentId
                                            ? `(${ev.documentId})`
                                            : ""}{" "}
                                          {ev.vcId
                                            ? `(VC: ${ev.vcId.substring(0, 15)}...)`
                                            : ""}
                                        </p>
                                      ))}
                                  </div>
          ...
          <div className="text-center text-muted-foreground py-6">
            ...
            {!canManageLinks && product.id.startsWith("USER_PROD") && (
              <p className="text-sm mt-1">
                Your role does not permit linking suppliers.
              </p>
            )}
            {!canManageLinks && !product.id.startsWith("USER_PROD") && (
              <p className="text-sm mt-1">
                Linking suppliers is only enabled for user-created products.
              </p>
            )}
            ...
            {canManageLinks && availableSuppliers.length > 0 && (
              <p className="text-sm mt-1">
                Click "Link New Supplier" to add one.
              </p>
            )}
          </div>
        )}
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Modify the details for this supplier link.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <div>
              <Label htmlFor="edit-supplied-item">
                Supplied Item/Component
              </Label>
              <Input
                id="edit-supplied-item"
                value={editSuppliedItem}
                onChange={(e) => setEditSuppliedItem(e.target.value)}
              />
            </div>
            <div>
