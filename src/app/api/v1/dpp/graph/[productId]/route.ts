...
export async function GET(
  ...
  if (product.lifecycleEvents && product.lifecycleEvents.length > 0) {
    product.lifecycleEvents.slice(0, 3).forEach(event => { // Limit to 3 events for simplicity
      const eventNodeId = `event_${event.id}`;
      nodes.push({
        id: eventNodeId,
        label: event.type,
        type: 'lifecycle_event',
        data: { timestamp: event.timestamp, location: event.location, responsibleParty: event.responsibleParty }
      });
      edges.push({
