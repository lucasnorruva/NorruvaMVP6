...
export default function ApiReferenceComplianceEndpoints({
  ...
  const complianceEndpoints: EndpointDefinition[] = [
    { 
      ...
      pathParams: [{ name: "productId", type: "string", required: true, description: "The unique identifier of the product." }], 
    ...
    { 
      ...
      description: "Performs compliance and authenticity checks on a specific DPP (conceptual EBSI-linked verification).", 
      pathParams: [{ name: "productId", type: "string", required: true, description: "The unique identifier of the product to verify." }], 
  ...
  return (
    <section id="compliance-endpoints">
      <h2 className="text-2xl font-semibold font-headline mt-8 mb-4 flex items-center">
        <ShieldCheck className="mr-3 h-6 w-6 text-primary" /> Compliance & Verification Endpoints
      </h2>
