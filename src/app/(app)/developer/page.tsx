...
import {
  ...
  History, // Renamed FileText to FileTextIconPg
  UploadCloud,
  ShieldCheck,
  Cpu,
  HardDrive,
...
const dashboardQuickActions = [
  ...
  { label: "Manage Webhooks", href: "#", targetTab: "webhooks", icon: Webhook },
  {
    ...
    targetTab: "dashboard",
    icon: ServerCrash,
    tooltip: "View API Status on Dashboard",
  },
];
...
export default function DeveloperPortalPage() {
  ...
  const handleGenerateSandboxKey = () => {
    ...
    toast({
      ...
      description: `New key ${newKey.key.substring(0, 12)}... created.`,
    });
  };
  ...
  const makeApiCall = async (
    ...
    setResponse(null);
    toast({
      title: "Sending Mock API Request...",
    ...
    try {
      ...
      if (!res.ok) {
        setResponse(
          JSON.stringify({ status: res.status, error: responseData }, null, 2),
        );
        toast({
          title: `Mock API Error: ${res.status}`,
          description:
            responseData.error?.message || "A mock API error occurred.",
          variant: "destructive",
        });
      } else {
        setResponse(JSON.stringify(responseData, null, 2));
        toast({
          title: "Mock API Response Received",
          description: `${method} request to ${url} was successful (simulated).`,
          variant: "default",
        });
      }
    } catch (e) {
      ...
      setResponse(
        JSON.stringify(
          {
            error: "Network or parsing error during mock call",
            details: errorMsg,
          },
          null,
          2,
      ...
      >
        ...
        <TabsContent value="playground" className="mt-6 space-y-6">
          ...
          >
            ...
            <AlertDescription>
              This playground interacts with a <strong>mock API backend</strong>
              . Responses are simulated based on predefined data (like
              MOCK_DPPS) and conceptual API logic. This is for testing and
              exploration in the{" "}
