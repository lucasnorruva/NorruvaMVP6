...
export async function GET(request: NextRequest) {
    ...
    return NextResponse.json(
      {
        error: {
          code: 404,
          message: "No products found matching criteria or specified IDs.",
        },
      },
      { status: 404 },
    );
  ...
  await new Promise((resolve) =>
    setTimeout(resolve, 200 + dppsToExport.length * 20),
  );

  if (format === "csv") {
    if (!Array.isArray(exportData) || exportData.length === 0) {
      return NextResponse.json(
        { error: { code: 400, message: "No data to export to CSV." } },
        { status: 400 },
      );
    }
    ...
    return NextResponse.json(
      {
        error: {
          code: 400,
          message: `Unsupported format: ${format}. Supported formats: json, csv, xml.`,
        },
      },
      { status: 400 },
    );
