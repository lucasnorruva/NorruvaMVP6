  ...
  return (
    <Card className="shadow-md">
      <CardHeader>
        ...
        <CardDescription>
          Access common developer tasks and resources directly.
        </CardDescription>
      </CardHeader>
      <CardContent className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
        {" "}
        {/* Adjusted grid cols for more items */}
        {actions.map((action, index) => {
          const ActionIcon = action.icon;
          let buttonVariant: "default" | "secondary" | "outline" = "outline";
          if (index === 0)
            buttonVariant = "default"; // First action as primary
          ...
          const actionButton = (
            ...
            >
              <ActionIcon className="mr-3 h-5 w-5 text-current flex-shrink-0" />{" "}
              {/* Use text-current for icon color from button */}
              <div>
