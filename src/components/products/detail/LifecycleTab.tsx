...
} from "@/ai/flows/suggest-maintenance-schedule"; // Import AI flow
import {
  Loader2,
  AlertTriangle as AlertTriangleIcon,
  Bot,
  Wrench,
} from "lucide-react"; // Renamed AlertTriangle to AlertTriangleIcon
import {
...
const getStatusBadgeVariant = (
  status: SimpleLifecycleEvent["status"],
...
export default function LifecycleTab({ product }: LifecycleTabProps) {
  ...
  const [isLoadingComplianceCheck, setIsLoadingComplianceCheck] = useState<
    string | null
  >(null);
  ...
  const handleAdvanceStage = async (
    targetEvent: SimpleLifecycleEvent,
    currentIndex: number,
  ) => {
    ...
    if (currentIndex > 0) {
      ...
      for (let i = currentIndex - 1; i >= 0; i--) {
        if (
          product.lifecycleEvents[i].status === "Completed" ||
          product.lifecycleEvents[i].status === "In Progress"
        ) {
      ...
      if (!foundPreviousStage) {
        currentLifecycleStageName =
          product.lifecycleEvents[currentIndex - 1]?.eventName ||
          "Pre-Production / Design";
      }
    } else if (
      currentIndex === 0 &&
      (targetEvent.status === "Upcoming" ||
        targetEvent.status === "In Progress")
    ) {
        ...
        description: (
          <div className="text-sm space-y-2">
            <p className="font-semibold">
              New Simulated Status:{" "}
              <span className="text-primary">
                {complianceResult.simulatedOverallStatus}
              </span>
            </p>
            <div>
              <p className="text-sm font-semibold mb-1">Simulated Report:</p>
              <p className="text-xs bg-muted p-2 rounded-md whitespace-pre-line max-h-40 overflow-y-auto">
                {complianceResult.simulatedReport}
              </p>
            </div>
            <p className="text-xs text-muted-foreground mt-2">
              (This is a mock advancement. In a real app, product status would
              update.)
            </p>
          </div>
        ),
  ...
  const handleGetMaintenanceSuggestions = async () => {
    ...
    const creationEvent = product.lifecycleEvents?.find(
      (e) =>
        e.eventName.toLowerCase().includes("manufactured") ||
        e.eventName.toLowerCase().includes("created"),
    );
    const soldEvent = product.lifecycleEvents?.find(
      (e) =>
        e.eventName.toLowerCase().includes("sold") ||
        e.eventName.toLowerCase().includes("purchase"),
    );
    ...
    if (referenceDateStr) {
      ...
      if (diffDays < 30) ageInfo = `${diffDays} days old`;
        ...
        ageInfo = `${Math.round(diffDays / 30)} months old`;
  ...
  return (
    <div className="space-y-6">
      <Card className="shadow-sm">
        <CardHeader>
          <CardTitle className="text-lg font-semibold flex items-center">
            <LucideIcons.History className="mr-2 h-5 w-5 text-primary" />{" "}
        ...
        <CardContent>
          {!product.lifecycleEvents || product.lifecycleEvents.length === 0 ? (
            <p className="text-muted-foreground">
              No lifecycle events recorded for this product.
            </p>
          ) : (
                    ...
                    >
                      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-1.5">
                        ...
                        <Badge
                          ...
                          className={cn(
                            "text-xs capitalize mt-1 sm:mt-0",
      ...
      <Card className="shadow-sm">
        <CardHeader>
          <CardTitle className="text-lg font-semibold flex items-center">
            <Bot className="mr-2 h-5 w-5 text-primary" /> AI Predictive
            Maintenance Advisor
          </CardTitle>
          <CardDescription>
            Get AI-powered suggestions for product maintenance.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <Button
            onClick={handleGetMaintenanceSuggestions}
            ...
            <div className="p-4 border rounded-lg bg-muted/30 space-y-3">
              <div>
                ...
                <p className="text-sm text-foreground">
                  {maintenanceSuggestion.nextCheckupDate}
                </p>
              </div>
              <div>
                <h5 className="font-semibold text-primary">
              ...
              <div>
                ...
                <p className="text-sm text-foreground italic">
                  {maintenanceSuggestion.reasoning}
                </p>
              </div>
            </div>
          )}
