  ...
  return (
    <Card className="shadow-lg">
      <CardHeader>
        ...
        <CardDescription>Distribution of Digital Product Passports across different categories.</CardDescription>
      </CardHeader>
        ...
        {categoryData.length > 0 ? (
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={categoryData} layout="vertical" margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
              ...
              <YAxis dataKey="name" type="category" stroke="hsl(var(--muted-foreground))" fontSize={10} width={100} interval={0} />
              <Tooltip
                cursor={{fill: 'hsl(var(--muted))'}}
                contentStyle={{backgroundColor: 'hsl(var(--popover))', borderRadius: 'var(--radius)', border: '1px solid hsl(var(--border))'}}
                labelStyle={{color: 'hsl(var(--popover-foreground))'}}
              />
              <Bar dataKey="value" name="DPP Count" radius={[0, 4, 4, 0]} barSize={18}>
          ...
          <p className="text-center text-muted-foreground pt-10">No category data available to display.</p>
        )}
      </CardContent>
    </Card>
  );
