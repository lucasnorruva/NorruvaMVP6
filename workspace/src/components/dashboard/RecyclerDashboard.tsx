  ...
  const handleMaterialSearch = () => {
    if (!materialSearchTerm.trim()) {
      toast({
        ...
        description: "Please enter a material to search for.",
        variant: "destructive",
      });
      return;
    }
  ...
  const handleViewDisassemblyGuide = () => {
    if (!disassemblyProductId.trim()) {
      toast({
        ...
        description: "Please enter a Product ID to view its disassembly guide.",
        variant: "destructive",
      });
      return;
    }
    const productExists = MOCK_DPPS.some(
      (p) => p.id === disassemblyProductId.trim(),
    );
    if (productExists) {
      toast({
    ...
    <div className="space-y-6">
      <Card className="shadow-lg">
        <CardHeader>
          <CardTitle className="font-headline flex items-center">
            <RecycleIcon className="mr-2 text-primary" />
            End-of-Life & Material Recovery
          </CardTitle>
          <CardDescription>
            Access DPP information for disassembly and material recovery.
          </CardDescription>
        </CardHeader>
        <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Card className="bg-muted/50">
            <CardHeader>
              <CardTitle className="text-lg">Products Processed</CardTitle>
            </CardHeader>
            <CardContent>
          ...
          <Card className="bg-muted/50">
            <CardHeader>
              <CardTitle className="text-lg">Material Recovery Rate</CardTitle>
            </CardHeader>
            <CardContent>
      ...
      <Card className="shadow-lg">
        <CardHeader>
          <CardTitle className="font-headline flex items-center">
            ...
            Common Recoverable Materials (Mock Volume)
          </CardTitle>
          <CardDescription>
            Estimated volume of materials recovered in the last reporting
            period.
          </CardDescription>
        </CardHeader>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <Card className="shadow-md">
          <CardHeader>
            <CardTitle className="font-headline flex items-center">
              ...
              Search Products by Material
            </CardTitle>
            <CardDescription>
              Find products containing specific materials for targeted
              recycling.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            <Label htmlFor="materialSearch" className="sr-only">
              Material to Search
            </Label>
        ...
        <Card className="shadow-md">
          <CardHeader>
            <CardTitle className="font-headline flex items-center">
              <FileTextIcon className="mr-2 text-primary" />
              View Disassembly Guide
            </CardTitle>
            <CardDescription>
              Access EOL instructions and disassembly guides for specific
              products.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            <Label htmlFor="disassemblyProductId" className="sr-only">
              Product ID for Disassembly Guide
            </Label>
            <div className="flex gap-2">
              <Input
      ...
      <Card>
        ...
        <CardContent>
          <p className="text-muted-foreground">
            High volume of 'Recycled PET' available from recent batch. Low stock
            of 'Lithium Carbonate' for battery recycling.
          </p>
        </CardContent>
      </Card>
