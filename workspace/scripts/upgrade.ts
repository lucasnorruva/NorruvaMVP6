...
async function main() {
  ...
  console.log(
    `Attempting to upgrade ${contractName} at proxy address ${proxyAddress} to ${V2ContractName}...`,
  );
  ...
  try {
    ...
    console.error(
      `Error getting contract factory for ${V2ContractName}:`,
      error,
    );
    throw new Error(
  ...
  console.log(
    `${contractName} (proxy at ${proxyAddress}) successfully upgraded to ${V2ContractName}.`,
  );
  ...
  console.log(
    `${V2ContractName} implementation deployed to: ${newImplementationAddress}`,
  );
  ...
  if (typeof (upgraded as any).version === "function") {
    try {
      ...
      console.log(
        `Could not call version() on ${V2ContractName}, or it does not exist.`,
      );
    }
  }
}
