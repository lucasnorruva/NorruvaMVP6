...
export default function GlobeV2Page() {
  ...
  const [selectedProductTransitInfo, setSelectedProductTransitInfo] =
  ...
  const getCountryFromLocationString = (
  ...
  useEffect(() => {
    ...
    if (transitInfo) {
      ...
      if (destinationCountry && destinationCountry !== originCountry)
        newHighlights.push(destinationCountry);
            ...
            if (manufacturerCoords) {
              supplyChainCountries.forEach((countryName) => {
                if (countryName !== manufacturerCountry) {
                  ...
                  if (supplierCoords) {
                    newSupplyArcs.push({
                      startLat: manufacturerCoords.lat,
                      startLng: manufacturerCoords.lng,
                      endLat: supplierCoords.lat,
                      endLng: supplierCoords.lng,
                      color: "#F59E0B",
          ...
          dimensions.height > 0 && (
            <GlobeComponent
              ...
              polygonLabel={({ properties }: object) => {
                ...
                if (isHighlighted) {
                  if (
                    selectedProductTransitInfo?.origin &&
                  ...
                  if (
                    selectedProductTransitInfo?.destination &&
                    getCountryFromLocationString(
                      selectedProductTransitInfo.destination,
                    ) === name
                  )
        ...
        <div className="absolute top-4 left-4 z-10">
          <Select
            onValueChange={(value) => {
              ...
              if (newProductId)
                router.push(
                  `/dpp-global-tracker-v2?productId=${newProductId}`,
                  { scroll: false },
                );
          ...
          {countryFilter === "all"
            ...
            : countryFilter === "eu"
              ...
              : countryFilter === "supplyChain" && selectedProduct
                ? `Displaying Focus Area for ${selectedProduct}.`
                : "Select a product to view its focus area."}
          {selectedProduct &&
            highlightedCountries.length > 0 &&
            ` Highlighted: Amber/Orange.`}
          {arcsData.length > 0 &&
            (arcsData[0].color === "#3B82F6"
              ? ` Transit Arc: Blue.`
              : ` Supply Arc(s): Orange.`)}
        </div>
      </div>
