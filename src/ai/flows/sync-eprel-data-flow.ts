...
const SyncEprelOutputSchema = z.object({
  ...
  eprelId: z.string().optional().describe('The EPREL registration ID, if successfully synced/found.'),
  message: z.string().describe('A descriptive message about the sync attempt.'),
  lastChecked: z.string().describe('ISO string timestamp of this sync attempt.'),
});
export type SyncEprelOutput = z.infer<typeof SyncEprelOutputSchema>;

export async function syncEprelData(input: SyncEprelInput): Promise<SyncEprelOutput> {
  ...
  async (input) => {
    ...
    if (input.productId === "PROD001") {
      ...
      return {
        ...
        eprelId: input.modelNumber ? `EPREL_EXISTING_${input.modelNumber}` : undefined,
