...
export async function POST(request: NextRequest) {
  ...
  if (!qrIdentifier || typeof qrIdentifier !== 'string' || qrIdentifier.trim() === '') {
    return NextResponse.json({ error: { code: 400, message: "Invalid request body. 'qrIdentifier' (string) is required." } }, { status: 400 });
  }
  ...
  if (product) {
    ...
    return NextResponse.json({ error: { code: 404, message: `Product not found for QR identifier ${qrIdentifier}.` } }, { status: 404 });
  }
}
