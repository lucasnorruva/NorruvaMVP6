  ...
  const geoData = useMemo(() => {
    ...
    return Object.entries(counts)
      .map(([countryCode, count]) => ({
        countryCode,
        countryName:
          countryCodeToName[countryCode.toUpperCase()] || countryCode,
        count,
      }))
  ...
  return (
    <Card className="shadow-lg">
      <CardHeader>
        ...
        <CardDescription>
          Top 5 countries by number of DPPs originating from them.
        </CardDescription>
      </CardHeader>
      <CardContent>
        {geoData.length > 0 ? (
            ...
            {geoData.map((item) => (
              ...
              >
                ...
                <span className="font-semibold text-primary">
                  {item.count.toLocaleString()} DPPs
                </span>
              </li>
            ))}
          </ul>
          ...
          <p className="text-center text-muted-foreground py-6">
            No geographic origin data available.
          </p>
        )}
        <Button
          variant="link"
          asChild
          className="p-0 h-auto text-sm mt-4 text-primary"
        >
