...
import type {
  ...
  RecycledContentData,
  StateOfHealthData,
} from "@/types/dpp"; // Import specific types
...
const ExtractProductDataInputSchema = z.object({
  ...
  documentType: z
    .string()
    .describe(
      "The type of document being uploaded (e.g., 'invoice', 'specification', 'battery_spec_sheet', 'commercial_invoice', 'packing_list', 'material_safety_data_sheet').",
    ),
});
export type ExtractProductDataInput = z.infer<
  typeof ExtractProductDataInputSchema
>;

const CarbonFootprintSchemaForAI = z
  .object({
    value: z.coerce
      .number()
      .nullable()
      .optional()
      .describe("The carbon footprint value."),
    unit: z
      .string()
      .optional()
      .describe("Unit of the carbon footprint (e.g., kg CO2e/kWh)."),
    calculationMethod: z
      .string()
      .optional()
      .describe("Methodology used (e.g., PEFCR, ISO 14067)."),
    scope1Emissions: z.coerce
      .number()
      .nullable()
      .optional()
      .describe("Scope 1 emissions value."),
    scope2Emissions: z.coerce
      .number()
      .nullable()
      .optional()
      .describe("Scope 2 emissions value."),
    scope3Emissions: z.coerce
      .number()
      .nullable()
      .optional()
      .describe("Scope 3 emissions value."),
    dataSource: z
      .string()
      .optional()
      .describe("Primary data source for CF calculation."),
    vcId: z
      .string()
      .optional()
      .describe("Verifiable Credential ID for the CF data."),
  })
  .optional();

const RecycledContentSchemaForAI = z
  .object({
    material: z
      .string()
      .optional()
      .describe("Specific material (e.g., Cobalt, Lithium, Overall Battery)."),
    percentage: z.coerce
      .number()
      .nullable()
      .optional()
      .describe("Percentage of recycled content."),
    source: z
      .enum(["Pre-consumer", "Post-consumer", "Mixed", "Unknown"])
      .optional()
      .describe("Source of recycled material."),
    vcId: z
      .string()
      .optional()
      .describe("Verifiable Credential ID for this claim."),
  })
  .optional();

const StateOfHealthSchemaForAI = z
  .object({
    value: z.coerce
      .number()
      .nullable()
      .optional()
      .describe("State of Health value (e.g., percentage)."),
    unit: z
      .string()
      .optional()
      .describe("Unit for State of Health (e.g., '%')."),
    measurementDate: z
      .string()
      .optional()
      .describe("Date of SoH measurement (YYYY-MM-DD)."),
    measurementMethod: z
      .string()
      .optional()
      .describe("Method used for SoH measurement."),
    vcId: z
      .string()
      .optional()
      .describe("Verifiable Credential ID for SoH data."),
  })
  .optional();

const BatteryRegulationDetailsSchemaForAI = z
  .object({
    ...
    manufacturerName: z
      .string()
      .optional()
      .describe("Battery Manufacturer Name"),
    ...
    vcId: z
      .string()
      .optional()
      .describe("Overall VC ID for battery regulation compliance."),
  })
  .optional();

const ScipDataSchemaForAI = z
  .object({
    articleName: z
      .string()
      .optional()
      .describe("Name of the article as notified to SCIP."),
    primaryArticleId: z
      .string()
      .optional()
      .describe("Primary identifier of the article (e.g., EAN, Part Number)."),
    svhcListVersion: z
      .string()
      .optional()
      .describe("Version of the ECHA Candidate List used for assessment."),
    submittingLegalEntity: z
      .string()
      .optional()
      .describe(
        "Name or identifier of the legal entity that submitted the notification.",
      ),
    safeUseInstructionsLink: z
      .string()
      .url()
      .optional()
      .describe("Link to safe use instructions, if SVHCs are present."),
  })
  .optional();

const CustomsDataSchemaForAI = z
  .object({
    hsCode: z
      .string()
      .optional()
      .describe("Harmonized System (HS) code for customs classification."),
    countryOfOrigin: z
      .string()
      .optional()
      .describe("ISO 3166-1 alpha-2 code for the country of origin."),
  })
  .optional();
...
const ExtractProductDataOutputSchema = z.object({
  ...
  modelNumber: z
    .string()
    .describe("The model number of the product.")
    .optional(),
  specifications: z
  ...
  energyLabel: z
    .string()
    .describe("The energy label of the product, if available.")
    .optional(),

  batteryRegulation: BatteryRegulationDetailsSchemaForAI.describe(
    "Comprehensive battery regulation details.",
  ),

  scipData: ScipDataSchemaForAI.describe(
    "SCIP database related information if relevant and found.",
  ).optional(),
  customsData: CustomsDataSchemaForAI.describe(
    "Basic customs data if relevant and found.",
  ).optional(),
});
export type ExtractProductDataOutput = z.infer<
  typeof ExtractProductDataOutputSchema
>;

export async function extractProductData(
  input: ExtractProductDataInput,
): Promise<ExtractProductDataOutput> {
...
const extractProductDataFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    if (
      input.documentType.includes("battery") &&
      (!output || !output.batteryRegulation)
    ) {
