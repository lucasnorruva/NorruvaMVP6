...
export async function GET(
  ...
  const credentialSubject: Record<string, any> = {
    ...
    description:
      product.productDetails?.description?.substring(0, 150) +
        (product.productDetails?.description &&
        product.productDetails.description.length > 150
          ? "..."
  ...
  if (product.productDetails?.specifications) {
    try {
      ...
      specKeys.forEach((key) => {
        credentialSubject[
          `specification_${key.toLowerCase().replace(/\s+/g, "_")}`
        ] = specs[key];
      });
    } catch (e) {
  ...
  if (
    product.productDetails?.customAttributes &&
    product.productDetails.customAttributes.length > 0
  ) {
    product.productDetails.customAttributes.slice(0, 2).forEach((attr) => {
      credentialSubject[
        `custom_${attr.key.toLowerCase().replace(/\s+/g, "_")}`
      ] = attr.value;
    });
  }
  ...
  const credential = {
    ...
    type: [
      ...
      "DigitalProductPassportSummaryCredential_v1.0",
    ],
    issuer:
      product.manufacturer?.did ||
      `did:example:issuer:${product.manufacturer?.name.replace(/\s+/g, "") || "norruva"}`,
    issuanceDate: now,
