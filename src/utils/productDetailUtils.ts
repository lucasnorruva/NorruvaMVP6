...
function mapDppToSimpleProductDetail(
  ...
  if (dpp.productDetails?.keyCompliancePoints) {
    // Use directly if populated
    keyCompliancePointsPopulated.push(
      ...dpp.productDetails.keyCompliancePoints
        .split("\n")
        .map((s) => s.trim())
        .filter(Boolean),
    );
    ...
    if (
      complianceOverallStatusDetails.text &&
      complianceOverallStatusDetails.text.toLowerCase() !== "n/a" &&
      complianceOverallStatusDetails.text.toLowerCase() !== "no data"
    ) {
      keyCompliancePointsPopulated.push(
        `Overall Status: ${complianceOverallStatusDetails.text}`,
      );
    }
    if (
      dpp.ebsiVerification?.status &&
      dpp.ebsiVerification.status.toLowerCase() !== "n/a"
    ) {
      const ebsiStatusText = dpp.ebsiVerification.status.replace(/_/g, " ");
      const capitalizedEbsiStatus =
        ebsiStatusText.charAt(0).toUpperCase() + ebsiStatusText.slice(1);
      keyCompliancePointsPopulated.push(
        `EBSI Status: ${capitalizedEbsiStatus}`,
      );
    }
    if (
      dpp.compliance.battery_regulation?.status &&
      dpp.compliance.battery_regulation.status.toLowerCase() !==
        "not_applicable" &&
      dpp.compliance.battery_regulation.status.toLowerCase() !== "n/a" &&
      keyCompliancePointsPopulated.length < 3
    ) {
      const batteryStatusText =
        dpp.compliance.battery_regulation.status.replace(/_/g, " ");
      const capitalizedBatteryStatus =
        batteryStatusText.charAt(0).toUpperCase() + batteryStatusText.slice(1);
      keyCompliancePointsPopulated.push(
        `Battery Reg: ${capitalizedBatteryStatus}`,
      );
    }
    ...
    specificRegulations.forEach((reg) => {
      if (
        keyCompliancePointsPopulated.length < 3 &&
        reg.status &&
        reg.status.toLowerCase() !== "n/a" &&
        reg.status.toLowerCase() !== "not applicable" &&
        reg.status.toLowerCase() !== "not required"
      ) {
        const regStatusText = reg.status.replace(/_/g, " ");
        const capitalizedRegStatus =
          regStatusText.charAt(0).toUpperCase() + regStatusText.slice(1);
        keyCompliancePointsPopulated.push(
          `${reg.regulationName}: ${capitalizedRegStatus}`,
        );
      }
    });
    if (
      keyCompliancePointsPopulated.length === 0 &&
      (specificRegulations.length > 0 || dpp.compliance.battery_regulation)
    ) {
      keyCompliancePointsPopulated.push(
        "Review Compliance tab for regulation details.",
      );
    }
  }
  ...
  return {
    ...
    keySustainabilityPoints:
      dpp.productDetails?.sustainabilityClaims
        ?.map((c) => c.claim)
        .filter(Boolean) || [],
    keyCompliancePoints: keyCompliancePointsPopulated.join("\n"), // Store as a single string or keep as array
    specifications: dpp.productDetails?.specifications,
    customAttributes: customAttributes,
    productDetails: {
      // Pass through productDetails which now contains esprSpecifics
  ...
  if (storedProductsString) {
    ...
    if (userProductData) {
      ...
      const dppEquivalent: DigitalProductPassport = {
        ...
        productDetails: {
          description: userProductData.productDetails?.description, // Access from productDetails
          ...
          esprSpecifics: userProductData.productDetails?.esprSpecifics, // Pass through esprSpecifics
        },
        compliance: {
          eprel:
