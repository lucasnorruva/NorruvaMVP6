...
export default function DppDataManagementApiTutorialPage() {
  const sections = [
    ...
    },
    {
      ...
      content: (
        <ul className="list-disc list-inside space-y-2 text-sm">
          <li><strong>Use Correct HTTP Methods:</strong> Use <code className="bg-muted px-1 rounded-sm">POST /api/v1/dpp</code> for creating new DPPs and <code className="bg-muted px-1 rounded-sm">PUT /api/v1/dpp/{'{productId}'}</code> for updating existing ones. PUT requests should ideally be idempotent.</li>
          <li><strong>Batch Operations (Conceptual):</strong> For creating or updating multiple DPPs, utilize conceptual batch import endpoints like <code className="bg-muted px-1 rounded-sm">POST /api/v1/dpp/import</code> if available. This is generally more efficient than many individual requests.</li>
          <li><strong>Partial Updates:</strong> When updating a DPP, only send the fields that have changed to minimize payload size and processing, if the API supports partial updates via PUT or PATCH (conceptual, use <code className="bg-muted px-1 rounded-sm">PATCH /api/v1/dpp/extend/{'{productId}'}</code> for adding documents currently).</li>
        </ul>
      )
    },
    {
