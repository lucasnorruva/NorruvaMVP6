...
export default function ApiReferenceZkpLayerEndpoints({
  ...
  const errorExamples = { "400": error400General, "401": error401, "404": error404, "500": error500 };
  
  const zkpEndpoints: EndpointDefinition[] = [
    { title: "Submit Zero-Knowledge Proof for a DPP", method: "POST", path: "/api/v1/zkp/submit-proof/{dppId}", description: "[ZKP Layer - Highly Conceptual] Allows a prover to submit a ZKP related to a specific DPP.", pathParams: [{ name: "dppId", type: "string", required: true, description: "DPP ID." }], requestBodySchema: "ZkpSubmissionRequest", requestBodyExample: exampleZkpSubmitRequestBody, responseSchema: "ZkpSubmissionResponse", responseExample: exampleZkpSubmitResponseBody, commonErrors: ["400", "401", "404", "500"], errorExamples },
    { title: "Verify Claim with ZKP for a DPP", method: "GET", path: "/api/v1/zkp/verify-claim/{dppId}", description: "[ZKP Layer - Highly Conceptual] Allows a verifier to check if a claim for a DPP has a valid ZKP.", pathParams: [{ name: "dppId", type: "string", required: true, description: "DPP ID." }], queryParams: [{ name: "claimType", type: "string", required: true, description: "Type of claim to verify.", example: "material_compliance_svhc_x" }], responseSchema: "ZkpVerificationResponse", responseExample: exampleZkpVerifyResponseBody, commonErrors: ["400", "401", "404", "500"], errorExamples },
  ];
