  ...
  event.waitUntil(
    caches.open(CACHE_NAME).then((cache) => {
      ...
      return cache.addAll(APP_SHELL_URLS).catch((error) => {
        console.error(
          "[SW] Failed to cache app shell resources during install:",
          error,
        );
      });
    }),
    ...
    event.respondWith(
      caches.match(request).then((cachedResponse) => {
        ...
        return fetch(request)
          ...
          .catch((error) => {
            console.error(
              "[SW] Fetch failed for app shell resource:",
              request.url,
              error,
            );
      ...
      fetch(request)
        ...
        .catch(() => {
          return caches.match(request).then((cachedResponse) => {
            ...
            return new Response(
              JSON.stringify({ error: "Network error and not in cache" }),
              {
                headers: { "Content-Type": "application/json" },
                status: 503, // Service Unavailable
                statusText:
                  "Network error, data might be stale or unavailable.",
              },
            );
          });
        }),
    ...
    caches.match(request).then((cachedResponse) => {
      return (
        ...
        fetch(request)
          ...
          .catch((error) => {
            console.warn(
              "[SW] Unhandled fetch, falling back to network, error:",
              request.url,
              error,
            );
