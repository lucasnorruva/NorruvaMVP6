      ...
      <CardContent className="space-y-4">
        {digitalTwinData.uri && (
          <div>
            ...
            <Link href={digitalTwinData.uri} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline break-all text-sm">
              {digitalTwinData.uri}
            </Link>
          </div>
        )}
        ...
        {digitalTwinData.sensorDataEndpoint && (
          <div>
            ...
            <Link href={digitalTwinData.sensorDataEndpoint} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline break-all text-sm">
              {digitalTwinData.sensorDataEndpoint}
            </Link>
          </div>
        )}
        ...
        {digitalTwinData.realTimeStatus && (
          <div>
            ...
            <p className="text-sm text-foreground/90 p-2 bg-muted/30 rounded-md whitespace-pre-line">{digitalTwinData.realTimeStatus}</p>
          </div>
        )}
        ...
        {digitalTwinData.predictiveMaintenanceAlerts && (
          <div>
            ...
            <div className="text-sm text-foreground/90 p-2 bg-orange-500/10 border border-orange-500/30 rounded-md whitespace-pre-line">
              {digitalTwinData.predictiveMaintenanceAlerts.split('\n').map((alert, index) => (
                <p key={index} className="mb-0.5 last:mb-0">{alert.startsWith('-') ? alert : `- ${alert}`}</p>
              ))}
            </div>
          </div>
        )}
