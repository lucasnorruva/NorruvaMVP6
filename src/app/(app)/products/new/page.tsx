...
import type {
  ...
  RecycledContentData,
  EsprSpecifics,
} from "@/types/dpp"; // Added EsprSpecifics
...
interface ProductDetailsOrigin {
  // New interface for origin tracking of productDetails sub-objects
...
export interface InitialProductFormData
  extends Omit<
    ProductFormData,
...
const defaultBatteryRegulationOriginState: BatteryRegulationOrigin = {
  batteryChemistryOrigin: undefined,
  batteryPassportIdOrigin: undefined,
  manufacturerNameOrigin: undefined,
  manufacturingDateOrigin: undefined,
  ratedCapacityAhOrigin: undefined,
  nominalVoltageOrigin: undefined,
  expectedLifetimeCyclesOrigin: undefined,
  recyclingEfficiencyRateOrigin: undefined,
  dismantlingInformationUrlOrigin: undefined,
  safetyInformationUrlOrigin: undefined,
  carbonFootprintOrigin: {
  ...
  stateOfHealthOrigin: {
    valueOrigin: undefined,
    unitOrigin: undefined,
    measurementDateOrigin: undefined,
    measurementMethodOrigin: undefined,
    vcIdOrigin: undefined,
  },
  materialRecoveryRatesOrigin: {
    cobaltOrigin: undefined,
    leadOrigin: undefined,
    lithiumOrigin: undefined,
    nickelOrigin: undefined,
  },
  vcIdOrigin: undefined,
};
...
export default function AddNewProductPage() {
  ...
  const [activeTab, setActiveTab] = useState(
    isEditMode ? "manual" : "ai-extraction",
  );
  const [aiExtractionAppliedSuccessfully, setAiExtractionAppliedSuccessfully] =
    useState(false);
  ...
  const defaultFormState: InitialProductFormData = {
    ...
    constructionProductInformation: {
      ...defaultConstructionProductInformationState,
    },
  };

  const [currentProductDataForForm, setCurrentProductDataForForm] =
    useState<InitialProductFormData>(defaultFormState);

  useEffect(() => {
    if (isEditMode && editProductId) {
      const storedProductsString = localStorage.getItem(
        USER_PRODUCTS_LOCAL_STORAGE_KEY,
      );
      const userProducts: StoredUserProduct[] = storedProductsString
      ...
      if (productToEdit) {
        const editData: InitialProductFormData = {
          ...
          onChainLifecycleStage:
            productToEdit.metadata?.onChainLifecycleStage || "Unknown",
          conflictMineralsReportUrl:
            productToEdit.conflictMineralsReportUrl || "",
          fairTradeCertificationId:
            productToEdit.fairTradeCertificationId || "",
          ethicalSourcingPolicyUrl:
            productToEdit.ethicalSourcingPolicyUrl || "",
          productDetails: {
            ...
            customAttributesJsonString:
              productToEdit.customAttributesJsonString || "",
            esprSpecifics: {
          ...
          compliance: {
            eprel: {
              ...(defaultFormState.compliance?.eprel || {}),
              ...(productToEdit.complianceData?.eprel || {}),
            },
            esprConformity: {
              ...(defaultFormState.compliance?.esprConformity || {}),
              ...(productToEdit.complianceData?.esprConformity || {}),
            },
            scipNotification: {
              ...defaultScipNotificationState,
              ...(productToEdit.complianceData?.scipNotification || {}),
            },
            euCustomsData: {
              ...defaultEuCustomsDataState,
              ...(productToEdit.complianceData?.euCustomsData || {}),
              customsValuation: {
            ...
            battery_regulation: {
              ...defaultBatteryRegulationState,
              ...(productToEdit.complianceData?.battery_regulation ||
                productToEdit.batteryRegulation ||
                {}),
              carbonFootprint: {
                ...defaultBatteryRegulationState.carbonFootprint,
          ...
          textileInformation: {
            ...defaultTextileInformationState,
            ...(productToEdit.textileInformation || {}),
          },
          constructionProductInformation: {
            ...defaultConstructionProductInformationState,
            ...(productToEdit.constructionProductInformation || {}),
          },
          ...
          productDetailsOrigin: productToEdit.productDetailsOrigin || {
            ...defaultProductDetailsOriginState,
          },
          batteryRegulationOrigin: productToEdit.batteryRegulationOrigin || {
            ...defaultBatteryRegulationOriginState,
          },
        };
  ...
  const handleExtractData = async () => {
    ...
    try {
      ...
      if (result.productName) {
        aiInitialFormData.productName = result.productName;
        aiInitialFormData.productNameOrigin = "AI_EXTRACTED";
      }
      if (aiInitialFormData.productDetails && result.productDescription) {
        aiInitialFormData.productDetails.description =
          result.productDescription;
        if (aiInitialFormData.productDetailsOrigin)
          aiInitialFormData.productDetailsOrigin.descriptionOrigin =
            "AI_EXTRACTED";
      }
      if (result.manufacturer) {
        aiInitialFormData.manufacturer = result.manufacturer;
        aiInitialFormData.manufacturerOrigin = "AI_EXTRACTED";
      }
      if (result.modelNumber) {
        aiInitialFormData.modelNumber = result.modelNumber;
        aiInitialFormData.modelNumberOrigin = "AI_EXTRACTED";
      }
      ...
      if (
        ...
        if (aiInitialFormData.productDetails)
          aiInitialFormData.productDetails.specifications = JSON.stringify(
            result.specifications,
            null,
            2,
          );
        if (aiInitialFormData.productDetailsOrigin)
          aiInitialFormData.productDetailsOrigin.specificationsOrigin =
            "AI_EXTRACTED";
      } else {
        if (aiInitialFormData.productDetails)
          aiInitialFormData.productDetails.specifications = "";
        if (result.specifications && aiInitialFormData.productDetailsOrigin)
          aiInitialFormData.productDetailsOrigin.specificationsOrigin =
            "AI_EXTRACTED";
      }

      if (result.energyLabel && aiInitialFormData.productDetails) {
        aiInitialFormData.productDetails.energyLabel = result.energyLabel;
        if (aiInitialFormData.productDetailsOrigin)
          aiInitialFormData.productDetailsOrigin.energyLabelOrigin =
            "AI_EXTRACTED";
      }

      if (aiInitialFormData.batteryRegulation) {
        if (result.batteryRegulation?.batteryChemistry) {
          aiInitialFormData.batteryRegulation.batteryChemistry =
            result.batteryRegulation.batteryChemistry;
          if (aiInitialFormData.batteryRegulationOrigin)
            aiInitialFormData.batteryRegulationOrigin.batteryChemistryOrigin =
              "AI_EXTRACTED";
        }
        if (result.batteryRegulation?.batteryPassportId) {
          aiInitialFormData.batteryRegulation.batteryPassportId =
            result.batteryRegulation.batteryPassportId;
          if (aiInitialFormData.batteryRegulationOrigin)
            aiInitialFormData.batteryRegulationOrigin.batteryPassportIdOrigin =
              "AI_EXTRACTED";
        }
        ...
        if (result.batteryRegulation?.carbonFootprint) {
          aiInitialFormData.batteryRegulation.carbonFootprint = {
            ...
            calculationMethod:
              result.batteryRegulation.carbonFootprint.calculationMethod || "",
            scope1Emissions:
              result.batteryRegulation.carbonFootprint.scope1Emissions ?? null,
            scope2Emissions:
              result.batteryRegulation.carbonFootprint.scope2Emissions ?? null,
            scope3Emissions:
              result.batteryRegulation.carbonFootprint.scope3Emissions ?? null,
            dataSource:
              result.batteryRegulation.carbonFootprint.dataSource || "",
            vcId: result.batteryRegulation.carbonFootprint.vcId || "",
          };
          if (aiInitialFormData.batteryRegulationOrigin)
            aiInitialFormData.batteryRegulationOrigin.carbonFootprintOrigin = {
              valueOrigin: "AI_EXTRACTED",
              unitOrigin: "AI_EXTRACTED",
              calculationMethodOrigin: "AI_EXTRACTED",
              scope1EmissionsOrigin: "AI_EXTRACTED",
              scope2EmissionsOrigin: "AI_EXTRACTED",
              scope3EmissionsOrigin: "AI_EXTRACTED",
              dataSourceOrigin: "AI_EXTRACTED",
              vcIdOrigin: "AI_EXTRACTED",
            };
        }

        if (result.batteryRegulation?.recycledContent) {
          aiInitialFormData.batteryRegulation.recycledContent =
            result.batteryRegulation.recycledContent.map((rc) => ({
          ...
          if (aiInitialFormData.batteryRegulationOrigin)
            aiInitialFormData.batteryRegulationOrigin.recycledContentOrigin =
              result.batteryRegulation.recycledContent.map(() => ({
                materialOrigin: "AI_EXTRACTED",
                percentageOrigin: "AI_EXTRACTED",
        ...
        if (result.batteryRegulation?.stateOfHealth) {
          aiInitialFormData.batteryRegulation.stateOfHealth = {
            ...
            measurementDate:
              result.batteryRegulation.stateOfHealth.measurementDate || "",
            measurementMethod:
              result.batteryRegulation.stateOfHealth.measurementMethod || "",
            vcId: result.batteryRegulation.stateOfHealth.vcId || "",
          };
          if (aiInitialFormData.batteryRegulationOrigin)
            aiInitialFormData.batteryRegulationOrigin.stateOfHealthOrigin = {
              valueOrigin: "AI_EXTRACTED",
              unitOrigin: "AI_EXTRACTED",
              measurementDateOrigin: "AI_EXTRACTED",
              measurementMethodOrigin: "AI_EXTRACTED",
              vcIdOrigin: "AI_EXTRACTED",
            };
        }
        if (result.batteryRegulation?.vcId) {
          aiInitialFormData.batteryRegulation.vcId =
            result.batteryRegulation.vcId;
          if (aiInitialFormData.batteryRegulationOrigin)
            aiInitialFormData.batteryRegulationOrigin.vcIdOrigin =
              "AI_EXTRACTED";
        }
      }

      if (result.scipData && aiInitialFormData.compliance?.scipNotification) {
        if (result.scipData.articleName)
          aiInitialFormData.compliance.scipNotification.articleName =
            result.scipData.articleName;
        if (result.scipData.primaryArticleId)
          aiInitialFormData.compliance.scipNotification.primaryArticleId =
            result.scipData.primaryArticleId;
        if (result.scipData.svhcListVersion)
          aiInitialFormData.compliance.scipNotification.svhcListVersion =
            result.scipData.svhcListVersion;
        if (result.scipData.submittingLegalEntity)
          aiInitialFormData.compliance.scipNotification.submittingLegalEntity =
            result.scipData.submittingLegalEntity;
        if (result.scipData.safeUseInstructionsLink)
          aiInitialFormData.compliance.scipNotification.safeUseInstructionsLink =
            result.scipData.safeUseInstructionsLink;
      }
      if (result.customsData && aiInitialFormData.compliance?.euCustomsData) {
        if (result.customsData.hsCode)
          aiInitialFormData.compliance.euCustomsData.hsCode =
            result.customsData.hsCode;
        if (result.customsData.countryOfOrigin)
          aiInitialFormData.compliance.euCustomsData.countryOfOrigin =
            result.customsData.countryOfOrigin;
      }
      ...
      if (aiInitialFormData.productDetails) {
        ...
        if (aiInitialFormData.productDetailsOrigin)
          aiInitialFormData.productDetailsOrigin.imageUrlOrigin = undefined;
      ...
      toast({
        ...
        description:
          "Information has been extracted. Please review and complete in the 'Manual Entry / Review' tab. AI-suggested fields are marked.",
        variant: "default",
        duration: 7000,
  ...
  const handleProductFormSubmit = async (formDataFromForm: ProductFormData) => {
    ...
    try {
      ...
      const productCoreData: StoredUserProduct = {
        id:
          isEditMode && editProductId
            ? editProductId
            : `USER_PROD_${Date.now().toString().slice(-6)}`,
        ...formDataFromForm,
        productName: formDataFromForm.productName || "Unnamed Product",
        status:
          (isEditMode && editProductId
            ? userProducts.find((p) => p.id === editProductId)?.status
            : "Draft") || "Draft",
        compliance:
          (isEditMode && editProductId
            ? userProducts.find((p) => p.id === editProductId)?.compliance
            : "N/A") || "N/A",
        lastUpdated: new Date().toISOString(),

        supplyChainLinks:
          isEditMode && editProductId
            ? userProducts.find((p) => p.id === editProductId)
                ?.supplyChainLinks || []
            : [],
        lifecycleEvents:
          isEditMode && editProductId
        ...
        productDetails: {
          ...
          sustainabilityClaims:
            formDataFromForm.productDetails?.sustainabilityClaims,
          keyCompliancePoints:
            formDataFromForm.productDetails?.keyCompliancePoints,
          ...
          customAttributesJsonString:
            formDataFromForm.productDetails?.customAttributesJsonString,
        ...
        metadata: {
          ...
          status:
            (isEditMode && editProductId
              ? (userProducts.find((p) => p.id === editProductId)?.metadata
                  ?.status as StoredUserProduct["status"])
              : "draft") || "draft",
          dppStandardVersion:
            (isEditMode && editProductId
              ? userProducts.find((p) => p.id === editProductId)?.metadata
                  ?.dppStandardVersion
              : "CIRPASS v1.0 Draft") || "CIRPASS v1.0 Draft",
        },

        complianceSummary: {
          overallStatus: "Pending Review",
          eprel: {
            status: formDataFromForm.compliance?.eprel?.status || "N/A",
          ...
          scip: formDataFromForm.compliance?.scipNotification
            ? {
                status:
                  formDataFromForm.compliance.scipNotification.status || "N/A",
                notificationId:
                  formDataFromForm.compliance.scipNotification.notificationId,
          ...
          euCustomsData: formDataFromForm.compliance?.euCustomsData
            ? {
                status:
                  formDataFromForm.compliance.euCustomsData.status || "N/A",
                declarationId:
                  formDataFromForm.compliance.euCustomsData.declarationId,
                cbamGoodsIdentifier:
                  formDataFromForm.compliance.euCustomsData.cbamGoodsIdentifier,
                lastChecked: new Date().toISOString(),
              }
            : undefined,
          battery: formDataFromForm.compliance?.battery_regulation
            ? {
                status:
                  formDataFromForm.compliance.battery_regulation.status ||
                  "not_applicable",
                batteryChemistry:
                  formDataFromForm.compliance.battery_regulation
                    .batteryChemistry,
              }
            : undefined,
        },
      ...
      if (isEditMode && editProductId) {
        ...
        if (productIndex > -1) {
          ...
          toast({
            ...
            description: `${productCoreData.productName} has been updated.`,
            variant: "default",
            action: <CheckCircle2 className="text-success" />,
          });
        ...
        toast({
          title: "Product Saved",
          description: `${productCoreData.productName} has been saved.`,
          variant: "default",
          action: <CheckCircle2 className="text-success" />,
        });
      ...
      toast({
        ...
        description: `Could not ${action} the product. ${e instanceof Error ? e.message : ""}`,
        variant: "destructive",
      });
    } finally {
    ...
    <div className="space-y-8">
        ...
        <Card className="shadow-md bg-muted/30 border-primary/20">
          ...
          <CardContent className="text-sm text-muted-foreground grid md:grid-cols-2 gap-4">
            <div>
              ...
              <p>
                Upload a product document (like a spec sheet or invoice) and let
                our AI extract key information to pre-fill the form. Great for
                quick starts and complex products.
              </p>
            </div>
            <div>
              <h4 className="font-semibold text-primary mb-1 flex items-center">
              ...
              <p>
                Prefer to enter details yourself or have specific information
                ready? Head straight to the manual form. You can still use AI to
                suggest content for individual fields.
              </p>
            </div>
          </CardContent>
        </Card>
      ...
      >
        <TabsList className="grid w-full grid-cols-2 md:w-[400px]">
          <TabsTrigger value="ai-extraction" disabled={isEditMode}>
            AI Data Extraction
          </TabsTrigger>
          <TabsTrigger value="manual">
            {isEditMode ? "Edit Product Details" : "Manual Entry / Review"}
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="ai-extraction" className="mt-6">
          {isEditMode ? (
            <Alert>
              ...
              <AlertDescription>
                To use AI data extraction for a new base, please create a new
                product. You are currently editing an existing product.
              </AlertDescription>
            </Alert>
          ) : (
            <AiExtractionSection
