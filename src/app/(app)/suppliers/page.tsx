...
} from "@/components/suppliers/AddSupplierForm";
...
export default function SuppliersPage() {
  ...
  const saveSuppliersToLocalStorage = (updatedSuppliers: Supplier[]) => {
    const userAddedSuppliers = updatedSuppliers.filter((sup) =>
      sup.id.startsWith("USER_SUP"),
    );
    localStorage.setItem(
      USER_SUPPLIERS_LOCAL_STORAGE_KEY,
      JSON.stringify(userAddedSuppliers),
    );
  };
    ...
    if (editingSupplier) {
      ...
      updatedSuppliers = suppliers.map((s) =>
        s.id === editingSupplier.id
          ? { ...s, ...data, id: s.id, lastUpdated: now }
          : s,
      );
      toast({
      ...
      updatedSuppliers = [...suppliers, newSupplier].sort((a, b) =>
        a.name.localeCompare(b.name),
      );
      toast({
        title: "Supplier Added",
        description: `Supplier "${newSupplier.name}" has been added.`,
      });
    }
  ...
  const handleDeleteSupplier = (supplierId: string) => {
    if (!supplierId.startsWith("USER_SUP")) {
      toast({
        ...
        description: "Mock suppliers cannot be deleted.",
        variant: "destructive",
      });
      return;
    }
    ...
    <div className="space-y-8">
      ...
      <Card className="shadow-lg">
        <CardHeader>
          ...
          <CardDescription>
            View, add, or manage suppliers in your network.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <SupplierList
