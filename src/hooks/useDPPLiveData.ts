...
export function useDPPLiveData() {
  ...
  useEffect(() => {
    ...
    const combinedProducts = [
      ...MOCK_DPPS.filter(
        (mockDpp) =>
          !userAddedProducts.find((userDpp) => userDpp.id === mockDpp.id),
      ),
      ...userAddedProducts,
    ];
    ...
    const filtered = dpps.filter((dpp) => {
      ...
      if (filters.regulation !== "all") {
        ...
        if (filters.regulation === "eu_espr") {
          isCompliantForRegulation =
            complianceSection.eu_espr?.status === "compliant" ||
            complianceSection.esprConformity?.status === "conformant";
        } else if (filters.regulation === "us_scope3") {
          isCompliantForRegulation =
            complianceSection.us_scope3?.status === "compliant";
        } else if (filters.regulation === "battery_regulation") {
          isCompliantForRegulation =
            complianceSection.battery_regulation?.status === "compliant";
        }
      ...
      if (filters.category !== "all" && dpp.category !== filters.category)
        return false;
      if (
        filters.blockchainAnchored === "anchored" &&
        !dpp.blockchainIdentifiers?.anchorTransactionHash
      )
        return false;
      if (
        filters.blockchainAnchored === "not_anchored" &&
        dpp.blockchainIdentifiers?.anchorTransactionHash
      )
        return false;
      return true;
  ...
  const metrics = useMemo(() => {
    ...
    const fullyCompliantDPPsCount = dpps.filter((dpp) => {
      ...
      if (
        relevantComplianceSections.length === 0 &&
        Object.keys(dpp.compliance).filter(
          (k) => dpp.compliance[k as keyof typeof dpp.compliance] !== undefined,
        ).length > 0
      )
        return false; // If compliance sections exist but all are N/A
      if (
        relevantComplianceSections.length === 0 &&
        Object.keys(dpp.compliance).filter(
          (k) => dpp.compliance[k as keyof typeof dpp.compliance] !== undefined,
        ...
        return true; // No applicable regulations, considered compliant

      const isEbsiCompliant =
        !dpp.ebsiVerification ||
        dpp.ebsiVerification.status === "verified" ||
        dpp.ebsiVerification.status === "N/A";

      return (
        isEbsiCompliant &&
        relevantComplianceSections.every(
          (section) =>
    ...
    const compliantPercentage =
      totalDPPs > 0
        ...
        : "0%";
    const pendingReviewDPPs = dpps.filter(
      (d) => d.metadata.status === "pending_review",
    ).length;
    const totalConsumerScans = dpps.reduce(
      (sum, dpp) => sum + (dpp.consumerScans || 0),
      0,
    );
    const averageConsumerScans =
      totalDPPs > 0 ? (totalConsumerScans / totalDPPs).toFixed(1) : "0";
    return {
      totalDPPs,
      compliantPercentage,
      pendingReviewDPPs,
  ...
  const handleFiltersChange = useCallback(
    (newFilters: Partial<DashboardFiltersState>) => {
  ...
  const confirmDeleteProduct = useCallback(() => {
    ...
    if (productIsUserAdded) {
      ...
      let userProducts: DigitalProductPassport[] = storedProductsString
