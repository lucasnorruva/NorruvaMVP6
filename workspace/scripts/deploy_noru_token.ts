...
async function main() {
  ...
  console.log(
    "Deploying NORUToken contract with the account:",
    deployer.address,
  );
  ...
  const implementationAddress =
    await upgrades.erc1967.getImplementationAddress(noruTokenAddress);
  ...
  console.log(
    `Initial supply of ${ethers.formatUnits(balance, 18)} NORU minted to deployer ${deployer.address}`,
  );
  console.log(
    `Token Cap set to: ${ethers.formatUnits(await noruToken.cap(), 18)} NORU`,
  );
  console.log(
    `Deployer has MINTER_ROLE: ${await noruToken.hasRole(await noruToken.MINTER_ROLE(), deployer.address)}`,
  );
}
