...
export default async function handler(
  ...
  if (req.method === "POST") {
    try {
      ...
      const dppToken = new ethers.Contract(
        dppTokenAddress,
        dppTokenAbi,
        signer,
      );
      ...
      if (!recipient || !metadataHash) {
        return NextResponse.json(
          { error: "Recipient and metadataHash are required" },
          { status: 400 },
        );
      }

      console.log(
        `Attempting to mint DPP for recipient: ${recipient} with metadataHash: ${metadataHash} using contract at ${dppTokenAddress}`,
      );
      ...
      return NextResponse.json(
        { message: "DPP minted successfully", transactionHash: tx.hash },
        { status: 200 },
      );
    } catch (error) {
