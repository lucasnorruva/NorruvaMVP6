...
export default function ProductImageFormSection({
  ...
  const triggerImageGeneration = async () => {
    const newImageUrl = await aiImageHelper(
      form,
      toast,
      setIsGeneratingImageState,
    );
      ...
      <div className="w-full max-w-md mx-auto">
        {currentImageUrl ? (
          <div className="border rounded-md overflow-hidden shadow-sm bg-muted/30 p-2">
            ...
            {(form.getValues("imageUrlOrigin") === "AI_EXTRACTED" ||
              (initialImageUrlOrigin === "AI_EXTRACTED" &&
                form.getValues("imageUrl") === initialImageUrl)) && (
              <AiIndicator
        ...
        render={({ field }) => (
          <FormItem>
            <FormLabel className="flex items-center">
              ...
              <AiIndicator
                fieldOrigin={
                  form.getValues("imageUrlOrigin") || initialImageUrlOrigin
                }
                fieldName="Image URL"
              />
            </FormLabel>
