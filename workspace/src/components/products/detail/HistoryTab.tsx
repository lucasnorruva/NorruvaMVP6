...
import { Loader2, AlertTriangle, History as HistoryIconLucide, User, Edit, CalendarDays, Info as InfoIcon, FilePlus2, ShieldCheck, Award, ListChecks, Truck, Factory, Search } from 'lucide-react';
...
const getActionIcon = (actionType: string): React.ElementType => {
  ...
  if (lowerAction.includes("updated") || lowerAction.includes("modified")) return Edit;
  ...
  if (lowerAction.includes("ebsi verification") || lowerAction.includes("blockchain")) return ShieldCheck;
      ...
      <CardContent>
        ...
        {filteredHistory.length === 0 ? (
          <p className="text-muted-foreground text-center py-6">
            {debouncedFilterText.trim() ? "No history entries match your filter." : "No historical events recorded for this product."}
          </p>
        ) : (
