...
const TOKEN_EXPLORER_BASE_URL =
  "https://mock-token-explorer.example.com/token/";
...
const getEbsiStatusBadge = (
  ...
  return (
    <Badge variant="outline" className={cn("capitalize", classes)}>
...
function BlockchainStatus({ product }: { product: DigitalProductPassport }) {
  ...
  const hasEbsiInfo =
    ...
    product.ebsiVerification?.issuanceDate;
  ...
  return (
    <div className="space-y-3 text-xs">
      {hasBlockchainInfo && (
        <div>
          <h4 className="text-sm font-semibold text-primary mb-1.5 flex items-center">
            <LinkIconPath className="h-4 w-4 mr-1.5" />
          ...
          {product.blockchainIdentifiers?.platform && (
            <TooltipProvider delayDuration={100}>
              <Tooltip>
                ...
                <TooltipContent>
                  <p>The blockchain network or platform used.</p>
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
          )}
          {product.blockchainIdentifiers?.contractAddress && (
            <TooltipProvider delayDuration={100}>
              <Tooltip>
                ...
                <TooltipContent>
                  <p>
                    Address of the smart contract managing this DPP or token.
                  </p>
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
          ...
          {product.blockchainIdentifiers?.tokenId && (
            <TooltipProvider delayDuration={100}>
              <Tooltip>
                ...
                <TooltipContent>
                  <p>Unique identifier of the NFT representing this DPP.</p>
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
          )}
          {product.blockchainIdentifiers?.anchorTransactionHash && (
            <>
                ...
                <Tooltip>
                  <TooltipTrigger className="text-left w-full">
                    <div className="flex items-center gap-1 mb-0.5">
                      ...
                      >
                        {product.blockchainIdentifiers.anchorTransactionHash.substring(
                          0,
                          10,
                        )}
                        ...
                        {product.blockchainIdentifiers.anchorTransactionHash.slice(
                          -8,
                        )}
                      </span>
                    </div>
                  ...
                  <TooltipContent>
                    <p>
                      Transaction hash proving the DPP data was anchored
                      on-chain.
                    </p>
                  </TooltipContent>
                </Tooltip>
              ...
              <div className="flex items-center gap-1">
                ...
                <Link
                  href={`${TX_EXPLORER_BASE_URL}${product.blockchainIdentifiers.anchorTransactionHash}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-primary hover:underline"
                >
                  View on Mock Explorer{" "}
      ...
      {hasEbsiInfo && (
        <div
          className={cn(
            hasBlockchainInfo && "mt-2 pt-2 border-t border-border/30",
          )}
        >
          <h4 className="text-sm font-semibold text-primary mb-1.5 flex items-center">
          ...
          {product.ebsiVerification?.status && (
            <div className="flex items-center gap-1 mb-0.5">
              ...
              <div className="flex items-center mt-0.5">
                {getEbsiStatusBadge(product.ebsiVerification.status)}
              </div>
            </div>
          )}
          {product.ebsiVerification?.verificationId && (
          ...
          {product.ebsiVerification?.issuerDid && (
            <TooltipProvider delayDuration={100}>
              <Tooltip>
                ...
                <TooltipContent>
                  <p>
                    Decentralized Identifier of the entity that issued the EBSI
          ...
          {product.ebsiVerification?.schema && (
            <TooltipProvider delayDuration={100}>
              <Tooltip>
                ...
                <TooltipContent>
                  <p>
                    The schema or type definition of the Verifiable Credential.
                  </p>
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
      ...
      {(hasAuthVc || hasOwnershipNft) && (
        ...
        >
          <h4 className="text-sm font-semibold text-primary mb-1.5 flex items-center">
            ...
            Authenticity & Ownership VCs/NFTs
          </h4>
          {hasAuthVc && (
            <div className="flex items-center gap-1 mb-0.5">
              <FileLock className="h-3.5 w-3.5 text-muted-foreground" />
              <span className="text-muted-foreground">Auth VC ID:</span>
              <span className="font-mono break-all text-foreground/90">
          ...
          {hasOwnershipNft && (
            <div className="mt-1">
              <div className="flex items-center gap-1 mb-0.5">
                <Tag className="h-3.5 w-3.5 text-muted-foreground" />
                <span className="text-muted-foreground">Ownership NFT:</span>
              </div>
      ...
      {!hasBlockchainInfo && !hasEbsiInfo && !hasAuthVc && !hasOwnershipNft && (
        <p className="text-muted-foreground text-sm">
          No specific blockchain, EBSI, authenticity VC or ownership NFT details
...
const getTokenStatusVisuals = (status?: string) => {
  ...
  if (s.includes("minted") || s.includes("active"))
    return {
      icon: CheckCircleLucide,
      color: "text-green-600",
      badge: "default" as const,
    };
  if (s.includes("transferred"))
    return { icon: Send, color: "text-blue-500", badge: "outline" as const };
  if (s.includes("burned") || s.includes("locked"))
    return {
      icon: AlertCircle,
      color: "text-red-600",
...
const initialMockDaoProposals: MockDaoProposal[] = [
  {
    id: "PROP001",
    title: "Enable Transfer for Token DPP001",
    description: "Owner request for resale of EcoSmart Refrigerator X500.",
  ...
  {
    id: "PROP002",
    title: "Grant MINTER_ROLE to 0xNewMinterAddr...",
    description:
      "Onboarding a new manufacturing partner for smart bulb production.",
    status: "Succeeded & Executed",
  },
  {
    id: "PROP003",
    title: "Adjust Voting Quorum to 3%",
    description:
      "Proposal to lower the quorum threshold for faster decision-making.",
    status: "Defeated",
    votesFor: 120,
    votesAgainst: 300,
  },
  {
    id: "PROP004",
    title: "Pause DPPToken Contract for Maintenance",
    description:
      "Scheduled upgrade of DPPToken logic requires temporary pause.",
    status: "Queued",
  },
];

export default function BlockchainPage() {
  const [filter, setFilter] = useState<"all" | "anchored" | "not_anchored">(
    "all",
  );
  ...
  const [isActionLoading, setIsActionLoading] = useState<string | boolean>(
    false,
  );

  const [anchorPlatform, setAnchorPlatform] = useState("EBSI Mock Ledger");
  const [custodyStep, setCustodyStep] = useState({
    stepName: "",
    actorDid: "",
    timestamp: "",
    location: "",
  ...
  const [mintContractAddress, setMintContractAddress] = useState(
    "0xMOCK_DPP_TOKEN_CONTRACT",
  );
  const [mintRecipientAddress, setMintRecipientAddress] = useState(
    "0xRECIPIENT_MOCK_ADDRESS",
  );
  const [mintMetadataUri, setMintMetadataUri] = useState("");
  const [mintResponse, setMintResponse] = useState<MintTokenResponse | null>(
    null,
  );

  const [updateTokenId, setUpdateTokenId] = useState("");
  const [updateMetadataUri, setUpdateMetadataUri] = useState(
    "ipfs://new-mock-metadata-cid",
  );
  const [updateTokenResponse, setUpdateTokenResponse] =
    useState<UpdateTokenMetadataResponse | null>(null);

  const [statusTokenId, setStatusTokenId] = useState("");
  const [statusTokenResponse, setStatusTokenResponse] = useState<string | null>(
    null,
  );
  const [parsedTokenStatus, setParsedTokenStatus] =
    useState<TokenStatusResponse | null>(null);

  const [viewTokenId, setViewTokenId] = useState("");
  const [viewTokenMetadataResponse, setViewTokenMetadataResponse] = useState<
    string | null
  >(null);
  const [isViewingTokenMetadata, setIsViewingTokenMetadata] = useState(false);

  const [onChainStatusUpdate, setOnChainStatusUpdate] =
    useState<string>("active");
  const [criticalEventDescription, setCriticalEventDescription] =
    useState<string>("");
  const [criticalEventSeverity, setCriticalEventSeverity] = useState<
    "High" | "Medium" | "Low"
  >("High");
  const [isUpdatingOnChainStatusLoading, setIsUpdatingOnChainStatusLoading] =
    useState(false);
  const [isLoggingCriticalEventLoading, setIsLoggingCriticalEventLoading] =
    useState(false);

  const [onChainLifecycleStage, setOnChainLifecycleStage] =
  ...
  const [isUpdatingLifecycleStageLoading, setIsUpdatingLifecycleStageLoading] =
    useState(false);
  const [isRegisteringVcHashLoading, setIsRegisteringVcHashLoading] =
    useState(false);
  ...
  const [proposalTargetTokenId, setProposalTargetTokenId] =
    useState<string>("");
  const [proposalActionType, setProposalActionType] =
    useState<string>("enableTransfer");
  const [proposalTargetAddress, setProposalTargetAddress] =
    useState<string>("");
  const [proposalDescription, setProposalDescription] = useState<string>("");
  const [isSubmittingProposal, setIsSubmittingProposal] = useState(false);
  const [daoProposals, setDaoProposals] = useState<MockDaoProposal[]>(
    initialMockDaoProposals,
  );
    ...
    fetch(`/api/v1/dpp?blockchainAnchored=${filter}`, {
      ...
      .then(async (res) => {
        if (!res.ok) {
          ...
          toast({
            title: "Error fetching DPPs",
            description: errorBody?.error?.message || `Status: ${res.status}`,
            variant: "destructive",
          });
          setDpps([]);
          return {
            ...
            error: {
              message:
                ...
                `Error fetching DPPs: ${res.status}`,
            },
          };
        }
        return res.json();
      ...
      .then((data) => {
        if (data && data.error && data.error.message && data.status !== 200) {
          ...
          toast({
            title: "Data Error",
            description: "Received unexpected data format for DPPs.",
            variant: "destructive",
          });
          setDpps([]);
        }
      ...
      .catch((err) => {
        ...
        toast({
          title: "Fetching DPPs Failed",
          description:
            err.message || "Network error or failed to parse response.",
          variant: "destructive",
        });
        setDpps([]);
      })
  ...
  const handleSelectProduct = (dpp: DigitalProductPassport | null) => {
    ...
    if (dpp) {
      ...
      setMintMetadataUri(
        tokenId
          ? `ipfs://dpp_metadata_for_${tokenId}`
          : `ipfs://dpp_metadata_for_${dpp.id}`,
      );
      ...
      setNftContractAddress(
        dpp.ownershipNftLink?.contractAddress || contractAddr || "",
      );
      setNftTokenId(dpp.ownershipNftLink?.tokenId || tokenId || "");
      setNftChainName(
        dpp.ownershipNftLink?.chainName ||
          dpp.blockchainIdentifiers?.platform ||
          "",
      );
  ...
  const handleAnchor = async (e: FormEvent) => {
    ...
    if (!anchorPlatform) {
      toast({
        ...
        description: "Please enter a blockchain platform.",
        variant: "destructive",
      });
      return;
    }
    ...
    if (res.ok) {
      ...
      toast({
        title: "DPP Anchored",
        description: `Product ${selected.productName} successfully anchored to ${anchorPlatform}. Mock contract, token ID, and tx hash also set.`,
      });
    } else {
  ...
  const handleAddCustody = async (e: FormEvent) => {
    ...
    if (!selected) return;
    if (
      !custodyStep.stepName ||
      !custodyStep.actorDid ||
      !custodyStep.timestamp
    ) {
      toast({
        title: "Validation Error",
        description: "Please fill in step name, actor DID, and timestamp.",
        variant: "destructive",
      });
      return;
    }
    ...
    if (res.ok) {
      ...
      toast({
        ...
        description: `New custody step added to ${selected.productName}.`,
      });
    } else {
  ...
  const handleTransfer = async (e: FormEvent) => {
    ...
    if (!selected) {
      toast({
        ...
        description: "Please select a product to transfer ownership.",
        variant: "destructive",
      });
      return;
    }
    if (!transferName || !transferTime) {
      toast({
        title: "Validation Error",
        description: "Please fill in new owner name and transfer timestamp.",
        variant: "destructive",
      });
      return;
    }
    ...
    const res = await fetch(`/api/v1/dpp/transfer-ownership/${selected.id}`, {
      ...
      body: JSON.stringify({
        newOwner: { name: transferName, did: transferDid || undefined },
        transferTimestamp: transferTime,
      }),
    });
    if (res.ok) {
      const data: DigitalProductPassport = await res.json();
      ...
      toast({
        ...
        description: `Ownership of ${selected.productName} transferred to ${transferName}.`,
      });
    } else {
  ...
  const fetchAndDisplayCredential = async (productId: string) => {
    ...
    await new Promise((resolve) =>
      setTimeout(resolve, MOCK_TRANSACTION_DELAY / 2),
    );
    ...
    if (res.ok) {
      ...
      toast({
        ...
        description: "Verifiable credential data fetched and displayed.",
      });
    } else {
  ...
  const handleMintToken = async (e: FormEvent) => {
    ...
    try {
      ...
      if (res.ok && data.tokenId && selected) {
        toast({
          title: "Token Mint Initiated (Mock)",
          description: `Token ${data.tokenId} for ${selected.productName} minted. Tx: ${data.transactionHash}`,
        });
        const updatedSelectedDpp: DigitalProductPassport = {
          ...
          blockchainIdentifiers: {
            ...
            contractAddress:
              ...
              selected.blockchainIdentifiers?.contractAddress,
          },
        };
  ...
  const handleUpdateTokenMetadata = async (e: FormEvent) => {
    ...
    if (!updateTokenId) {
      toast({
        ...
        description: "Please enter a Token ID to update metadata.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        toast({
          title: "Token Metadata Update Initiated (Mock)",
          description: `Metadata for token ${updateTokenId} updated. Tx: ${data.transactionHash}`,
        });
      } else {
      ...
      const errorMsg =
        error instanceof Error ? error.message : "An unknown error occurred";
      setUpdateTokenResponse({
        error: "Client-side error",
        message: errorMsg,
      } as any);
      toast({
        title: "Updating Token Metadata Failed",
        description: errorMsg,
        variant: "destructive",
      });
    }
  ...
  const handleGetTokenStatus = async (e: FormEvent) => {
    ...
    if (!statusTokenId) {
      toast({
        title: "Token ID Required",
        description: "Please enter a Token ID to get its status.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast({
          title: "Token Status Retrieved (Mock)",
          description: `Status for token ${statusTokenId} displayed.`,
        });
      } else {
      ...
      const errorMsg =
        error instanceof Error ? error.message : "An unknown error occurred";
      setStatusTokenResponse(
        JSON.stringify(
          { error: "Client-side error", message: errorMsg },
          null,
          2,
        ),
      );
      toast({
        title: "Getting Token Status Failed",
        description: errorMsg,
        variant: "destructive",
      });
    }
  ...
  const handleViewTokenMetadata = async (e: FormEvent) => {
    ...
    if (!viewTokenId && !parsedTokenStatus?.metadataUri) {
      toast({
        title: "Token ID or URI Required",
        description:
          "Please ensure a Token ID is entered or Get Token Status was run.",
        variant: "destructive",
      });
      return;
    }
    ...
    const metadataUriToFetch =
      parsedTokenStatus?.metadataUri ||
      `ipfs://dpp_metadata_for_${viewTokenId || selected?.id}`;
    ...
    const mockMetadata = {
      ...
      image:
        selected?.productDetails?.imageUrl ||
        "https://placehold.co/300x300.png?text=DPP+NFT",
      external_url: selected
        ? `https://norruva.example.com/passport/${selected.id}`
        : "https://norruva.example.com",
      attributes: [
        ...
        { trait_type: "Category", value: selected?.category || "N/A" },
        {
          ...
          value: selected?.manufacturer?.name || "N/A",
        },
        { trait_type: "GTIN", value: selected?.gtin || "N/A" },
        {
          trait_type: "DPP Standard Version",
          value: selected?.metadata?.dppStandardVersion || "N/A",
        },
        ...(selected?.productDetails?.customAttributes || []).map((attr) => ({
          trait_type: attr.key,
          value: attr.value,
        })),
      ],
    ...
    toast({
      title: "Token Metadata Fetched (Mock)",
      description: `Displaying conceptual off-chain metadata for token URI: ${metadataUriToFetch}`,
    });
    ...
    try {
      ...
      if (res.ok && data.updatedProduct) {
        ...
        toast({
          ...
          description:
            data.message ||
            `Conceptual status for ${selected.productName} updated to '${onChainStatusUpdate}'.`,
        });
      } else {
  ...
  const handleLogCriticalEvent = async (e: FormEvent) => {
    ...
    if (!selected || !criticalEventDescription.trim()) {
      toast({
        title: "Missing Info",
        description: "Please select a product and enter an event description.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      const res = await fetch(`/api/v1/dpp/${selected.id}/log-critical-event`, {
        ...
        body: JSON.stringify({
          eventDescription: criticalEventDescription,
      ...
      if (res.ok && data.updatedProduct) {
        ...
        toast({
          ...
          description:
            data.message || `Event logged for ${selected.productName}.`,
        });
  ...
  const handleUpdateOnChainLifecycleStage = async (e: FormEvent) => {
    ...
    try {
      const res = await fetch(
        `/api/v1/dpp/${selected.id}/onchain-lifecycle-stage`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (res.ok && data.updatedProduct) {
        ...
        toast({
          ...
          description:
            data.message ||
            `Conceptual stage for ${selected.productName} updated to '${onChainLifecycleStage}'.`,
        });
      } else {
  ...
  const handleRegisterVcHash = async (e: FormEvent) => {
    ...
    if (!selected || !vcIdToRegister.trim() || !vcHashToRegister.trim()) {
      toast({
        title: "Missing Info",
        description: "Please select a product, enter a VC ID and VC Hash.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      if (res.ok && data.updatedProduct) {
        ...
        toast({
          ...
          description:
            data.message ||
            `VC Hash for ID '${vcIdToRegister}' registered for ${selected.productName}.`,
        });
        setVcIdToRegister("");
        setVcHashToRegister("");
      } else {
  ...
  const handleIssueAuthVc = async (e: FormEvent) => {
    ...
    if (!authVcProductId) {
      toast({
        ...
        description: "Please enter a Product ID to issue an Auth VC for.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        toast({
          title: "Authentication VC Issued (Mock)",
  ...
  const handleLinkNft = async (e: FormEvent) => {
    ...
    if (!nftProductId || !nftContractAddress || !nftTokenId) {
      toast({
        ...
        description:
          "Product ID, NFT Contract Address, and Token ID are required.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        toast({
          title: "Ownership NFT Linked (Mock)",
          description: data.message,
        });
        if (data.updatedProduct) updateDppLocally(data.updatedProduct);
      } else {
  ...
  const handleSubmitMockProposal = async (e: FormEvent) => {
    ...
    if (!proposalTargetTokenId && proposalActionType === "enableTransfer") {
      toast({
        title: "Token ID Required",
        description:
          "Please specify a Target Token ID for 'Enable Transfer' action.",
        variant: "destructive",
      });
      return;
    }
    if (!proposalTargetAddress && proposalActionType.includes("ROLE")) {
      toast({
        title: "Target Address Required",
        description:
          "Please specify a Target Address for role management actions.",
        variant: "destructive",
      });
      return;
    }
    if (!proposalDescription.trim()) {
      toast({
        title: "Proposal Description Required",
        description: "Please provide a rationale for the proposal.",
        variant: "destructive",
      });
      return;
    }
  ...
  const handleVoteOnProposal = (
      ...
      prev.map((p) => {
        if (p.id === proposalId && p.status === "Voting Active") {
          return {
            ...
            votesFor:
              voteType === "for"
                ? (p.votesFor || 0) + (Math.floor(Math.random() * 50) + 10)
                : p.votesFor,
            votesAgainst:
              voteType === "against"
                ? (p.votesAgainst || 0) + (Math.floor(Math.random() * 20) + 5)
                : p.votesAgainst,
          };
        }
        return p;
    ...
    toast({
      title: "Vote Recorded (Mock)",
      description: `Your vote on proposal ${proposalId} has been conceptually recorded.`,
    });
  };

  const renderApiResult = (
    title: string,
    responseString: string | null | object,
    ...
    if (typeof responseString === "string") {
      ...
      try {
        ...
        /* ignore parsing error for raw string display */
      }
    } else {
      ...
      return (
        <details className="mt-2 text-xs">
          <summary className="cursor-pointer text-muted-foreground hover:text-foreground">
            View Parsed & Raw Credential Response
          </summary>
          <div className="mt-1 p-3 border rounded-md bg-muted/30 space-y-1.5">
            ...
            {parsedContent.id && (
              <div className="flex items-center gap-1">
                ...
                <strong>ID:</strong>{" "}
                <span className="font-mono break-all">{parsedContent.id}</span>
              </div>
            )}
            {parsedContent.type && Array.isArray(parsedContent.type) && (
              <div className="flex items-center gap-1">
                <Sigma className="h-3 w-3 text-muted-foreground" />
                <strong>Type:</strong> {parsedContent.type.join(", ")}
              </div>
            )}
            {parsedContent.issuer && (
              <div className="flex items-center gap-1">
            ...
            {parsedContent.issuanceDate && (
              <div className="flex items-center gap-1">
                <CalendarDays className="h-3 w-3 text-muted-foreground" />
                <strong>Issued:</strong>{" "}
                {new Date(parsedContent.issuanceDate).toLocaleString()}
              </div>
            )}
            {parsedContent.credentialSubject && (
              <div className="flex items-start gap-1">
                ...
                <pre className="font-mono break-all text-xs bg-background/50 p-1 rounded max-w-full overflow-x-auto">
                  {JSON.stringify(
                    parsedContent.credentialSubject,
                    null,
                    1,
    ...
    return (
      <details className="mt-2 text-xs">
        <summary className="cursor-pointer text-muted-foreground hover:text-foreground">
          View Mock {title} Response (Raw JSON)
        </summary>
    ...
    <div className="space-y-6 p-4 md:p-6 lg:p-8">
      ...
      <Card className="shadow-md bg-muted/30 border-primary/10">
        ...
        <CardContent className="text-sm text-foreground/90 space-y-2">
          <p>
            This page provides tools to manage on-chain aspects of your Digital
            Product Passports (DPPs). You can simulate:
          </p>
          <ul className="list-disc list-inside pl-4 space-y-1">
            <li>
              <strong>Anchoring DPPs:</strong> Recording DPP data hashes and key
              identifiers on a mock blockchain.
            </li>
            <li>
              <strong>Updating Chain of Custody:</strong> Documenting product
              transfers and lifecycle events.
            </li>
            <li>
              <strong>Transferring Ownership:</strong> Modifying DPP ownership
              records.
            </li>
            <li>
              <strong>Managing Verifiable Credentials:</strong> Viewing and
              retrieving product-related VCs.
            </li>
            <li>
              <strong>DPP Token Operations:</strong> Conceptual minting,
              metadata updates, and status checks for DPP tokens.
            </li>
            <li>
              <strong>Smart Contract Actions:</strong> Conceptual interactions
              for on-chain status updates and event logging.
            </li>
            <li>
              <strong>Authenticity & Ownership:</strong> Issuing conceptual VCs
              for authenticity and linking NFTs for ownership.
            </li>
            <li>
              <strong>DAO Governance:</strong> Simulating DAO proposals for
              token management.
            </li>
          </ul>
          <p className="mt-2">
            These operations interact with mock API endpoints. For technical
            ...
            >
              ...
              <ExternalLink className="inline h-3.5 w-3.5 ml-0.5" />
            </Link>{" "}
            and the conceptual{" "}
            <Link
              href="/developer/docs/smart-contract-interactions"
                        ...
                        <TableCell colSpan={4} className="py-4">
                          <div className="p-4 border rounded-md space-y-6 bg-card">
                            <h3 className="text-lg font-semibold text-primary mb-2">
                              Managing: {selected.productName} ({selected.id})
                            </h3>
                            <div className="grid md:grid-cols-2 gap-6">
                              ...
                              {dpp.blockchainIdentifiers
                                ?.anchorTransactionHash ? (
                                <Card className="bg-background">
                                  <CardHeader>
                                    <CardTitle className="text-md flex items-center">
                                      <Anchor className="mr-2 h-4 w-4 text-green-600" />
                                      DPP Anchoring Status
                                    </CardTitle>
                                  </CardHeader>
                                  <CardContent className="space-y-1.5 text-sm">
                                    ...
                                    {dpp.blockchainIdentifiers.tokenId && (
                                      <p>
                                        <strong className="text-muted-foreground">
                                          Token ID:
                                        </strong>{" "}
                                    ...
                                    <Link
                                      href={`${TX_EXPLORER_BASE_URL}${dpp.blockchainIdentifiers.anchorTransactionHash}`}
                                      target="_blank"
                                      rel="noopener noreferrer"
                                ...
                                <Card className="bg-background">
                                  <CardHeader>
                                    <CardTitle className="text-md flex items-center">
                                      <Anchor className="mr-2 h-4 w-4 text-info" />
                                      Anchor DPP on Blockchain
                                    </CardTitle>
                                  </CardHeader>
                                  <CardContent>
                                    <form
                                    ...
                                    >
                                      <Input
                                        ...
                                        onChange={(e) =>
                                          setAnchorPlatform(e.target.value)
                                        }
                                        placeholder="Blockchain Platform (e.g., EBSI)"
                                        className="flex-grow sm:flex-grow-0 sm:w-60"
                                      />
                                      <Button
                                        type="submit"
                                        size="sm"
                                        disabled={
                                          isActionLoading === "anchor" ||
                                          !anchorPlatform.trim()
                                        }
                                      >
                                        {isActionLoading === "anchor" ? (
                              ...
                              <Card className="bg-background">
                                ...
                                <CardContent>
                                  ...
                                  <ul className="space-y-2 text-xs mb-3 max-h-32 overflow-y-auto border p-2 rounded-md bg-muted/30">
                                    {(dpp.traceability?.supplyChainSteps || [])
                                      .length > 0 ? (
                                      (
                                        dpp.traceability.supplyChainSteps || []
                                      ).map((step, idx) => (
                                        <li
                                          key={idx}
                                          className="p-1.5 border-b last:border-b-0 bg-background/50 rounded-sm"
                                        >
                                  ...
                                  >
                                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2 items-end">
                                      ...
                                      <Input
                                        ...
                                        value={custodyStep.actorDid}
                                        onChange={(e) =>
                                          setCustodyStep({
                                            ...custodyStep,
                                            actorDid: e.target.value,
                                          })
                                        }
                                      />
                                      <Input
                                        type="datetime-local"
                                        value={custodyStep.timestamp}
                                        onChange={(e) =>
                                          setCustodyStep({
                                            ...custodyStep,
                                      ...
                                      <Input
                                        ...
                                        value={custodyStep.transactionHash}
                                        onChange={(e) =>
                                          setCustodyStep({
                                            ...custodyStep,
                                      ...
                                      <Button
                                        type="submit"
                                        size="sm"
                                        className="sm:col-start-3 md:col-start-auto"
                                        disabled={isActionLoading === "custody"}
                                      >
                                        {isActionLoading === "custody" ? (
                                          <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                                        ) : (
                              ...
                              <Card className="bg-background">
                                ...
                                <CardContent>
                                  <h4 className="font-medium mb-1.5 text-sm">
                                    Linked VCs in Product Data:
                                  </h4>
                                  {dpp.verifiableCredentials &&
                                  dpp.verifiableCredentials.length > 0 ? (
                                    <ul className="space-y-2 text-xs mb-3 max-h-40 overflow-y-auto border p-2 rounded-md bg-muted/30">
                                      {dpp.verifiableCredentials.map(
                                        (vc, idx) => (
                                          <li
                                            key={idx}
                                            className="border-b last:border-b-0 pb-1.5 p-1.5 bg-background/50 rounded-sm"
                                          >
                                            <p className="font-semibold">
                                              {vc.name || vc.type.join(", ")}
                                            </p>
                                            <p
                                              className="truncate"
                                              title={vc.id}
                                    ...
                                    <p className="text-xs text-muted-foreground mb-2">
                                      No verifiable credentials directly linked
                                      in main product record.
                                    </p>
                                  )}
                                  <Button
                                    size="sm"
                                    variant="secondary"
                                    onClick={() =>
                                      fetchAndDisplayCredential(dpp.id)
                                    }
                                    className="w-full sm:w-auto"
                                    disabled={
                                      isActionLoading === "fetchCredential"
                                    }
                                  >
                                    {isActionLoading === "fetchCredential" ? (
                                      <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                              ...
                              <Card className="bg-background">
                                <CardHeader>
                                  <CardTitle className="text-md flex items-center">
                                    <UploadCloud className="mr-2 h-4 w-4 text-info" />
                                    Transfer Ownership
                                  </CardTitle>
                                </CardHeader>
                                <CardContent>
                                  ...
                                  >
                                    ...
                                    />
                                    <Input
                                      placeholder="New Owner DID (Optional)"
                                      value={transferDid}
                                      onChange={(e) =>
                                        setTransferDid(e.target.value)
                                      }
                                    />
                                    <Input
                                      type="datetime-local"
                                    ...
                                    <Button
                                      ...
                                      size="sm"
                                      className="w-full"
                                      disabled={isActionLoading === "transfer"}
                                    >
                                      {isActionLoading === "transfer" ? (
                                        <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                                      ) : (
                                        <Send className="mr-2 h-4 w-4" />
                                      )}
                            ...
                            <Card className="bg-background mt-6 md:col-span-2">
                              <CardHeader>
                                <CardTitle className="text-md flex items-center">
                                  <ListCollapse className="mr-2 h-4 w-4 text-info" />
                                  Conceptual Smart Contract &amp; Public Layer
                                  Actions
                                </CardTitle>
                                <CardDescription className="text-xs">
                                  Simulate direct interactions with a DPP smart
                                  contract or public layer operations.
                                </CardDescription>
                              </CardHeader>
                              <CardContent className="grid sm:grid-cols-2 gap-x-6 gap-y-8">
                                <form
                                  onSubmit={handleUpdateOnChainStatus}
                                  className="space-y-3 p-3 border rounded-md"
                                >
                                  <h4 className="font-medium text-sm flex items-center">
                                    <Sigma className="h-4 w-4 mr-1.5 text-primary" />
                                    Update On-Chain DPP Status
                                  </h4>
                                  <p className="text-xs text-muted-foreground">
                                    Simulate updating the product's status on
                                    the blockchain (e.g., after recall).
                                  </p>
                                  <Select
                                    value={onChainStatusUpdate}
                                    onValueChange={setOnChainStatusUpdate}
                                  >
                                    <SelectTrigger>
                                      <SelectValue />
                                    </SelectTrigger>
                                  ...
                                  <Button
                                    type="submit"
                                    size="sm"
                                    disabled={isUpdatingOnChainStatusLoading}
                                  >
                                    {isUpdatingOnChainStatusLoading ? (
                                      <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                                    ) : (
                                ...
                                <form
                                  onSubmit={handleLogCriticalEvent}
                                  className="space-y-3 p-3 border rounded-md"
                                >
                                  <h4 className="font-medium text-sm flex items-center">
                                    <MessageSquareWarning className="h-4 w-4 mr-1.5 text-primary" />
                                    Log Critical Event On-Chain
                                  </h4>
                                  <p className="text-xs text-muted-foreground">
                                    Simulate recording a critical event (e.g.,
                                    major defect, safety recall details).
                                  </p>
                                  <Textarea
                                    value={criticalEventDescription}
                                    onChange={(e) =>
                                      setCriticalEventDescription(
                                        e.target.value,
                                      )
                                  ...
                                  />
                                  <Select
                                    value={criticalEventSeverity}
                                    onValueChange={(value) =>
                                      setCriticalEventSeverity(
                                  ...
                                  <Button
                                    type="submit"
                                    size="sm"
                                    disabled={isLoggingCriticalEventLoading}
                                  >
                                    {isLoggingCriticalEventLoading ? (
                                      <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                                    ) : (
                                ...
                                <form
                                  onSubmit={handleUpdateOnChainLifecycleStage}
                                  className="space-y-3 p-3 border rounded-md"
                                >
                                  <h4 className="font-medium text-sm flex items-center">
                                    <Layers className="h-4 w-4 mr-1.5 text-primary" />
                                    Update DPP Lifecycle Stage On-Chain
                                  </h4>
                                  <p className="text-xs text-muted-foreground">
                                    Simulate updating the product's lifecycle
                                    stage on the blockchain.
                                  </p>
                                  <Select
                                    value={onChainLifecycleStage}
                                    onValueChange={setOnChainLifecycleStage}
                                  >
                                    <SelectTrigger>
                                  ...
                                  <Button
                                    type="submit"
                                    size="sm"
                                    disabled={isUpdatingLifecycleStageLoading}
                                  >
                                    {isUpdatingLifecycleStageLoading ? (
                                      <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                                    ) : (
                                ...
                                <form
                                  onSubmit={handleRegisterVcHash}
                                  className="space-y-3 p-3 border rounded-md"
                                >
                                  <h4 className="font-medium text-sm flex items-center">
                                    <Hash className="h-4 w-4 mr-1.5 text-primary" />
                                    Register Verifiable Credential Hash On-Chain
                                  </h4>
                                  <p className="text-xs text-muted-foreground">
                                    Simulate registering a VC hash on the
                                    blockchain for integrity verification.
                                  </p>
                                  <Input
                                    value={vcIdToRegister}
                                    onChange={(e) =>
                                      setVcIdToRegister(e.target.value)
                                    }
                                    placeholder="Verifiable Credential ID (e.g., urn:uuid:...)"
                                  />
                                  <Input
                                    value={vcHashToRegister}
                                    onChange={(e) =>
                                      setVcHashToRegister(e.target.value)
                                    }
                                    placeholder="VC Hash (SHA256)"
                                  />
                                  <Button
                                    type="submit"
                                    size="sm"
                                    disabled={isRegisteringVcHashLoading}
                                  >
                                    {isRegisteringVcHashLoading ? (
                                      <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                                    ) : (
                                    ...
                                    {isRegisteringVcHashLoading
                                      ? "Registering..."
                                      : "Register Mock VC Hash"}
                                  </Button>
                                </form>
                                <form
                                  onSubmit={handleIssueAuthVc}
                                  className="space-y-3 p-3 border rounded-md"
                                >
                                  <h4 className="font-medium text-sm flex items-center">
                                    <FileLock className="h-4 w-4 mr-1.5 text-primary" />
                                    Issue Authentication VC (Mock)
                                  </h4>
                                  <p className="text-xs text-muted-foreground">
                                    Simulate issuing a Verifiable Credential
                                    attesting to product authenticity.
                                  </p>
                                  <Input
                                    value={authVcProductId}
                                    onChange={(e) =>
                                      setAuthVcProductId(e.target.value)
                                    }
                                    placeholder="Product ID (pre-filled)"
                                  />
                                  <Button
                                    type="submit"
                                    size="sm"
                                    disabled={
                                      isActionLoading === "issueAuthVc" ||
                                      !authVcProductId
                                    }
                                  >
                                    {isActionLoading === "issueAuthVc" ? (
                                ...
                                <form
                                  onSubmit={handleLinkNft}
                                  className="space-y-3 p-3 border rounded-md"
                                >
                                  <h4 className="font-medium text-sm flex items-center">
                                    <Tag className="h-4 w-4 mr-1.5 text-primary" />
                                    Link Ownership NFT (Mock)
                                  </h4>
                                  <p className="text-xs text-muted-foreground">
                                    Simulate linking an NFT representing product
                                    ownership.
                                  </p>
                                  <Input
                                    value={nftProductId}
                                    onChange={(e) =>
                                      setNftProductId(e.target.value)
                                    }
                                    placeholder="Product ID (pre-filled)"
                                  />
                                  <Input
                                    value={nftRegistryUrl}
                                    onChange={(e) =>
                                      setNftRegistryUrl(e.target.value)
                                    }
                                    placeholder="NFT Registry URL (e.g., OpenSea, Rarible)"
                                  />
                                  <Input
                                  ...
                                  <Input
                                    value={nftChainName}
                                    onChange={(e) =>
                                      setNftChainName(e.target.value)
                                    }
                                    placeholder="Blockchain Name (e.g., Ethereum)"
                                  />
                                  <Button
                                    type="submit"
                                    size="sm"
                                    disabled={
                                      isActionLoading === "linkNft" ||
                                      !nftProductId ||
                                      !nftContractAddress ||
                                      !nftTokenId
                                    }
                            ...
                            <div className="md:col-span-2 mt-6">
                              <h3 className="text-xl font-semibold text-foreground mb-3 flex items-center">
                                ...
                                DPP Token Operations (Conceptual)
                              </h3>
                              <p className="text-sm text-muted-foreground mb-4">
                                These operations simulate interactions with
                                smart contracts related to DPP tokens. For
                                details on the underlying design, refer to the
                                project's{" "}
                                <Link
                                  href="/developer/docs/smart-contract-interactions"
                                  className="text-primary hover:underline"
                              ...
                              <div className="grid md:grid-cols-2 gap-6">
                                <Card className="bg-background">
                                  <CardHeader>
                                    ...
                                    <CardDescription className="text-xs">
                                      Simulates creating a unique blockchain
                                      token (e.g., ERC-721 NFT) representing
                                      this DPP.
                                    </CardDescription>
                                  </CardHeader>
                                  <CardContent>
                                    ...
                                    >
                                      <Input
                                        ...
                                        onChange={(e) =>
                                          setMintContractAddress(e.target.value)
                                        }
                                      ...
                                      <Input
                                        ...
                                        onChange={(e) =>
                                          setMintMetadataUri(e.target.value)
                                        }
                                      ...
                                      {mintResponse?.transactionHash && (
                                        <p className="text-xs text-muted-foreground mt-1">
                                          ...
                                          Tx Hash:{" "}
                                          <Link
                                            href={`${TX_EXPLORER_BASE_URL}${mintResponse.transactionHash}`}
                                            target="_blank"
                                            rel="noopener noreferrer"
                                      ...
                                      {renderApiResult(
                                        "Mint Token",
                                        mintResponse,
                                        !!(mintResponse && mintResponse.error),
                                      )}
                                    </form>
                                  </CardContent>
                                </Card>
                                <Card className="bg-background">
                                  <CardHeader>
                                    ...
                                    <CardDescription className="text-xs">
                                      Simulates updating the on-chain metadata
                                      URI (e.g., tokenURI) for an existing
                                      token.
                                    </CardDescription>
                                  </CardHeader>
                                  <CardContent>
                                    <form
                                      onSubmit={handleUpdateTokenMetadata}
                                      className="space-y-3"
                                    >
                                      <Input
                                        value={updateTokenId}
                                        onChange={(e) =>
                                          setUpdateTokenId(e.target.value)
                                        }
                                        placeholder={
                                          selected?.blockchainIdentifiers
                                            ?.tokenId
                                            ? `Token ID (e.g., ${selected.blockchainIdentifiers.tokenId})`
                                            : "Enter Token ID"
                                        }
                                      />
                                      <Input
                                        ...
                                        onChange={(e) =>
                                          setUpdateMetadataUri(e.target.value)
                                        }
                                        placeholder="New Metadata URI (e.g., ipfs://...)"
                                      />
                                      <Button
                                        type="submit"
                                        size="sm"
                                      ...
                                      >
                                        ...
                                        {isActionLoading === "updateTokenMeta"
                                          ? "Processing..."
                                          : "Update Metadata"}
                                      </Button>
                                      {updateTokenResponse?.transactionHash && (
                                        <p className="text-xs text-muted-foreground mt-1">
                                          Tx Hash:{" "}
                                          <Link
                                            href={`${TX_EXPLORER_BASE_URL}${updateTokenResponse.transactionHash}`}
                                            target="_blank"
                                            rel="noopener noreferrer"
                                            className="text-primary hover:underline font-mono break-all"
                                          >
                                ...
                                <Card className="bg-background">
                                  <CardHeader>
                                    <CardTitle className="text-md flex items-center">
                                      <InfoIconLucide className="h-4 w-4 mr-1.5 text-primary" />
                                      Get Token Status
                                    </CardTitle>
                                    <CardDescription className="text-xs">
                                      Simulates fetching the current on-chain
                                      status of a token.
                                    </CardDescription>
                                  </CardHeader>
                                  <CardContent>
                                    <form
                                      onSubmit={handleGetTokenStatus}
                                      className="space-y-3"
                                    >
                                      <Input
                                        value={statusTokenId}
                                        onChange={(e) =>
                                          setStatusTokenId(e.target.value)
                                        }
                                        placeholder={
                                          selected?.blockchainIdentifiers
                                            ?.tokenId
                                      ...
                                      >
                                        ...
                                        "gettingTokenStatus" ? (
                                          ...
                                          <InfoIconLucide className="mr-2 h-4 w-4" />
                                        )}
                                        {isActionLoading ===
                                        "gettingTokenStatus"
                                          ? "Fetching..."
                                      ...
                                      {parsedTokenStatus && (
                                        <div className="mt-2 space-y-1.5 p-3 text-xs border rounded bg-muted/30">
                                          <div className="flex items-center justify-between">
                                            ...
                                            <span className="font-mono text-foreground/90 break-all">
                                              {parsedTokenStatus.ownerAddress}
                                            </span>
                                          </div>
                                          <div className="flex items-center justify-between">
                                            <strong className="text-muted-foreground">
                                          ...
                                          <div className="flex items-center justify-between">
                                            ...
                                            <span className="text-foreground/90">
                                              {new Date(
                                                parsedTokenStatus.mintedAt,
                                              ).toLocaleString()}
                                            </span>
                                          </div>
                                          {parsedTokenStatus.lastTransactionHash && (
                                            <div className="flex items-center justify-between">
                                              <strong className="text-muted-foreground">
                                                Last Tx:
                                              </strong>{" "}
                                              <Link
                                                href={`${TX_EXPLORER_BASE_URL}${parsedTokenStatus.lastTransactionHash}`}
                                                target="_blank"
                                                rel="noopener noreferrer"
                                                className="text-primary hover:underline font-mono break-all"
                                              >
                                ...
                                <Card className="bg-background">
                                  <CardHeader>
                                    <CardTitle className="text-md flex items-center">
                                      <FileJson className="h-4 w-4 mr-1.5 text-primary" />
                                      View Off-Chain Token Metadata
                                    </CardTitle>
                                    <CardDescription className="text-xs">
                                      Simulates fetching and displaying the JSON
                                      metadata linked by the token's URI (e.g.,
                                      from IPFS).
                                    </CardDescription>
                                  </CardHeader>
                                  <CardContent>
                                    <form
                                      onSubmit={handleViewTokenMetadata}
                                      className="space-y-3"
                                    >
                                      <Input
                                        value={viewTokenId}
                                        onChange={(e) =>
                                          setViewTokenId(e.target.value)
                                        }
                                        placeholder={
                                          selected?.blockchainIdentifiers
                                            ?.tokenId ||
                                          parsedTokenStatus?.tokenId ||
                                          "Enter Token ID or Get Status"
                                        }
                                      />
                                      <Button
                                        type="submit"
                            ...
                            <Card className="bg-background mt-6 md:col-span-2">
                              <CardHeader>
                                <CardTitle className="text-md flex items-center">
                                  <DaoIcon className="mr-2 h-5 w-5 text-primary" />{" "}
                                  DAO Governance & Proposals (Conceptual)
                                </CardTitle>
                                <CardDescription className="text-xs">
                                  Simulate creating and interacting with DAO
                                  proposals for DPPToken management.
                                </CardDescription>
                              </CardHeader>
                              <CardContent className="space-y-6">
                                <form
                                  onSubmit={handleSubmitMockProposal}
                                  className="p-4 border rounded-md space-y-4 bg-muted/30"
                                >
                                  <h4 className="font-medium text-sm text-foreground">
                                    Create New Mock Proposal
                                  </h4>
                                  <div>
                                    <Label htmlFor="proposalTokenId">
                                      Target Token ID (for 'Enable Transfer')
                                    </Label>
                                    <Input
                                      id="proposalTokenId"
                                      value={proposalTargetTokenId}
                                      onChange={(e) =>
                                        setProposalTargetTokenId(e.target.value)
                                      }
                                      placeholder="e.g., DPP001"
                                      disabled={
                                        proposalActionType !== "enableTransfer"
                                      }
                                    />
                                  </div>
                                  <div>
                                    <Label htmlFor="proposalActionType">
                                      Action Type
                                    </Label>
                                    <Select
                                      value={proposalActionType}
                                      onValueChange={setProposalActionType}
                                    >
                                      <SelectTrigger id="proposalActionType">
                                        <SelectValue />
                                      </SelectTrigger>
                                      <SelectContent>
                                        <SelectItem value="enableTransfer">
                                          Enable Transfer for Token
                                        </SelectItem>
                                        <SelectItem value="grantMinterRole">
                                          Grant MINTER_ROLE
                                        </SelectItem>
                                        <SelectItem value="grantUpdaterRole">
                                          Grant UPDATER_ROLE
                                        </SelectItem>
                                        <SelectItem value="revokeMinterRole">
                                          Revoke MINTER_ROLE
                                        </SelectItem>
                                      </SelectContent>
                                    </Select>
                                  ...
                                  {proposalActionType.includes("ROLE") && (
                                    <div>
                                      <Label htmlFor="proposalTargetAddress">
                                        Target Address (for Role Action)
                                      </Label>
                                      <Input
                                        id="proposalTargetAddress"
                                ...
                                <div>
                                  ...
                                  <div className="space-y-3 max-h-60 overflow-y-auto">
                                    {daoProposals.map((prop) => (
                                      <Card
                                        key={prop.id}
                                        className="p-3 bg-background/70"
                                      >
                                          ...
                                          <div className="flex items-center justify-between text-xs">
                                            <div className="flex gap-2">
                                              <Button
                                                variant="ghost"
                                                size="sm"
                                                className="text-green-600 hover:bg-green-500/10 h-auto p-1 text-xs"
                                                onClick={() =>
                                              ...
                                              <Button
                                                variant="ghost"
                                                size="sm"
                                                className="text-red-600 hover:bg-red-500/10 h-auto p-1 text-xs"
                                                onClick={() =>
                                                  handleVoteOnProposal(
                                                    prop.id,
                                                    "against",
