...
describe("DPPGovernor", function () {
  ...
  beforeEach(async function () {
    ...
    await targetContract.connect(owner).transferOwnership(await timelock.getAddress()); 
  });
  ...
  describe("Deployment & Configuration", function () {
    ...
    it("Governor should have DEFAULT_ADMIN_ROLE for itself", async function () {
      expect(await governor.hasRole(DEFAULT_ADMIN_ROLE, await governor.getAddress())).to.be.true;
    });
  ...
  describe("Proposal Lifecycle", function () {
    it("Should allow proposal creation, voting, queuing, and execution", async function () {
      ...
      if (receipt && receipt.logs) {
         const eventLog = receipt.logs.find(log => log.address === await governor.getAddress() && governor.interface.hasFunction("ProposalCreated"));
         if (eventLog) {
    ...
    it("Should allow admin (Governor) to cancel a proposal before it's executed (via proposal)", async function () {
      ...
      let proposalToCancelId: any;
       if (receiptCancel && receiptCancel.logs) {
         const eventLog = receiptCancel.logs.find(log => log.address === await governor.getAddress() && governor.interface.hasFunction("ProposalCreated"));
         if (eventLog) {
            const parsedLog = governor.interface.parseLog(eventLog as any);
            if (parsedLog && parsedLog.name === "ProposalCreated") {
      ...
      if (receiptMainCancel && receiptMainCancel.logs) {
         const eventLog = receiptMainCancel.logs.find(log => log.address === await governor.getAddress() && governor.interface.hasFunction("ProposalCreated"));
         if (eventLog) {
            const parsedLog = governor.interface.parseLog(eventLog as any);
            if (parsedLog && parsedLog.name === "ProposalCreated") {
      ...
      await expect(governor.connect(proposer).execute([await governor.getAddress()], [0], [cancelCallData], cancelProposalDescriptionHash))
        .to.emit(governor, "ProposalCanceled").withArgs(proposalToCancelId); 
