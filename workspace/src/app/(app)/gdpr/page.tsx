...
import { ShieldCheck, FileCog, Trash2, DownloadCloud, CheckCircle, Mail, UserCircle as UserCircleIcon } from "lucide-react"; // Renamed UserCircle to UserCircleIcon
...
export default function GdprPage() {
  ...
  const consentItems = [
    { id: "analytics" as keyof ConsentSettings, label: "Analytics Cookies", description: "Allow collection of anonymized usage data to improve our services." },
    { id: "marketing" as keyof ConsentSettings, label: "Marketing Communications", description: "Receive promotional emails, newsletters, and special offers." },
    { id: "personalization" as keyof ConsentSettings, label: "Content Personalization", description: "Allow us to tailor content and recommendations based on your activity." },
  ];
  ...
  const handleSaveConsent = () => {
    ...
    toast({
      ...
      description: "Your consent preferences have been updated successfully (mock).",
  ...
  const handleRequestDeletion = () => {
    if (!deletionUserId.trim()) {
      toast({ title: "User ID Required", description: "Please enter a User ID or Email for data deletion.", variant: "destructive" });
      return;
    }
    // Simulate request
    if (process.env.NODE_ENV !== 'production') {
  ...
  const handleRequestExport = () => {
     if (!exportUserId.trim()) {
      toast({ title: "User ID Required", description: "Please enter a User ID or Email for data export.", variant: "destructive" });
      return;
    }
    // Simulate request
    if (process.env.NODE_ENV !== 'production') {
    ...
    <div className="space-y-8">
      ...
      <Card className="shadow-lg">
        <CardHeader>
          ...
          <CardDescription>Manage user consent preferences for data processing activities. Your choices here are simulated and will not affect actual platform behavior.</CardDescription>
        </CardHeader>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <Card className="shadow-lg">
          <CardHeader>
            ...
            <CardDescription>Process and manage user requests for data deletion (mock functionality).</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <Label htmlFor="userIdDeletion">User ID or Email for Deletion</Label>
            <Input 
              id="userIdDeletion" 
        ...
        <Card className="shadow-lg">
          <CardHeader>
            ...
            <CardDescription>Provide users with their data in a portable format (mock functionality).</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <Label htmlFor="userIdPortability">User ID or Email for Export</Label>
            <Input 
            ...
            />
            <Button className="w-full bg-accent text-accent-foreground hover:bg-accent/90" onClick={handleRequestExport}>
              Export User Data
            </Button>
            <p className="text-xs text-muted-foreground">
       ...
       <Card className="shadow-lg">
        <CardHeader>
          <CardTitle className="font-headline flex items-center"><UserCircleIcon className="mr-3 h-6 w-6 text-primary" />Data Protection Officer (DPO)</CardTitle>
          <CardDescription>Contact our DPO for any data protection inquiries or concerns.</CardDescription>
        </CardHeader>
        <CardContent>
          <p className="text-muted-foreground">
            The Data Processing Register is maintained internally and regularly audited.
          </p>
          <Button variant="link" className="p-0 h-auto mt-2" asChild>
            <a href="mailto:dpo@norruva-mock.com">
                <Mail className="mr-2 h-4 w-4"/> Contact Data Protection Officer (dpo@norruva-mock.com)
            </a>
          </Button>
        </CardContent>
      </Card>
